 ...
 *
 */
$footer_type = JobPortal_Helper::get_setting("footer_type");

$footer_type  = apply_filters('jobportal_footer_type', $footer_type);

$id = get_the_ID();
$footer_show = $footer_page = '';
if (!empty($id)) {
	$footer_page = get_post_meta($id, 'jobportal-footer_type', true);
	$footer_show = get_post_meta($id, 'jobportal-footer_show', true);
} else {
	$footer_page = '';
}
?>
...
<?php if ($footer_show !== '0') { ?>
	<footer class="site-footer">
		<div class="inner-footer">
			<?php if ($footer_page == '') { ?>
				<?php $footer_default = jobportal_get_footer_elementor();
				if (!$footer_type && is_array($footer_default) && !empty($footer_default)) {
					$footer_keys = array_keys($footer_default);
					$setkey = (count($footer_keys) > 1) ? count($footer_keys) - 1 : 0;
					$footer_type = $footer_keys[$setkey];
				}
				?>
				<?php if ($footer_type !== '') { ?>
					<?php
