	...
	<form action="<?php echo esc_url($canvas_action); ?>" method="get" class="form-search-canvas">
		...
		<div class="jobs-search-inner">
			...
			<div class="form-group">
				<input class="jobs-search-canvas archive-search-control" data-key='<?php esc_html_e($jobs_keyword) ?>' id="jobs_filter_search-canvas" type="text" name="s" placeholder="<?php esc_attr_e('Jobs title or keywords', 'jobportal') ?>" autocomplete="off">
				<span class="btn-filter-search"><i class="far fa-search"></i></span>
			</div>
		<div class="form-group">
			<div class="select2-field jobportal-form-location">
				<input class="input-search-location jobportal-ajax-ui" type="text" name="jobs-location"
					placeholder="<?php esc_attr_e('All Cities', 'jobportal') ?>" autocomplete="off" data-taxonomy="jobs-location">
				<span class="icon-arrow"><i class="fal fa-angle-down"></i></span>
			</div>
			<i class="fas fa-map-marker-alt"></i>
		</div>
			<div class="form-group">
				<button type="submit" class="btn-jobs-search jobportal-button">
					<?php esc_html_e('Search', 'jobportal') ?>
				</button>
			</div>
		</div>
