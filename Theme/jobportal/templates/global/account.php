...
<div class="popup popup-account <?php esc_html_e($class_open) ?>" id="popup-form">
	...
	<div class="inner-popup custom-scrollbar">
		...
		<div class="head-popup">
			<div class="tabs-form">
				<a class="btn-login active" href="#ux-login" data-form="ux-login"><?php esc_html_e('Log in', 'jobportal'); ?></a>
				<a class="btn-register" href="#ux-register" data-form="ux-register"><?php esc_html_e('Sign Up', 'jobportal'); ?></a>
				<div class="loading-effect"><span class="jobportal-dual-ring"></span></div>
			</div>
			<?php if (is_user_logged_in()) { ?>
				<p class="notice"><i class="fal fa-exclamation-circle"></i><?php esc_html_e('Please login role Candidate to view', 'jobportal'); ?></p>
			<?php } ?>
		</div>
		...
		<div class="body-popup">
			...
			?>
				...
				<div class="jobportal-new-password-wrap">
					<form action="#" method="post">
						<div class="form-group control-password">
							<input name="new_password" type="password" id="new-password" class="form-control control-icon" placeholder="<?php esc_attr_e('Enter new password', 'jobportal'); ?>">
							<span><i class="fas fa-eye"></i></span>
						</div>
						<div class="form-group control-password">
							<input name="confirm_password" type="password" id="confirm-password" class="form-control control-icon" placeholder="<?php esc_attr_e('Enter confirm password', 'jobportal'); ?>">
							<span><i class="fas fa-eye"></i></span>
						</div>
						<div class="button-wrap">
							<a href="#" class="generate-password"><?php esc_html_e('Generate', 'jobportal'); ?></a>
							<button type="submit" id="jobportal_newpass" class="btn gl-button"><?php esc_html_e('Save password', 'jobportal'); ?></button>
			...
			<?php else : ?>
				...
				<form action="#" class="form-account active ux-login" method="post">
					...
					<div class="form-group">
						<label for="ip_email" class="label-field"><?php esc_html_e('Account or Email', 'jobportal'); ?></label>
						<input type="text" id="ip_email" class="form-control input-field" name="email" placeholder="<?php esc_attr_e('Enter Account or Email', 'jobportal') ?>">
					</div>
					<div class="form-group">
						<label for="ip_password" class="label-field"><?php esc_html_e('Password', 'jobportal'); ?></label>
						<input type="password" id="ip_password" class="form-control input-field" name="password" autocomplete="on" placeholder="<?php esc_attr_e('Enter Password', 'jobportal') ?>">
						<span toggle="#ip_password" class="fa fa-fw fa-eye field-icon jobportal-toggle-password"></span>
					</div>
					...
					<form method="post" enctype="multipart/form-data">
						<div class="form-group control-username">
							<div class="form-field email-field">
								<input name="user_login" id="user_login" class="form-control control-icon" placeholder="<?php esc_attr_e('Enter your email', 'jobportal'); ?>" required>
								<a href="#" class="verify-email" data-title="<?php esc_attr_e('Resend', 'jobportal'); ?>"><?php echo esc_html__('Verify', 'jobportal'); ?></a>
							</div>
							<div class="form-field verify-field">
								<input name="verify_code" id="verify_code" class="form-control control-icon" placeholder="<?php esc_attr_e('Verify code', 'jobportal'); ?>">
							</div>
							<?php wp_nonce_field('jobportal_reset_password_ajax_nonce', 'jobportal_security_reset_password'); ?>
							<input type="hidden" name="action" id="reset_password_action" value="jobportal_reset_password_ajax">
				...
				<form action="#" class="form-account ux-register" method="post">
					...
					$enable_default_user_role = JobPortal_Helper::jobportal_get_option('enable_default_user_role');
					if ($enable_user_role) {
					?>
						<div class="form-group">
							<div class="row">
								<div class="col-6">
									<div class="col-group">
										<label for="jobportal_user_candidate" class="label-field radio-field">
											<input type="radio" value="jobportal_user_candidate" id="jobportal_user_candidate" name="account_type">
								...
								<div class="col-6">
									<div class="col-group">
										<label for="jobportal_user_employer" class="label-field radio-field">
											<input type="radio" value="jobportal_user_employer" id="jobportal_user_employer" name="account_type" checked>
					...
					<?php } else { ?>
						<?php if ($enable_default_user_role === 'employer') { ?>
							<input type="radio" checked value="jobportal_user_employer" id="jobportal_user_employer" name="account_type" class="hide">
						<?php } else { ?>
							<input type="radio" checked value="jobportal_user_candidate" id="jobportal_user_candidate" name="account_type" class="hide">
						<?php } ?>
					<?php } ?>
					<div class="form-group">
						<div class="row">
							<div class="col-6">
								<div class="col-group">
									<label for="ip_reg_firstname" class="label-field"><?php esc_html_e('First Name', 'jobportal'); ?></label>
									<input type="text" id="ip_reg_firstname" class="form-control input-field" name="reg_firstname" placeholder="<?php esc_attr_e('First Name', 'jobportal') ?>">
								</div>
							</div>
							<div class="col-6">
								<div class="col-group">
									<label for="ip_reg_lastname" class="label-field"><?php esc_html_e('Last Name', 'jobportal'); ?></label>
					...
					<div class="form-group">
						<label for="ip_reg_company_name" class="label-field"><?php esc_html_e('Username', 'jobportal'); ?></label>
					...
					<div class="form-group">
						<label for="ip_reg_email" class="label-field"><?php esc_html_e('Email', 'jobportal'); ?></label>
						<input type="email" id="ip_reg_email" class="form-control input-field" name="reg_email" placeholder="<?php esc_attr_e('Enter Email', 'jobportal') ?>">
					</div>
					<div class="form-group">
						<label for="ip_reg_phone" class="label-field"><?php esc_html_e('Phone number', 'jobportal') ?></label>
						<div class="tel-group">
					...
					<div class="form-group">
						<label for="ip_reg_password" class="label-field"><?php esc_html_e('Password', 'jobportal'); ?></label>
						...
						<span toggle="#ip_reg_password" class="fa fa-fw fa-eye field-icon jobportal-toggle-password"></span>
					</div>
					<?php
					$enable_captcha = JobPortal_Helper::jobportal_get_option('enable_captcha');
					$recaptcha_site_key = JobPortal_Helper::jobportal_get_option('recaptcha_site_key');
					...
					<div class="form-group accept-account">
						<?php
						$terms_condition 	= JobPortal_Helper::jobportal_get_option('terms_condition');
						$privacy_policy = JobPortal_Helper::jobportal_get_option('privacy_policy');
						?>
						<label for="ip_accept_account">
							<input type="checkbox" id="ip_accept_account" class="form-control custom-checkbox" name="accept_account">
							<?php printf(esc_html__('Accept the %1$s and %2$s', 'jobportal'), '<a href="' . get_permalink($terms_condition) . '">' . esc_html__('Terms', 'jobportal') . '</a>', '<a href="' . get_permalink($privacy_policy) . '">' . esc_html__('Privacy Policy', 'jobportal') . '</a>'); ?>
						</label>
					</div>
