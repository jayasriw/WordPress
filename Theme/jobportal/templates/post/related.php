...
<?php if ($related) : ?>
	<div class="related-post block-line">
		...
		<div class="block-heading">
			<h3 class="entry-title"><?php esc_html_e('Related Articles', 'jobportal'); ?></h3>
		</div>
		...
		<div class="list-posts slick-carousel" <?php echo implode(' ', $wrapper_attributes); ?>>
			...
			foreach ($related as $related_post) {
				...
				$thumb_url = JobPortal_Helper::jobportal_image_resize($attach_id, $size);

				$no_image_src    = JOBPORTAL_IMAGES . 'no-image.jpg';
				if (!class_exists("JobPortal_Framework")) {
					$default_image = '';
				} else {
					$default_image   =JobPortal_Helper::jobportal_get_option('default_jobs_image', '');
				}
					...
					<div class="inner-post-wrap">
						...
						<div class="entry-post-detail">
							...
							<?php if (is_sticky($postid)) { ?>
								<span class="is-sticky"><?php esc_html_e('Featured', 'jobportal'); ?></span>
							<?php } ?>
