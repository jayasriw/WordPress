...
if (!class_exists('JobPortal_Ajax_Include')) {

	/**
	 *  Class JobPortal_Ajax
	 */
	class JobPortal_Ajax_Include
	{
		...
		public function __construct()
		{
			...
			add_action('wp_ajax_jobportal_verify_resend', array($this, 'jobportal_verify_resend'));
			add_action('wp_ajax_nopriv_jobportal_verify_resend', array($this, 'jobportal_verify_resend'));

			// Reset password
			add_action('wp_ajax_jobportal_verify_email', array($this, 'jobportal_verify_email'));
			add_action('wp_ajax_nopriv_jobportal_verify_email', array($this, 'jobportal_verify_email'));

			add_action('wp_ajax_jobportal_reset_password_ajax', array($this, 'reset_password_ajax'));
			add_action('wp_ajax_nopriv_jobportal_reset_password_ajax', array($this, 'reset_password_ajax'));
		...
		function get_login_user()
		{
			...
			$user_login                  = $email;
			$url_redirect                = '';
			$enable_redirect_after_login = JobPortal_Helper::jobportal_get_option('enable_redirect_after_login');
			$enable_status               = JobPortal_Helper::jobportal_get_option('enable_status_user');
			$enable_captcha              = JobPortal_Helper::jobportal_get_option('enable_captcha');
			$redirect_for_admin          = JobPortal_Helper::jobportal_get_option('redirect_for_admin');
			$redirect_for_employer       = JobPortal_Helper::jobportal_get_option('redirect_for_employer');
			$redirect_for_candidate      = JobPortal_Helper::jobportal_get_option('redirect_for_candidate');

		// Verify reCAPTCHA only if enabled
		if ($enable_captcha === '1') {
			$recaptcha_secret_key = JobPortal_Helper::jobportal_get_option('recaptcha_secret_key');
		...
		if (is_email($email)) {
			...
			if (!is_wp_error($user)) {
				...
				if (in_array('jobportal_user_candidate', (array) $users->roles) && $enable_redirect_after_login && $redirect_for_candidate != '') {
					if ($redirect_for_candidate == 'reload') {
						$url_redirect = $reload;
					} else {
						$url_redirect = get_page_link($redirect_for_candidate);
					}
				} else if (in_array('jobportal_user_employer', (array) $users->roles) && $enable_redirect_after_login && $redirect_for_employer != '') {
		...
		function get_register_user()
		{
			...
			$redirect_for_employer  = JobPortal_Helper::jobportal_get_option('redirect_for_employer');
			$redirect_for_candidate = JobPortal_Helper::jobportal_get_option('redirect_for_candidate');
		$type_name_candidate    = JobPortal_Helper::jobportal_get_option('type_name_candidate');

		$enabled_user_role = JobPortal_Helper::jobportal_get_option('enable_user_role');
		$default_user_role = JobPortal_Helper::jobportal_get_option('enable_default_user_role');

		if ($enabled_user_role === '0') {
			$account_type = ($default_user_role === 'candidate') ? 'jobportal_user_candidate' : 'jobportal_user_employer';
		}
		...
		$userdata = array(
			...
			if ($user) {
				...
				if ($enable_verify_user) {
					...
					JobPortal_Helper::jobportal_send_email($email, 'mail_verify_user', $args);
					echo json_encode(array(
						'success'      => true,
						'verify'       => true,
						'user_login'   => $user_login,
					...
					global $jobportal_registering_account_type, $jobportal_registering_phone, $jobportal_registering_phone_code;
					$jobportal_registering_account_type = $account_type;
					$jobportal_registering_phone = $phone;
					$jobportal_registering_phone_code = $phone_code;
					...
					$jobportal_registering_account_type = null;
					$jobportal_registering_phone = null;
					$jobportal_registering_phone_code = null;

					if (!is_wp_error($user_id) && $user_id > 0) {
						update_user_meta($user_id, JOBPORTAL_METABOX_PREFIX . 'author_mobile_number', $phone);
					...
					if (!is_wp_error($user_id)) {
						if ($account_type == 'jobportal_user_employer') {
							$u = new WP_User($user_id);
							if (get_role('jobportal_user_candidate')) {
								$u->remove_role('jobportal_user_candidate');
							}
							$u->add_role('jobportal_user_employer');
						}
					...
					if ($enable_status === '1') {
						...
						$msg_text = esc_html__('Registration successful. Awaiting admin approval.', 'jobportal');

							$send_emails = apply_filters('before_jobportal_send_email_register_user', true, $email, $args);
							if ($send_emails) {
								JobPortal_Helper::jobportal_send_email($email, 'mail_register_user', $args);
							}

							$send_admin_emails = apply_filters('before_jobportal_send_email_register_admin', true, $admin_email, $args);
							if ($send_admin_emails) {
								JobPortal_Helper::jobportal_send_email($admin_email, 'admin_mail_register_user', $args);
							}
							...
							$msg                    = esc_html__('Register success', 'jobportal');
		...
		function verify_code()
		{
			...
			if ($enable_verify_user) {
				if (intval($verify_code) !== intval(get_option($email))) {
					$msg = esc_html__('The code mail is incorrect or has expired.', 'jobportal');
					echo json_encode(array(
						'success'      => false,
						'messages'     => $msg,
						'class'        => 'text-error',
			...
			if ($enable_status === '1') {
				...
				echo json_encode(array(
					'success'      => true,
					'messages'     => esc_html__('Registration successful. Awaiting admin approval.', 'jobportal'),
					'class'        => 'text-success',
					'url_redirect' => ''
				));
			} else {
				...
				if (in_array('jobportal_user_candidate', (array) $users->roles) && $enable_redirect_after_login && $redirect_for_candidate) {
					$url_redirect = get_page_link($redirect_for_candidate);
				} elseif (in_array('jobportal_user_employer', (array) $users->roles) && $enable_redirect_after_login && $redirect_for_employer) {
					$url_redirect = get_page_link($redirect_for_employer);
				...
				echo json_encode(array(
					'success'      => true,
					'messages'     => esc_html__('Verify success', 'jobportal'),
					'class'        => 'text-success',
					'url_redirect' => $url_redirect
				));
			}
		...
		//////////////////////////////////////////////////////////////////
		function jobportal_verify_resend()
		{
			// Security check with nonce
		...
		// Ajax reset password
		//////////////////////////////////////////////////////////////////
		function jobportal_verify_email()
		{
			$email = sanitize_email($_POST['email']);
			if (empty($email)) {
				echo json_encode(array(
					'success' => false,
					'class'   => 'text-error',
					'message' => esc_html__('Please enter your email address.', 'jobportal')
				));
				wp_die();
			}
			if (!is_email($email)) {
				echo json_encode(array(
					'success' => false,
					'class'   => 'text-error',
					'message' => esc_html__('Invalid email format.', 'jobportal')
				));
				wp_die();
			}
			if (!email_exists($email)) {
				echo json_encode(array(
					'success' => false,
					'class'   => 'text-error',
					'message' => esc_html__('This email address is not registered.', 'jobportal')
				));
				wp_die();
			}
			$code = rand(100000, 999999);
			...
			echo json_encode(array(
				'success' => true,
				'message' => esc_html__('Please check your email to get the verification code.', 'jobportal'),
				'class'   => 'text-success'
			));
			wp_die();
		}
		...
		function reset_password_ajax()
		{
			if ($_POST['type'] == 'elementor') {
				check_ajax_referer('jobportal_reset_password_ajax_nonce', 'el_jobportal_security_reset_password');
			} else {
				check_ajax_referer('jobportal_reset_password_ajax_nonce', 'jobportal_security_reset_password');
			}
			...
			if (empty($user_login)) {
				echo json_encode(array(
					'success' => false,
					'class'   => 'text-warning',
					'message' => esc_html__('Enter your email address.', 'jobportal')
				));
				wp_die();
			}
			...
			if ($code != $verify_code) {
				echo json_encode(array(
					'success' => false,
					'class'   => 'text-error',
					'message' => esc_html__('The code is incorrect or has expired.', 'jobportal')
				));
				wp_die();
			}
			...
			if (strpos($user_login, '@')) {
				...
				if (empty($user_data)) {
					echo json_encode(array(
						'success' => false,
						'class'   => 'text-error',
						'message' => esc_html__('There is no user registered with that email address.', 'jobportal')
					));
					wp_die();
				}
			} else {
				...
				if (!$user_data) {
					echo json_encode(array(
						'success' => false,
						'class'   => 'text-error',
						'message' => esc_html__('Invalid username', 'jobportal')
					));
					wp_die();
				}
			}
			...
			$message = esc_html__('Someone has requested a password reset for the following account:', 'jobportal') . "\r\n\r\n";
			$message .= network_home_url('/') . "\r\n\r\n";
			$message .= sprintf(esc_html__('Username: %s', 'jobportal'), $user_login) . "\r\n\r\n";
			$message .= esc_html__('If this was a mistake, just ignore this email and nothing will happen.', 'jobportal') . "\r\n\r\n";
			$message .= esc_html__('To reset your password, visit the following address:', 'jobportal') . "\r\n\r\n";
			...
			$title   = sprintf(esc_html__('[%s] Password Reset', 'jobportal'), $blogname);
			...
			if ($message && !wp_mail($user_email, wp_specialchars_decode($title), $message)) {
				echo json_encode(array(
					'success' => false,
					'class'   => 'text-error',
					'message' => esc_html__('The email could not be sent.', 'jobportal') . "\r\n" . esc_html__('Possible reason: your host may have disabled the mail() function.', 'jobportal')
				));
				wp_die();
			} else {
				echo json_encode(array(
					'success' => true,
					'class'   => 'text-success',
					'message' => esc_html__('Please check your email for the link to reset your password.', 'jobportal')
				));
				wp_die();
			}
		}
		...
		function change_password_ajax()
		{
			...
			if (empty($new_password) || empty($confirm_password)) {
				echo json_encode(array(
					'success' => false,
					'class'   => 'text-error',
					'message' => esc_html__('Please provide both current and new password.', 'jobportal')
				));
			...
			if (is_wp_error($check)) {
				echo json_encode(array(
					'success' => false,
					'class'   => 'text-error',
					'message' => esc_html__('The password reset key is invalid or has expired.', 'jobportal')
				));
				wp_die(); // Terminate execution after sending the response
			}
			...
			if ($new_password != $confirm_password) {
				echo json_encode(array(
					'success' => false,
					'class'   => 'text-error',
					'message' => esc_html__('Confirm password does not match.', 'jobportal')
				));
				wp_die(); // Terminate execution after sending the response
			}
			...
			echo json_encode(array(
				'success' => true,
				'class'   => 'text-success',
				'message' => esc_html__('Your password has been successfully changed. Please re-login!', 'jobportal')
			));
			wp_die(); // Terminate execution after sending the response
		}
	...
	private function verify_recaptcha($secret_key, $response, $version = 'v2')
	{
		...
		if ($version === 'v3') {
			$score_threshold = floatval(JobPortal_Helper::jobportal_get_option('recaptcha_v3_score_threshold', '0.5'));
