...
(function ($) {
  ...
  CIVI.element = {
    init: function () {
      ...
      CIVI.element.WidgetJobPortalCarouselHandler();
    ...
    general: function () {
      ...
      $("body").on("click", ".jobportal-categories li", function (event) {
        event.returnValue = true;
      });

      $("body").on("click", ".jobportal-categories li > a", function (event) {
        event.returnValue = true;
      });
    },
    ...
    page_loading_effect: function () {
      ...
      $(".jobportal-jobs-item").removeClass("skeleton-loading");
    ...
    handler_entrance_queue_animation: function () {
      var animateQueueDelay = 200,
        queueResetDelay;
      $(".jobportal-entrance-animation-queue").each(function () {
        var itemQueue = [],
          queueTimer,
    ...
    swiper_carousel: function () {
      $(".jobportal-slider").each(function () {
        if ($(this).hasClass("jobportal-swiper-linked-yes")) {
          var mainSlider = $(this).children(".jobportal-main-swiper").JobPortalSwiper();
          var thumbsSlider = $(this)
            .children(".jobportal-thumbs-swiper")
            .JobPortalSwiper();

          mainSlider.controller.control = thumbsSlider;
          thumbsSlider.controller.control = mainSlider;
        } else {
          $(this).JobPortalSwiper();
        }
      });
    },

    WidgetJobPortalCarouselHandler: function () {
      $(".jobportal-carousel-activation").each(function () {
        ...
        if (carousel_elem.length > 0) {
          ...
          if (carousel_style_ck == 4) {
            carousel_elem.slick({
              arrows: arrows,
              prevArrow:
                '<button class="jobportal-carosul-prev">' +
                arrow_prev_txt +
                "</button>",
              nextArrow:
                '<button class="jobportal-carosul-next">' +
                arrow_next_txt +
                "</button>",
              dots: dots,
    ...
    popup: function () {
      $(".jobportal-on-popup").on("click", function (event) {
      ...
      $(".jobportal-popup").on("click", ".btn-close,.bg-overlay", function () {
    ...
    nav_tabs: function () {
      ...
      $(".btn-reset-password").on("click", function (e) {
        ...
        $(".jobportal-reset-password-wrap").addClass("active");
      });

      $(".back-to-login").on("click", function (e) {
        e.preventDefault();
        $(".jobportal-reset-password-wrap").removeClass("active");
    ...
    validate_form: function () {
      ...
      $(".ux-login").each(function (index) {
        ...
        _this.validate({
          ...
          submitHandler: function (form) {
            var jobportal_recaptcha = _this.find('input[name="jobportal_recaptcha"]').val();

            if (jobportal_recaptcha !== '0') {
            ...
            function submitLoginForm(recaptchaToken) {
              const data = {
                ...
                captcha: _this.find(".jobportal-captcha").val(),
                num_captcha: _this.find(".jobportal-num-captcha").data("captcha"),
      ...
      $(".ux-verify .resend").on("click", function (e) {
        ...
        const data = {
          ...
          action: "jobportal_verify_resend"
        };
      ...
      $(".verify-email").on("click", function (e) {
        ...
        $.ajax({
          ...
          data: {
            email: email,
            action: "jobportal_verify_email"
          },
          beforeSend: function () {
            form.closest(".jobportal-reset-password-wrap").find("p.msg").removeClass("text-error text-success").hide();
            form.find(".verify-email").addClass("loading");
            form.find(".verify-field").hide();
          },
          success: function (data) {
            form.closest(".jobportal-reset-password-wrap").find("p.msg").text(data.message).addClass(data.class).show();
          ...
          error: function (xhr, status, error) {
            form.closest(".jobportal-reset-password-wrap").find("p.msg").text(theme_vars.ajax_error || "An error occurred. Please try again.").addClass("text-error").show();
            form.find(".verify-email").removeClass("loading");
          }
        });
      });
    ...
    forget_password: function ($this) {
      $(".forgot-password").on("click", function () {
        $(".jobportal-resset-password-wrap").slideToggle();
      });

      $(".jobportal_forgetpass").on("click", function (e) {
        e.preventDefault();
        ...
        $.ajax({
          ...
          beforeSend: function () {
            ...
            $(".jobportal-reset-password-wrap p.msg").show();
            $(".popup-account .loading-effect").fadeIn();
          },
          success: function (data) {
            $(".jobportal-reset-password-wrap p.msg").text(data.message);
            $(".jobportal-reset-password-wrap p.msg").addClass(data.class);
            $(".popup-account .loading-effect").fadeOut();
          },
        });
      });
      ...
      $(".jobportal-new-password-wrap form").validate({
        rules: {
          new_password: {
            required: true,
            minlength: 8,
        ...
        submitHandler: function (form) {
          ...
          $.ajax({
            ...
            beforeSend: function () {
              $(".jobportal-new-password-wrap p.msg").removeClass(
                "text-error text-success text-warning"
              );
              $(".jobportal-new-password-wrap p.msg").text(
                theme_vars.change_password
              );
              $(".jobportal-new-password-wrap p.msg").show();
              $(".jobportal-new-password-wrap .loading-effect").fadeIn();
            },
            success: function (data) {
              var data = $.parseJSON(data);
              $(".jobportal-new-password-wrap p.msg").text(data.message);
              $(".jobportal-new-password-wrap p.msg").addClass(data.class);
              $(".popup-account .loading-effect").fadeOut();
              if (data.success) {
    ...
    cookie_notices: function () {
      if (
        ...
        $("#jobportal-button-cookie-notice-not-ok").on("click", function () {
          $(this).closest("#growls-br").remove();
          sessionStorage.setItem("hide-cookie-form", "true");
        });

        $("#jobportal-button-cookie-notice-ok").on("click", function () {
          $(this).closest("#growls-br").remove();
    ...
    user_form_settings: function () {
      if ($(".elementor-widget-jobportal-user-form").length > 0) {
    ...
    check_company_tooltip: function () {
      document.querySelectorAll('.jobportal-check-company').forEach((el) => {
