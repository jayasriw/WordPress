...
class Widget_Image_Gallery extends Base
{
	...
	{
		return "jobportal-image-gallery';
	}

	public function get_title()
	{
		return esc_html__('Image Gallery', 'jobportal');
	}
	...
	{
		return ["jobportal-el-widget-image-gallery'];
	}
	...
	private function add_layout_section()
	{
		...
		$this->add_control('layout', [
			...
			'options' => [
				...
				'metro'   => esc_html__('Metro', 'jobportal'),
			],
			'default' => 'grid',
		]);

		$this->add_responsive_control('zigzag_height', [
			'label'     => esc_html__('Zigzag Height', 'jobportal'),
		...
		$this->add_control('zigzag_reversed', [
			'label'     => esc_html__('Zigzag Reversed?', 'jobportal'),
		...
		$this->add_control('lightbox_enable', [
			'label'        => esc_html__('Lightbox', 'jobportal'),
			'type'         => Controls_Manager::SWITCHER,
			'return_value' => '1',
		]);
		...
		$this->add_control('metro_image_ratio', [
			'label'     => esc_html__('Image Ratio', 'jobportal'),
		...
		$this->add_control('thumbnail_default_size', [
			'label'        => esc_html__('Use Default Thumbnail Size', 'jobportal'),
			'type'         => Controls_Manager::SWITCHER,
			'default'      => '1',
			'return_value' => '1',
			'separator'    => 'before',
	...
	private function add_grid_section()
	{
		...
		$metro_layout_repeater->add_control('size', [
			'label'   => esc_html__('Item Size', 'jobportal'),
			'type'    => Controls_Manager::SELECT,
			'default' => '1:1',
		...
		$this->add_control('grid_metro_layout', [
			'label'       => esc_html__('Metro Layout', 'jobportal'),
	...
	private function add_image_style_section()
	{
		$this->start_controls_section('images_style_section', [
			'label' => esc_html__('Images', 'jobportal'),
	...
	protected function render()
	{
		...
		$this->add_render_attribute('grid-wrapper', 'class', "jobportal-grid-wrapper');

		$this->add_render_attribute('content-wrapper', 'class', "jobportal-grid lazy-grid');

		if ('metro' === $settings['layout']) {
			$this->add_render_attribute('grid-wrapper', 'class', "jobportal-grid-metro');
		}
		...
		if (!empty($settings['lightbox_enable'])) {
			$this->add_render_attribute('grid-wrapper', 'class', "jobportal-light-gallery');
		}
?>
	...
	{
		$image_size = \JobPortal_Image::elementor_parse_image_size($settings, '600x600');
	...
	private function print_image($image, $image_size)
	{
		...
		$box_tag  = 'div';
		$this->add_render_attribute($box_key, 'class', "jobportal-box');

		if (!empty($settings['lightbox_enable'])) {
			$box_tag = 'a';

			$image_full = \JobPortal_Image::get_attachment_info($image['id']);
		...
		<div class="jobportal-image image">
			<?php \JobPortal_Image::the_attachment_by_id(array(
				'id'   => $image['id'],
				'size' => $image_size,
			)); ?>
		</div>
		<?php if (!empty($settings['lightbox_enable'])) { ?>
			<div class="jobportal-overlay">
