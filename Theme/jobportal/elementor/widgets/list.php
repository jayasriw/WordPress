...
class Widget_List extends Base
{
	...
	{
		return esc_html__('Modern Icon List', 'jobportal');
	}
	...
	{
		return ["jobportal-widget-list'];
	}

	public function get_style_depends()
	{
		return ["jobportal-el-widget-list'];
	}
	...
	private function add_list_section()
	{
		$this->start_controls_section('list_section', [
			'label' => esc_html__('Icon List', 'jobportal'),
		]);
		...
		$this->add_control('style', [
			...
			'default'      => '',
			'options'      => [
				''            => esc_html__('Normal', 'jobportal'),
				'icon-border' => esc_html__('Icon Border', 'jobportal'),
			],
			'prefix_class' => "jobportal-list-style-',
		]);
		...
		$this->add_control('layout', [
			...
			'options'      => [
				...
				'columns' => [
					'title' => esc_html__('Columns', 'jobportal'),
					'icon'  => 'eicon-columns',
				],
			],
			'prefix_class' => "jobportal-list-layout-',
		]);

		$this->add_control('icon', [
			'label'       => esc_html__('Default Icon', 'jobportal'),
			'description' => esc_html__('Choose default icon for all items.', 'jobportal'),
			'type'        => Controls_Manager::ICONS,
		]);

		$this->add_control('icon_vertical_alignment', [
			'label'                => esc_html__('Icon Alignment', 'jobportal'),
			'type'                 => Controls_Manager::CHOOSE,
		...
		$this->add_control(
			'enable_toggle',
			[
				'label' => esc_html__('Enable Toggle (Mobie)', 'jobportal'),
		...
		$this->add_control('heading', [
			...
			'default'     => esc_html__('Heading', 'jobportal'),
			'label_block' => true,
			'condition' => [
				'enable_toggle!' => '',
			],
		...
		$repeater->add_control(
			'enable_badge',
			[
				'label' => esc_html__('Enable Badge', 'jobportal'),
				'type' => Controls_Manager::SWITCHER,
				'default' => '',
			]
		);
		...
		$repeater->add_control('badge_text', [
			'label'       => esc_html__('Text', 'jobportal'),
			'type'        => Controls_Manager::TEXT,
			'default'     => esc_html__('Hot', 'jobportal'),
			'condition' => [
				'enable_badge' => 'yes',
			],
		]);
	...
	private function add_styling_section()
	{
		$this->start_controls_section('styling_section', [
			'label' => esc_html__('Styling', 'jobportal'),
			'tab'   => Controls_Manager::TAB_STYLE,
		]);

		$this->add_control('items_vertical_spacing', [
			'label'      => esc_html__('Items Spacing', 'jobportal'),
			...
			'selectors'  => [
				'{{WRAPPER}}.jobportal-list-layout-block .item + .item, {{WRAPPER}}.jobportal-list-layout-columns .item:nth-child(2) ~ .item' => 'margin-top: {{SIZE}}{{UNIT}};',
				'{{WRAPPER}}.jobportal-list-layout-inline .item'                                                                             => 'margin-bottom: {{SIZE}}{{UNIT}};',
			],
		]);
		...
		$this->add_control('width', [
			...
			'selectors'  => [
				'{{WRAPPER}} .jobportal-list' => 'width: {{SIZE}}{{UNIT}};',
			],
		]);
	...
	private function add_icon_style_section()
	{
		...
		$this->add_responsive_control('min_width', [
			'label'          => esc_html__('Min Width', 'jobportal'),
			'type'           => Controls_Manager::SLIDER,
			'default'        => [
				'unit' => 'px',
			],
		...
		$this->add_control('icon_marker_color', [
			'label'     => esc_html__('Icon Marker', 'jobportal'),
			'type'      => Controls_Manager::COLOR,
			'selectors' => [
				'{{WRAPPER}}.jobportal-list-style-icon-border .icon' => 'border-color: {{VALUE}};',
			],
			'condition' => [
				'style' => [
					'icon-border',
		...
		$this->add_control('hover_icon_marker_color', [
			...
			'type'      => Controls_Manager::COLOR,
			'selectors' => [
				'{{WRAPPER}}.jobportal-list-style-icon-border .link:hover .icon' => 'border-color: {{VALUE}};',
			],
			'condition' => [
				'style' => [
					'icon-border',
	...
	protected function render()
	{
		...
		$this->add_render_attribute('wrapper', 'class', "jobportal-list');
		if ($settings['enable_toggle'] == 'yes') {
			$this->add_render_attribute('wrapper', 'class', 'toggle');
		}
		$global_icon_html = '';
		if (!empty($settings['icon']['value'])) {
			$global_icon_html = '<div class="jobportal-icon icon">' . $this->get_render_icon($settings, $settings['icon'], ['aria-hidden' => 'true'], false, 'icon') . '</div>';
		}
?>
			...
			<?php if ($settings['enable_toggle'] == 'yes') { ?>
				...
				?>
						<div <?php $this->print_attributes_string($item_key); ?>>
							...
							<div class="list-header <?php echo esc_attr__($settings['icon_vertical_alignment']); ?>">
