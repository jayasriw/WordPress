...
class Widget_Circle_Progress_Chart extends Base
{
	...
	{
		return "jobportal-circle-progress-chart';
	}

	public function get_title()
	{
		return esc_html__('Circle Progress Chart', 'jobportal');
	}
	...
	{
		return ["jobportal-widget-circle-progress'];
	}

    public function get_style_depends()
    {
        return ["jobportal-el-widget-circle-progress-chart'];
    }
	...
	private function add_chart_section()
	{
		$this->start_controls_section('chart_section', [
			'label' => esc_html__('Chart', 'jobportal'),
		]);

		$this->add_control('number', [
			'label'   => esc_html__('Number', 'jobportal'),
		...
		$this->add_control('size', [
			'label'       => esc_html__('Circle Size', 'jobportal'),
			'description' => esc_html__('Controls the size of the pie chart circle.', 'jobportal'),
			'type'        => Controls_Manager::NUMBER,
			'default'     => 180,
			'min'         => 100,
			'max'         => 1000,
		...
		$this->add_control('unit', [
			'label'       => esc_html__('Measuring unit', 'jobportal'),
			'description' => esc_html__('Controls the unit of chart.', 'jobportal'),
			'type'        => Controls_Manager::TEXT,
			'default'     => '%',
		]);

		$this->add_control('reverse', [
			'label' => esc_html__('Reverse animation and arc draw', 'jobportal'),
			'type'  => Controls_Manager::SWITCHER,
		]);

		$this->add_control('line_cap', [
			'label'   => esc_html__('Line Cap', 'jobportal'),
			'type'    => Controls_Manager::SELECT,
			'options' => [
				'butt'   => esc_html__('Butt', 'jobportal'),
		...
		$this->add_control('line_width', [
			'label'   => esc_html__('Line Width', 'jobportal'),
			'type'    => Controls_Manager::NUMBER,
			'default' => 3,
			'min'     => 1,
			'max'     => 50,
		...
		$this->add_control('inner_content_type', [
			'label'   => esc_html__('Inner Content', 'jobportal'),
			'type'    => Controls_Manager::SELECT,
			'options' => [
				''     => esc_html__('Animate Number', 'jobportal'),
				'text' => esc_html__('Custom Text', 'jobportal'),
			],
			'default' => '',
		]);
	...
	private function add_chart_style_section()
	{
		...
		$this->add_control('bar_color', [
			'label'  => esc_html__('Bar Color', 'jobportal'),
		...
		$this->add_control('track_color', [
			'label'   => esc_html__('Track Color', 'jobportal'),
			'type'    => Controls_Manager::COLOR,
			'default' => '#ededed',
		]);

		$this->add_control('number_color', [
			'label'     => esc_html__('Number Color', 'jobportal'),
			'type'      => Controls_Manager::COLOR,
			'selectors' => [
	...
	protected function render()
	{
		...
		$this->add_render_attribute('wrapper', 'class', "jobportal-circle-progress-chart');
