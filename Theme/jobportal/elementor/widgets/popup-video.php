...
class Widget_Popup_Video extends Base
{
	...
	{
		return "jobportal-popup-video';
	}

	public function get_title()
	{
		return esc_html__('Popup Video', 'jobportal');
	}
    ...
    {
        return ["jobportal-el-widget-popup-video'];
    }
	...
	private function add_video_section()
	{
		$this->start_controls_section('video_section', [
			'label' => esc_html__('Video', 'jobportal'),
		]);
		...
		$this->add_control('type', [
			...
			'default' => 'poster',
			'options' => [
				'poster' => esc_html__('Poster', 'jobportal'),
				'button' => esc_html__('Button', 'jobportal'),
			],
		]);

		$this->add_control('video_url', [
			'label'       => esc_html__('Video Url', 'jobportal'),
			'description' => esc_html__('Input Youtube video url or Vimeo video url. For e.g: "https://www.youtube.com/watch?v=XHOmBV4js_E"', 'jobportal'),
		...
		$this->add_control('video_text', [
			'label'         => esc_html__('Video Text', 'jobportal'),
			'type'          => Controls_Manager::TEXT,
			'label  _block' => true,
			'condition'     => [
				'type' => 'button',
		...
		$this->add_control('video_text_animate', [
			'label'        => esc_html__('Text Animate', 'jobportal'),
			'type'         => Controls_Manager::SELECT,
			'options'      => [
				''             => esc_html__('None', 'jobportal'),
				'animate-line' => esc_html__('Animate Line', 'jobportal'),
			],
			'default'      => '',
			'prefix_class' => "jobportal-text-',
			'condition'    => [
				'type'        => 'button',
				'video_text!' => '',
			],
		...
		$this->add_control('position', [
			...
			'prefix_class' => "jobportal-popup-video-icon-position-',
			'toggle'       => false,
			'condition'    => [
				'type'        => 'button',
				'video_text!' => '',
		...
		$this->add_control('poster', [
			'label'     => esc_html__('Poster Image', 'jobportal'),
		...
		$this->add_control('button_type', [
			'label'     => esc_html__('Button Type', 'jobportal'),
			'type'      => Controls_Manager::SELECT,
			'default'   => '',
			'options'   => [
		...
		$this->add_control('button_image', [
			'label'     => esc_html__('Button Image', 'jobportal'),
			'type'      => Controls_Manager::MEDIA,
			'default'   => [
	...
	private function add_image_style_section()
	{
		...
		$this->add_responsive_control('image_border_width', [
			...
			'selectors' => [
				'{{WRAPPER}} .jobportal-image' => 'border-width: {{SIZE}}{{UNIT}}',
			],
		]);
		...
		$this->add_responsive_control('image_border_radius', [
			...
			'selectors'  => [
				'{{WRAPPER}} .jobportal-image' => 'border-radius: {{SIZE}}{{UNIT}}',
			],
		]);
		...
		$this->add_control('image_border_color', [
			...
			'selectors' => [
				'{{WRAPPER}} .jobportal-image' => 'border-color: {{VALUE}};',
			],
		]);

		$this->add_group_control(Group_Control_Box_Shadow::get_type(), [
			'name'     => 'image_box_shadow',
			'selector' => '{{WRAPPER}} .jobportal-image',
		]);
		...
		$this->add_control('hover_image_border_color', [
			...
			'type'      => Controls_Manager::COLOR,
			'selectors' => [
				'{{WRAPPER}} .video-link:hover .jobportal-image' => 'border-color: {{VALUE}};',
			],
		]);

		$this->add_group_control(Group_Control_Box_Shadow::get_type(), [
			'name'     => 'hover_image_box_shadow',
			'selector' => '{{WRAPPER}} .video-link:hover .jobportal-image',
		]);
	...
	private function add_button_style_section()
	{
		...
		$this->add_responsive_control('button_size', [
			...
			'selectors' => [
				'{{WRAPPER}} .jobportal-popup-video-icon-play .video-play-icon' => 'width: {{SIZE}}{{UNIT}}; height: {{SIZE}}{{UNIT}}; font-size: {{SIZE}}{{UNIT}};',
				'{{WRAPPER}} .jobportal-popup-video-image-play .video-play img'            => 'width: {{SIZE}}{{UNIT}};',
			],
		]);
		...
		$this->add_control('button_text_color', [
			'label'     => esc_html__('Icon Color', 'jobportal'),
			'type'      => Controls_Manager::COLOR,
			'default'   => '',
			'selectors' => [
		...
		$this->add_control('video_text_line_heading', [
			'label'     => esc_html__('Line', 'jobportal'),
			'type'      => Controls_Manager::HEADING,
			'separator' => 'before',
	...
	protected function render()
	{
		...
		$this->add_render_attribute('wrapper', 'class', "jobportal-popup-video');
		$this->add_render_attribute('wrapper', 'class', 'type-' . $settings['type']);

		if (!empty($settings['button_type']) && 'image' === $settings['button_type']) {
			$this->add_render_attribute('wrapper', 'class', "jobportal-popup-video-image-play');
		} else {
			$this->add_render_attribute('wrapper', 'class', "jobportal-popup-video-icon-play');
		}

		$this->add_render_attribute('link', 'class', 'video-link jobportal-box link-secret');
	...
	?>
		...
		<?php if (!empty($settings['poster_caption'])) : ?>
			<div class="video-poster-caption">
				<?php esc_html_e($settings['poster_caption']); ?>
			</div>
		<?php endif; ?>
	<?php
	}
	...
	?>
		<div class="video-button">
			...
			<?php if (!empty($settings['video_text'])) : ?>
				<div class="video-text"><?php esc_html_e($settings['video_text']); ?></div>
			<?php endif; ?>
		</div>
	<?php
	}
	...
	?>
		<div class="video-play video-play-image">
			<?php echo \JobPortal_Image::get_elementor_attachment([
				...
				'attributes' => [
					'alt' => esc_attr__('Play Icon', 'jobportal'),
				],
			]); ?>
		</div>
<?php
	...
	{
		$icon_url = JOBPORTAL_ELEMENTOR_ASSETS . '/images/video-play-light.png';
