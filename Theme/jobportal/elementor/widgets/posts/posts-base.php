...
abstract class Posts_Base extends Base
{
	...
	{
		$options = [
			'date'           => esc_html__('Date', 'jobportal'),
			'ID'             => esc_html__('Post ID', 'jobportal'),
			'author'         => esc_html__('Author', 'jobportal'),
			'title'          => esc_html__('Title', 'jobportal'),
			'modified'       => esc_html__('Last modified date', 'jobportal'),
			'parent'         => esc_html__('Post/page parent ID', 'jobportal'),
			'comment_count'  => esc_html__('Number of comments', 'jobportal'),
			'menu_order'     => esc_html__('Menu order/Page Order', 'jobportal'),
			'meta_value'     => esc_html__('Meta value', 'jobportal'),
			'meta_value_num' => esc_html__('Meta value number', 'jobportal'),
			'rand'           => esc_html__('Random order', 'jobportal'),
		];
	...
	protected function register_query_section()
	{
		$this->start_controls_section('query_section', [
			'label' => esc_html__('Query', 'jobportal'),
		]);

		$this->add_control('query_source', [
			'label'   => esc_html__('Source', 'jobportal'),
			'type'    => Controls_Manager::SELECT,
			'options' => array(
				'custom_query'  => esc_html__('Custom Query', 'jobportal'),
				'current_query' => esc_html__('Current Query', 'jobportal'),
			),
		...
		$this->start_controls_tab('query_include_tab', [
			'label' => esc_html__('Include', 'jobportal'),
		]);

		$this->add_control('query_include', [
			'label'       => esc_html__('Include By', 'jobportal'),
			'label_block' => true,
			'type'        => Controls_Manager::SELECT2,
			'multiple'    => true,
			'options'     => [
				'terms'   => esc_html__('Term', 'jobportal'),
				'authors' => esc_html__('Author', 'jobportal'),
			],
			'condition'   => [
				'query_source!' => ['current_query'],
			],
		...
		// 	'label'        => esc_html__( 'Author', 'jobportal' ),
		// 	'label_block'  => true,
		// 	'type'         => Module_Query::QUERY_CONTROL_ID,
		// 	'multiple'     => true,
		// 	'default'      => [],
		...
		$this->start_controls_tab('query_exclude_tab', [
			'label' => esc_html__('Exclude', 'jobportal'),
		]);

		$this->add_control('query_exclude', [
			'label'       => esc_html__('Exclude By', 'jobportal'),
			'label_block' => true,
			'type'        => Controls_Manager::SELECT2,
			'multiple'    => true,
			'options'     => [
		...
		$this->add_control('query_number', [
			'label'       => esc_html__('Items per page', 'jobportal'),
			'description' => esc_html__('Number of items to show per page. Input "-1" to show all posts. Leave blank to use global setting.', 'jobportal'),
		...
		$this->add_control('query_orderby', [
			'label'       => esc_html__('Order by', 'jobportal'),
			'description' => esc_html__('Select order type. If "Meta value" or "Meta value Number" is chosen then meta key is required.', 'jobportal'),
			'type'        => Controls_Manager::SELECT,
		...
		$this->add_control('query_sort_meta_key', [
			'label'     => esc_html__('Meta key', 'jobportal'),
		...
		$this->add_control('query_order', [
			'label'     => esc_html__('Sort order', 'jobportal'),
			'type'      => Controls_Manager::SELECT,
			'options'   => array(
				'DESC' => esc_html__('Descending', 'jobportal'),
				'ASC'  => esc_html__('Ascending', 'jobportal'),
			),
			'default'   => 'DESC',
			'condition' => [
				'query_source!' => ['current_query'],
	...
	protected function add_pagination_section()
	{
		$this->start_controls_section('pagination_section', [
			'label' => esc_html__('Pagination', 'jobportal'),
		]);
		...
		$this->add_control('pagination_type', [
			...
			'options' => array(
				...
				'numbers'       => esc_html__('Numbers', 'jobportal'),
				'navigation'    => esc_html__('Navigation', 'jobportal'),
				'load-more'     => esc_html__('Button', 'jobportal'),
				'load-more-alt' => esc_html__('Custom Button', 'jobportal'),
				'infinite'      => esc_html__('Infinite Scroll', 'jobportal'),
			),
			'default' => '',
		]);

		$this->add_control('pagination_custom_button_id', [
			'label'       => esc_html__('Custom Button ID', 'jobportal'),
			'description' => esc_html__('Input id of custom button to load more posts when click. For e.g: #product-load-more-btn', 'jobportal'),
			'type'        => Controls_Manager::TEXT,
			'condition'   => [
	...
	protected function add_pagination_style_section()
	{
		...
		$this->add_responsive_control('pagination_alignment', [
			...
			'default'   => 'center',
			'selectors' => [
				'{{WRAPPER}} .jobportal-grid-pagination' => 'text-align: {{VALUE}};',
			],
		]);
		...
		$this->add_responsive_control('pagination_spacing', [
			...
			'selectors'   => [
				'{{WRAPPER}} .jobportal-grid-pagination' => 'padding-top: {{SIZE}}{{UNIT}};',
			],
		]);
		...
		$this->add_control('pagination_link_color', [
			'label'     => esc_html__('Link Color', 'jobportal'),
		...
		$this->add_control('pagination_loading_heading', [
			'label'     => esc_html__('Loading Icon', 'jobportal'),
		...
		$this->add_control('pagination_loading_color', [
			...
			'type'      => Controls_Manager::COLOR,
			'selectors' => [
				'{{WRAPPER}} .jobportal-infinite-loader .sk-wrap' => 'color: {{VALUE}};',
			],
			'condition' => [
	...
	protected function add_filter_section()
	{
		$this->start_controls_section('filter_section', [
			'label' => esc_html__('Filter', 'jobportal'),
		]);

		$this->add_control('filter_enable', [
			'label' => esc_html__('Show Filter', 'jobportal'),
		...
		$this->add_control('filter_counter', [
			'label'        => esc_html__('Show Counter', 'jobportal'),
			'type'         => Controls_Manager::SWITCHER,
			'return_value' => '1',
			'condition'    => [
				'filter_style!' => '',
		...
		$this->add_control('filter_in_grid', [
			'label'        => esc_html__('In Grid', 'jobportal'),
			'type'         => Controls_Manager::SWITCHER,
			'return_value' => '1',
			'condition'    => [
				'filter_style!' => '',
	...
	protected function add_filter_style_section()
	{
		...
		$this->add_responsive_control('filter_spacing', [
			...
			'selectors'  => [
				'{{WRAPPER}} .jobportal-grid-filter' => 'padding-bottom: {{SIZE}}{{UNIT}}',
			],
		]);
		...
		$this->add_responsive_control('filter_alignment', [
			...
			'default'   => 'center',
			'selectors' => [
				'{{WRAPPER}} .jobportal-grid-filter' => 'text-align: {{VALUE}};',
			],
		]);

		$this->add_group_control(Group_Control_Typography::get_type(), [
			'name'     => 'filter_link_typography',
			'label'    => esc_html__('Link Typography', 'jobportal'),
		...
		$this->add_control('filter_counter_style_heading', [
			'label'     => esc_html__('Filter Counter', 'jobportal'),
			'type'      => Controls_Manager::HEADING,
			'separator' => 'before',
			'condition' => [
				'filter_counter' => '1',
	...
	protected function add_sorting_section()
	{
		$this->start_controls_section('result_count_sorting_section', [
			'label' => esc_html__('Result Count & Sorting', 'jobportal'),
		]);

		$this->add_control('show_result_count', [
			'label'        => esc_html__('Show Result Count', 'jobportal'),
			'type'         => Controls_Manager::SWITCHER,
			'return_value' => '1',
		]);

		$this->add_control('show_ordering', [
			'label'        => esc_html__('Show Order', 'jobportal'),
			'type'         => Controls_Manager::SWITCHER,
			'return_value' => '1',
		]);
	...
	protected function get_sort_options()
	{
		return [
			...
			'popularity' => esc_html__('Popularity', 'jobportal'),
			'date'       => esc_html__('Latest', 'jobportal'),
			'price'      => esc_html__('Price: low to high', 'jobportal'),
			'price-desc' => esc_html__('Price: high to low', 'jobportal'),
		];
	}
...
?>
			<div class="jobportal-grid-pagination">
				<div class="pagination-wrapper">
					...
					<?php if (in_array($pagination_type, array(
						...
						<div class="inner">
							<div class="jobportal-infinite-loader">
						...
						<div class="inner">
							<?php if ($pagination_type === 'load-more') { ?>
								<a href="#" class="jobportal-load-more-button jobportal-button style-border icon-right">
									<span class="button-text"><?php echo esc_html__('Load More', 'jobportal'); ?></span>
					...
					<?php } elseif ($pagination_type === 'numbers') { ?>
						<?php \JobPortal_Templates::pagination($query); ?>
					<?php } ?>

				</div>
			</div>
			<div class="jobportal-grid-messages" style="display: none;">
				<?php esc_html_e('All items displayed.', 'jobportal'); ?>
			</div>
		<?php
		}
	}
	...
	protected function print_pagination_type_navigation()
	{
		...
		<div class="navigation-buttons">
			<div class="nav-link prev-link disabled" data-action="prev">
				<?php esc_html_e('Prev Projects', 'jobportal'); ?>
			</div>
			<div class="nav-line"></div>
			<div class="nav-link next-link" data-action="next">
				<?php esc_html_e('Next Projects', 'jobportal'); ?>
			</div>
		</div>
	<?php
	}
	...
	protected function print_filter($total = 0, $list = '')
	{
		...
		$this->add_render_attribute('filter', 'class', "jobportal-grid-filter');
	...
	?>
		<div <?php $this->print_render_attribute_string('filter') ?>>
			<?php ob_start(); ?>
			<div class="jobportal-grid-filter-buttons">
				<a href="<?php echo esc_url(get_post_type_archive_link($post_type)); ?>" class="<?php echo esc_attr($btn_filter_all_class); ?>" data-filter="*" data-filter-count="<?php echo esc_attr($total); ?>">
					<span class="filter-text"><?php esc_html_e('All', 'jobportal'); ?></span>
				</a>
				<?php
				if ($list === '') {
