...
class Widget_Blog extends Posts_Base
{
	...
	{
		return "jobportal-blog';
	}

	public function get_title()
	{
		return esc_html__('Blog', 'jobportal');
	}
	...
	{
		return [
			"jobportal-grid-query',
			"jobportal-widget-grid-post',
		];
	}

    public function get_style_depends()
    {
        return ["jobportal-el-widget-blog'];
    }
	...
	private function add_layout_section()
	{
		...
		$this->add_control('layout', [
			...
			'options' => [
				'grid'    => esc_html__('Grid', 'jobportal'),
				'masonry' => esc_html__('Masonry', 'jobportal'),
				'list-02'    => esc_html__('List', 'jobportal'),
			],
			'default' => 'grid',
		]);
		...
		$this->add_control('big_first', [
			'label'        => esc_html__('Big First', 'jobportal'),
	...
	private function add_banners_section()
	{
		$this->start_controls_section('banners_section', [
			'label'     => esc_html__('Banners', 'jobportal'),
			'condition' => [
				'layout' => ['masonry'],
			],
		]);
		...
		$banner_repeater->add_control('position', [
			'label' => esc_html__('After Post Number', 'jobportal'),
	...
	private function add_caption_popover()
	{
		$this->add_control('show_caption', [
			'label'        => esc_html__('Caption', 'jobportal'),
			'type'         => Controls_Manager::POPOVER_TOGGLE,
			'label_off'    => esc_html__('Default', 'jobportal'),
			'label_on'     => esc_html__('Custom', 'jobportal'),
			'return_value' => 'yes',
			'condition'    => [
				'layout!' => ['zigzag'],
			],
		...
		$this->add_control('show_caption_category', [
			'label'     => esc_html__('Category', 'jobportal'),
			'type'      => Controls_Manager::SWITCHER,
		...
		$this->add_control('show_caption_excerpt', [
			'label'     => esc_html__('Excerpt', 'jobportal'),
			'type'      => Controls_Manager::SWITCHER,
			'label_on'  => esc_html__('Show', 'jobportal'),
			'label_off' => esc_html__('Hide', 'jobportal'),
			'default'   => 'yes',
		...
		$this->add_control('excerpt_length', [
			'label'     => esc_html__('Excerpt Length', 'jobportal'),
			'type'      => Controls_Manager::NUMBER,
			'min'       => 5,
			'condition' => [
		...
		$this->add_control('show_caption_read_more', [
			'label'     => esc_html__('Read More', 'jobportal'),
			'type'      => Controls_Manager::SWITCHER,
			'label_on'  => esc_html__('Show', 'jobportal'),
			'label_off' => esc_html__('Hide', 'jobportal'),
			'default'   => 'yes',
		...
		$this->add_control('read_more_text', [
			'label'     => esc_html__('Read More Text', 'jobportal'),
			'type'      => Controls_Manager::TEXT,
			'default'   => esc_html__('Read More', 'jobportal'),
			'condition' => [
		...
		$this->add_control('show_caption_share', [
			'label'     => esc_html__('Share', 'jobportal'),
			'type'      => Controls_Manager::SWITCHER,
			'label_on'  => esc_html__('Show', 'jobportal'),
			'label_off' => esc_html__('Hide', 'jobportal'),
		]);
		...
		$this->add_control('show_caption_meta', [
			'label'       => esc_html__('Meta', 'jobportal'),
			'label_block' => true,
			...
			'options'     => [
				...
				'views'    => esc_html__('View Count', 'jobportal'),
				'comments' => esc_html__('Comment Count', 'jobportal'),
			],
		]);
	...
	private function add_overlay_popover()
	{
		$this->add_control('show_overlay', [
			'label'        => esc_html__('Overlay', 'jobportal'),
			'type'         => Controls_Manager::POPOVER_TOGGLE,
			'label_off'    => esc_html__('Default', 'jobportal'),
			'label_on'     => esc_html__('Custom', 'jobportal'),
			'return_value' => 'yes',
		...
		$this->add_control('overlay_style', [
			...
			'type'    => Controls_Manager::SELECT,
			'options' => [
				'float'    => esc_html__('Float', 'jobportal'),
				'float-02' => esc_html__('Float-02', 'jobportal'),
			],
			'default' => 'float',
		]);
		...
		$overlay_meta_repeater->add_control('meta', [
			'label'       => esc_html__('Select Meta', 'jobportal'),
			'label_block' => true,
			'type'        => Controls_Manager::SELECT,
			'default'     => 'date',
			'options'     => [
				'author'   => esc_html__('Author', 'jobportal'),
				'date'     => esc_html__('Date', 'jobportal'),
				'comments' => esc_html__('Comments', 'jobportal'),
			],
		]);

		$this->add_control('overlay_meta_data', [
			'label'         => esc_html__('Meta Data', 'jobportal'),
			'type'          => Controls_Manager::REPEATER,
			'fields'        => $overlay_meta_repeater->get_controls(),
			'title_field'   => '{{{ meta }}}',
			'classes'       => "jobportal-control-repeater-title-capitalize',
			'prevent_empty' => false,
		]);
	...
	private function add_grid_section()
	{
		$this->start_controls_section('grid_options_section', [
			'label'     => esc_html__('Grid Options', 'jobportal'),
			'condition' => [
				'layout' => [
					'grid',
					'masonry',
		...
		$this->add_responsive_control('grid_gutter', [
			'label'   => esc_html__('Gutter', 'jobportal'),
			'type'    => Controls_Manager::NUMBER,
			'min'     => 0,
			'max'     => 200,
			'step'    => 1,
	...
	private function add_thumbnail_style_section()
	{
		$this->start_controls_section('thumbnail_style_section', [
			'label' => esc_html__('Thumbnail', 'jobportal'),
		...
		$this->add_group_control(Group_Control_Box_Shadow::get_type(), [
			...
			'selector' => '{{WRAPPER}} .jobportal-box:hover .post-thumbnail',
		]);

		$this->add_group_control(Group_Control_Css_Filter::get_type(), [
			'name'     => 'css_filters_hover',
			'selector' => '{{WRAPPER}} .jobportal-box:hover .post-thumbnail img',
		]);
		...
		$this->add_control('thumbnail_opacity_hover', [
			...
			'selectors' => [
				'{{WRAPPER}} .jobportal-box:hover .post-thumbnail img' => 'opacity: {{SIZE}};',
			],
		]);
	...
	private function add_caption_style_section()
	{
		...
		$this->add_control('button_text_color', [
			...
			'default'   => '',
			'selectors' => [
				'{{WRAPPER}} .jobportal-button' => 'color: {{VALUE}};',
			],
		]);
		...
		$this->add_control('button_background_color', [
			...
			'type'      => Controls_Manager::COLOR,
			'selectors' => [
				'{{WRAPPER}} .jobportal-button:before' => 'background-color: {{VALUE}};',
			],
		]);
		...
		$this->add_control('button_hover_text_color', [
			...
			'type'      => Controls_Manager::COLOR,
			'selectors' => [
				'{{WRAPPER}} .jobportal-button:hover' => 'color: {{VALUE}};',
			],
		]);
		...
		$this->add_control('button_hover_background_color', [
			...
			'type'      => Controls_Manager::COLOR,
			'selectors' => [
				'{{WRAPPER}} .jobportal-button:after' => 'background-color: {{VALUE}};',
			],
		]);
	...
	protected function render()
	{
		...
		$this->add_render_attribute('wrapper', 'class', [
			"jobportal-grid-wrapper jobportal-blog',
			"jobportal-blog-' . $settings['layout'],
		]);

		if (!empty($settings['show_caption']) && 'yes' === $settings['show_caption']) {
			$this->add_render_attribute('wrapper', 'class', "jobportal-blog-caption-style-' . $settings['caption_style']);
		}

		if (!empty($settings['show_overlay']) && 'yes' === $settings['show_overlay']) {
			$this->add_render_attribute('wrapper', 'class', "jobportal-blog-overlay-style-' . $settings['overlay_style']);
		}

		$this->add_render_attribute('content-wrapper', 'class', "jobportal-grid');
...
?>
		...
		<div <?php $this->print_attributes_string('wrapper'); ?>>
			...
			?>
				...
				<?php
				$jobportal_grid_query['source']        = $settings['query_source'];
				$jobportal_grid_query['action']        = "{$post_type}_infinite_load";
				$jobportal_grid_query['max_num_pages'] = $query->max_num_pages;
				$jobportal_grid_query['found_posts']   = $query->found_posts;
				$jobportal_grid_query['count']         = $query->post_count;
				$jobportal_grid_query['query_vars']    = $this->get_query_args();
				$jobportal_grid_query['settings']      = $settings;
				$jobportal_grid_query                  = htmlspecialchars(wp_json_encode($jobportal_grid_query));
				?>
				<input type="hidden" class="jobportal-query-input" <?php echo 'value="' . $jobportal_grid_query . '"'; ?> />
				...
				<div <?php $this->print_attributes_string('content-wrapper'); ?>>
					...
					set_query_var('jobportal_query', $query);
