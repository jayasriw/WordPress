...
class Widget_Google_Map extends Base
{
	...
	{
		return "jobportal-google-map';
	}

	public function get_title()
	{
		return esc_html__('Advanced Google Map', 'jobportal');
	}
	...
	{
		return ['gmap3', 'gmap-api', "jobportal-widget-google-map'];
	}

    public function get_style_depends()
    {
        return ["jobportal-el-widget-google-map'];
    }
	...
	private function add_map_options_section()
	{
		$this->start_controls_section('map_options_section', [
			'label' => esc_html__('Map', 'jobportal'),
		]);
		...
		$this->add_control('style', [
			'label'   => esc_html__('Map Style', 'jobportal'),
		...
		$this->add_control('type', [
			'label'   => esc_html__('Map Type', 'jobportal'),
			'type'    => Controls_Manager::SELECT,
			'options' => [
				'roadmap'   => esc_html__('Roadmap', 'jobportal'),
				'satellite' => esc_html__('Satellite', 'jobportal'),
				'hybrid'    => esc_html__('Hybrid', 'jobportal'),
				'terrain'   => esc_html__('Terrain', 'jobportal'),
			],
			'default' => 'roadmap',
		]);

		$this->add_control('zoom_level', [
			'label'       => esc_html__('Zoom Level', 'jobportal'),
			'placeholder' => '12',
		...
		$this->add_control('scrollwheel', [
			'label'   => esc_html__('Mouse Scroll Wheel Zoom', 'jobportal'),
			'type'    => Controls_Manager::SWITCHER,
			'default' => 'yes',
		]);

		$this->add_control('draggable', [
			'label'   => esc_html__('Draggable', 'jobportal'),
			'type'    => Controls_Manager::SWITCHER,
			'default' => 'yes',
		]);

		$this->add_control('map_controls_heading', [
			'label'     => esc_html__('Controls', 'jobportal'),
			'type'      => Controls_Manager::HEADING,
			'separator' => 'before',
		]);

		$this->add_control('zoom_control', [
			'label' => esc_html__('Zoom Control', 'jobportal'),
			'type'  => Controls_Manager::SWITCHER,
		]);

		$this->add_control('scale_control', [
			'label' => esc_html__('Scale Control', 'jobportal'),
			'type'  => Controls_Manager::SWITCHER,
		]);

		$this->add_control('fullscreen_control', [
			'label' => esc_html__('Fullscreen Control', 'jobportal'),
			'type'  => Controls_Manager::SWITCHER,
		]);

		$this->add_control('street_view_control', [
			'label' => esc_html__('Street View Control', 'jobportal'),
			'type'  => Controls_Manager::SWITCHER,
		]);
	...
	private function add_map_markers_section()
	{
		$this->start_controls_section('markers_section', [
			'label' => esc_html__('Markers', 'jobportal'),
		]);

		$this->add_control('marker_style', [
			'label'       => esc_html__('Marker Style', 'jobportal'),
			'description' => esc_html__('Select default style for all markers', 'jobportal'),
			'type'        => Controls_Manager::SELECT,
			'options'     => [
				'icon'   => esc_html__('Icon', 'jobportal'),
				'signal' => esc_html__('Signal', 'jobportal'),
			],
			'default'     => 'icon',
		]);

		$this->add_control('marker_icon', [
			'label'     => esc_html__('Marker Icon', 'jobportal'),
			...
			'classes'   => "jobportal-control-media-auto',
		]);
		...
		$repeater->start_controls_tab('marker_content_tab', [
			'label' => esc_html__('Marker', 'jobportal'),
		]);

		$repeater->add_control('address', [
			'label'       => esc_html__('Address or Coordinate', 'jobportal'),
		...
		$repeater->start_controls_tab('marker_popup_tab', [
			'label' => esc_html__('Popup', 'jobportal'),
		]);

		$repeater->add_control('overlay_display', [
			'label'   => esc_html__('Display', 'jobportal'),
			'type'    => Controls_Manager::SELECT,
			'options' => [
				''       => esc_html__('Hover', 'jobportal'),
				'always' => esc_html__('Always', 'jobportal'),
			],
			'default' => '',
		]);
		...
		$repeater->add_control('image', [
			...
			'description' => esc_html__('Choose an image that displays on left info box', 'jobportal'),
			'type'        => Controls_Manager::MEDIA,
			'classes'     => "jobportal-control-media-auto',
		]);
		...
		$this->add_group_control(Group_Control_Image_Size::get_type(), [
			'label'     => esc_html__('Popup Image Size', 'jobportal'),
	...
	private function add_map_markers_style_section()
	{
		$this->start_controls_section('markers_popup_style_section', [
			'label' => esc_html__('Markers Popup', 'jobportal'),
		...
		$this->add_responsive_control('markers_popup_padding', [
			...
			'size_units' => ['px', '%', 'em'],
			'selectors'  => [
				'{{WRAPPER}} .jobportal-map-overlay-info' => 'padding: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
			],
		]);
		...
		$this->add_responsive_control('markers_popup_width', [
			...
			'selectors'  => [
				'{{WRAPPER}} .jobportal-map-overlay-content' => 'width: {{SIZE}}{{UNIT}};',
			],
		]);
...
<?php
	...
	private function get_overlay_value($settings, $marker, $position)
	{
		...
		$this->add_render_attribute($item_key, 'class', [
			"jobportal-map-overlay',
			"jobportal-map-overlay-{$marker['overlay_display']}",
		...
		if ($marker['title'] !== '' || $marker['content'] !== '') {
			$info_template .= '<div class="jobportal-map-overlay-template"><div class="jobportal-map-overlay-content"><div class="jobportal-map-overlay-info">';

			if (isset($marker['image'])) {
				$size  = \JobPortal_Image::elementor_parse_image_size($settings, '80x9999', 'popup_image_size');
				$image = \JobPortal_Image::get_attachment_by_id(array(
					'id'   => $marker['image']['id'],
					'size' => $size,
				));
	...
	{
		return [
			'no_label_bright_colors'  => esc_html__('No Label Bright Colors', 'jobportal'),
			'grayscale'               => esc_html__('Grayscale', 'jobportal'),
			'subtle_grayscale'        => esc_html__('Subtle Grayscale', 'jobportal'),
			'apple_paps_esque'        => esc_html__('Apple Maps-esque', 'jobportal'),
			'pale_dawn'               => esc_html__('Pale Dawn', 'jobportal'),
			'midnight_commander'      => esc_html__('Midnight Commander', 'jobportal'),
			'blue_water'              => esc_html__('Blue Water', 'jobportal'),
			'retro'                   => esc_html__('Retro', 'jobportal'),
			'paper'                   => esc_html__('Paper', 'jobportal'),
			'ultra_light_with_labels' => esc_html__('Ultra Light with Labels', 'jobportal'),
			'shades_of_grey'          => esc_html__('Shades Of Grey', 'jobportal'),
			'easy_light'              => esc_html__('Easy Light', 'jobportal'),
			'wy'                      => esc_html__('WY', 'jobportal'),
		];
	}
	...
	{
		$icon_url = JOBPORTAL_ELEMENTOR_ASSETS . '/images/map-marker.png';
