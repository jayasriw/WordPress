...
class Widget_Image_Box extends Base
{
	...
	{
		return "jobportal-image-box';
	}

	public function get_title()
	{
		return esc_html__('Image Box', 'jobportal');
	}
	...
	{
		return ["jobportal-el-widget-image-box'];
	}
	...
	private function add_image_box_section()
	{
		$this->start_controls_section('image_section', [
			'label' => esc_html__('Image Box', 'jobportal'),
		]);
		...
		$this->add_control('hover_effect', [
			'label'        => esc_html__('Hover Effect', 'jobportal'),
			'type'         => Controls_Manager::SELECT,
			'options'      => [
				''         => esc_html__('None', 'jobportal'),
				'zoom-in'  => esc_html__('Zoom In', 'jobportal'),
				'zoom-out' => esc_html__('Zoom Out', 'jobportal'),
				'move-up'  => esc_html__('Move Up', 'jobportal'),
			],
			'default'      => '',
		...
		$this->add_control('image', [
			'label'   => esc_html__('Choose Image', 'jobportal'),
		...
		$this->add_control('image_position', [
			'label'     => esc_html__('Image Position', 'jobportal'),
		...
		$this->add_control('description_text', [
			...
			'default'     => esc_html__('Click edit button to change this text. Lorem ipsum dolor sit amet, consectetur adipiscing elit. Ut elit tellus, luctus nec ullamcorper mattis, pulvinar dapibus leo.', 'jobportal'),
	...
	private function add_box_style_section()
	{
		...
		$this->add_responsive_control('text_align', [
			...
			'selectors' => [
				'{{WRAPPER}} .jobportal-box' => 'text-align: {{VALUE}};',
				'{{WRAPPER}} .jobportal-image.image' => 'margin-left: 0; margin-right: 0; display: inline-block;',
				'{{WRAPPER}} .jobportal-box.text-align-left .jobportal-image.image' => 'float: left;',
				'{{WRAPPER}} .jobportal-box.text-align-center .jobportal-image.image' => 'display: block; margin-left: auto; margin-right: auto; float: none;',
				'{{WRAPPER}} .jobportal-box.text-align-right .jobportal-image.image' => 'float: right;',
			],
		]);
		...
		$this->add_responsive_control('box_padding', [
			...
			'size_units' => ['px', '%', 'em'],
			'selectors'  => [
				'{{WRAPPER}} .jobportal-box' => 'padding: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
			],
		]);
		...
		$this->add_responsive_control('box_max_width', [
			...
			'selectors'  => [
				'{{WRAPPER}} .jobportal-box' => 'max-width: {{SIZE}}{{UNIT}};',
			],
		]);
		...
		$this->add_group_control(Group_Control_Background::get_type(), [
			'name'     => 'box',
			'selector' => '{{WRAPPER}} .jobportal-box',
		]);
		...
		$this->add_group_control(Group_Control_Background::get_type(), [
			'name'     => 'box_hover',
			'selector' => '{{WRAPPER}} .jobportal-box:before',
		]);

		$this->add_group_control(Group_Control_Advanced_Border::get_type(), [
			'name'     => 'box_hover_border',
			'selector' => '{{WRAPPER}} .jobportal-box:hover',
		]);
	...
	private function add_image_style_section()
	{
		...
		$this->add_responsive_control('image_wrap_height', [
			...
			'selectors' => [
				'{{WRAPPER}} .jobportal-image' => 'height: {{SIZE}}{{UNIT}};',
			],
		]);

		$this->add_responsive_control('image_space_top', [
			'label'     => esc_html__('Offset Top', 'jobportal'),
			'type'      => Controls_Manager::SLIDER,
			'range'     => [
				'px' => [
					'min' => 0,
		...
		$this->add_control('image_opacity', [
			'label'     => esc_html__('Opacity', 'jobportal'),
			'type'      => Controls_Manager::SLIDER,
			'range'     => [
				'px' => [
					'max'  => 1,
	...
	protected function render()
	{
		...
		$this->add_render_attribute('wrapper', 'class', "jobportal-image-box jobportal-box');
...
?>
		...
		<div class="content-wrap">
			...
			<?php if (!empty($settings['image']['url'])) : ?>
				<div class="jobportal-image image">
	...
	protected function content_template()
	{
		// @formatter:off
	?>
		<# view.addRenderAttribute( 'wrapper' , 'class' , "jobportal-image-box jobportal-box' ); view.addRenderAttribute( 'wrapper' , 'class' , 'style-' + settings.style ); view.addRenderAttribute( 'wrapper' , 'class' , 'image-position-' + settings.image_position ); view.addRenderAttribute( 'wrapper' , 'class' , 'content-alignment-' + settings.content_vertical_alignment ); var boxTag='div' ; if( '' !==settings.link.url && 'box'===settings.link_click ) { boxTag='a' ; view.addRenderAttribute( 'wrapper' , 'href' , '#' ); view.addRenderAttribute( 'wrapper' , 'class' , 'link-secret' ); } var imageHTML='' ; if ( settings.image.url ) { var image={ id: settings.image.id, url: settings.image.url, size: settings.image_size, dimension: settings.image_custom_dimension, model: view.getEditModel() }; var image_url=elementor.imagesManager.getImageUrl( image ); view.addRenderAttribute( 'image' , 'src' , image_url ); imageHTML='<div class="jobportal-image image"><img ' + view.getRenderAttributeString( 'image' ) + ' /></div>' ; } #>
										...
										<# view.addRenderAttribute( 'description_text' , 'class' , 'description' ); view.addInlineEditingAttributes( 'description_text' ); #>
											...
											<# } #>
												...
												<# if ( settings.button_text || settings.button_icon.value ) { #>
													<# var buttonIconHTML=elementor.helpers.renderIcon( view, settings.button_icon, { 'aria-hidden' : true }, 'i' , 'object' ); var buttonTag='div' ; view.addRenderAttribute( 'button' , 'class' , "jobportal-button button-' + settings.button_style ); view.addRenderAttribute( 'button' , 'class' , "jobportal-button-' + settings.button_size ); if ( '' !==settings.link.url && 'button'===settings.link_click ) { buttonTag='a' ; view.addRenderAttribute( 'button' , 'href' , '#' ); } if ( settings.button_icon.value ) { view.addRenderAttribute( 'button' , 'class' , 'icon-' + settings.button_icon_align ); } view.addRenderAttribute( 'button-icon' , 'class' , 'button-icon' ); #>
