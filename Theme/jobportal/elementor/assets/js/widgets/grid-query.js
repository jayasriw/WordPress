(function ($) {
	...
	$.fn.JobPortalGridQuery = function () {
		var $el, $grid;
		...
		function initFilterCount() {
			if (!$el.children(".jobportal-grid-filter").data("filter-counter")) {
				return;
			}
		...
		function handlerFilter() {
			$el
				.children(".jobportal-grid-filter")
				.on("click", ".btn-filter", function (e) {
					...
					$el.trigger("JobPortalBeginQuery");
		...
		function handlerPagination() {
			...
			if ($el.data("pagination") === "load-more") {
				$el
					.children(".jobportal-grid-pagination")
					.find(".jobportal-load-more-button")
					.on("click", function (e) {
						e.preventDefault();
		...
		function handlerQuery(reset) {
			...
			if (infiniteLoader === true) {
				loader = $el.find(".jobportal-infinite-loader");
			} else {
				loader = $grid.children(".jobportal-grid-loader");
			}
			...
			setTimeout(function () {
				...
				$.ajax({
					...
					success: function (results) {
						...
						$el.trigger("JobPortalQueryEnd", [$el, $newItems]);
		...
		function handlerQueryEnd() {
			...
			if (foundPosts <= paged * numberPages) {
				if ($el.data("pagination") === "load-more-alt") {
					...
					$el.children(".jobportal-grid-pagination").hide();
				}

				$el.children(".jobportal-grid-messages").show(1);
				setTimeout(function () {
					$el.children(".jobportal-grid-messages").remove();
				}, 5000);
			} else {
				if ($el.data("pagination") === "load-more-alt") {
					var loadMoreBtn = $($el.data("pagination-custom-button-id"));
					...
					$el.children(".jobportal-grid-pagination").show();
					$el
						.children(".jobportal-grid-pagination")
						.find(".jobportal-load-more-button")
						.show();
		...
		return this.each(function () {
			$el = $(this);
			$grid = $el.find(".jobportal-grid");
			$queryInput = $el.find(".jobportal-query-input").first();

			initFilterCount();
			handlerFilter();
			handlerPagination();

			$el.on("JobPortalBeginQuery", function () {
				// Reset to first page.
