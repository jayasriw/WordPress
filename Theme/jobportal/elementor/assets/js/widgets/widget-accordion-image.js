(function ($) {
	...
	var JobPortalAccordionHandler = function ($scope, $) {
		var $list_img = $scope.find( '.right img' ),
			$element = $scope.find(".jobportal-accordion-image");
		...
		$element.on("click", ".accordion-header", function (e) {
			...
			if (section.hasClass("active")) {
				...
				var parent = $(this).parents(".jobportal-accordion-image").first();
	...
	$(window).on("elementor/frontend/init", function () {
		elementorFrontend.hooks.addAction(
			"frontend/element_ready/jobportal-accordion-image.default",
			JobPortalAccordionHandler
		);
	});
})(jQuery);
