(function ($) {
    ...
    $(window).on('load resize',function(){
        var win_w=window.innerWidth,wrap=$('.jobportal-morphing-svg-wrapper');
        if(wrap.length > 0){
            var viewbox_morph=document.querySelectorAll('.jobportal-morphing-svg-wrapper');
    ...
    var JobPortalMorphingHandler = function($scope, $) {
        var morphing_wrap = $scope.find('.jobportal-morphing-svg-wrapper'),
            morph_fixed_bg = $scope.find('.jobportal-morph-fixed-scroll-bg'),
        ...
        if(wid_sec.length){
            ...
            if($scope.closest('.elementor').find('> [data-morph-id="'+morph_id+'"]').length > 0){
                var remove_morph=$scope.closest('.elementor').find("> .jobportal-morphing-svg-wrapper");
                remove_morph.remove();
            }
            //fixed bg Morph
            if($scope.closest('.elementor').find('> [data-morph-fixed="'+morph_fixed_bg_id+'"]').length > 0){
                var remove_morph=$scope.closest('.elementor').find("> .jobportal-morph-fixed-scroll-bg");
                remove_morph.remove();
            }
            //sections Morph
            if($scope.closest('section.elementor-element,.elementor-element.e-container,.elementor-element.e-con').find('> [data-morph-id="'+morph_id+'"]').length > 0){
                var remove_morph=$scope.closest('section.elementor-element,.elementor-element.e-container,.elementor-element.e-con').find("> .jobportal-morphing-svg-wrapper");
                remove_morph.remove();
            }
            //column Morph
            if($scope.closest('.elementor-column').find('> [data-morph-id="'+morph_id+'"]').length > 0){
                var remove_morph=$scope.closest('.elementor-column').find("> .jobportal-morphing-svg-wrapper");
                remove_morph.remove();
            }
        }
    ...
    $(window).on('elementor/frontend/init', function () {
        elementorFrontend.hooks.addAction('frontend/element_ready/jobportal-morphing.default', JobPortalMorphingHandler);
    });
