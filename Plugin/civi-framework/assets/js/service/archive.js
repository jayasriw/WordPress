...
(function ($) {
    ...
    var ajax_url = civi_template_vars.ajax_url,
        ...
        item_amount = civi_service_archive_vars.item_amount,
        not_service = civi_service_archive_vars.not_service;
    ...
    var civi_hover_map_effects = function () {
        if (map_effects !== '' && has_map) {
            $(".map-event .area-service .civi-service-item").each(function () {
    ...
    SERVICE.elements = {
        ...
        service_layout: function () {
            archive_service.find(".service-layout a").on("click", function (event) {
                ...
                $(".area-service .civi-service-item").each(function () {
        ...
        display_clear: function () {
            ...
            var has_active_filters = (
                ...
                $('.civi-menu-filter input[name="service_filter_price_min"]').val() !== "" ||
                $('.civi-menu-filter input[name="service_filter_price_max"]').val() !== "" ||
                $('.civi-menu-filter select[name="service_time_type"]').val() !== ""
            );
            ...
            if (
                ...
                $('.civi-menu-filter input[name="service_filter_price_min"]').closest(".entry-filter").addClass("open");
            }
            if ($('.civi-menu-filter select[name="service_time_type"]').val() !== "") {
                $('.civi-menu-filter select[name="service_time_type"]').closest(".entry-filter").addClass("open");
            }
            if (has_active_filters) {
                $(".civi-nav-filter").addClass("active");
                archive_service.find(".civi-clear-filter").show();
            } else {
                $(".civi-nav-filter").removeClass("active");
                archive_service.find(".civi-clear-filter").hide();
            }
        },

        filter_clear_top: function () {
            archive_service.find(".civi-clear-top-filter").on("click", function () {
                ...
                $(".form-service-top-filter .civi-select2").val("");
                $(".form-service-top-filter .civi-select2").select2("destroy");
                $(".form-service-top-filter .civi-select2").each(function () {
        ...
        filter_clear: function () {
            archive_service.find(".civi-clear-filter").on("click", function () {
                $(".civi-menu-filter ul.filter-control li").removeClass("active");
                $('.civi-menu-filter input[type="checkbox"]').prop("checked", false);
                $('.civi-menu-filter input[name="service_filter_price_min"]').val("");
                $('.civi-menu-filter input[name="service_filter_price_max"]').val("");
                $('.civi-menu-filter select[name="service_time_type"]').val(null).trigger('change');
        ...
        ajax_load: function (ajax_call, pagination) {
            ...
            radius_cities = $('.civi-form-location').find('input[name="service_number_radius"]').val();
                ...
                var civi_add_markers = function (props, map) {
                    $.each(props, function (i, prop) {
                        ...
                        google.maps.event.addListener(marker, "click", function () {
                            ...
                            var elem = $(marker.url);
                            $(".area-service .civi-service-item").removeClass("highlight");
                            if (
                                elem.length > 0 &&
                                click_marker &&
            ...
            var type_pagination = $(".civi-pagination").attr("data-type");
            $(".area-service .civi-service-item").addClass("skeleton-loading");

            $.ajax({
                dataType: "json",
                url: ajax_url,
                data: {
                    action: "civi_service_archive_ajax",
                    paged: paged,
                    title: title,
                ...
                success: function (data) {
                    ...
                    archive_service.find(".civi-filter-search-map .civi-loading-effect").fadeOut();
                    $(".area-service .civi-service-item").removeClass("skeleton-loading");
                        ...
                        if (ajax_call == true) {
                            if (
                                ...
                                if (data.hidden_pagination) {
                                    archive_service.find(".civi-pagination .pagination").html("");
                                }
                        ...
                        if (ajax_call == true) {
                            if (
                                ...
                                archive_service
                                    .find(".area-service")
                                    .html(
                                        '<div class="civi-ajax-result">' + not_service + "</div>"
                                    );
                                archive_service.find(".result-count").html(data.count_post);
                                archive_service.find(".civi-pagination .pagination").html("");
                            } else {
                    ...
                    if (has_map) {
                        if (mapType == "google_map") {
                            ...
                            markers = [];
                            civi_add_markers(data.service, civi_map);
                            ...
                            civi_osm_add_markers(data.service, maps);
                        } else {
                            civi_mapbox_add_markers(data.service, map);
                        }
                    }

                    // Scroll to first item after pagination
                    if (pagination === "pagination") {
                        SERVICE.elements.scroll_to(".area-service .civi-service-item:first");
                    }
                },
            });
        },
