(function ($) {
	$(document).on('click', '.civi-approve-candidate', function (e) {
		e.preventDefault();
		var $link = $(this);
		...
		$.post(civiApproveCandidate.ajax_url, {
			action: 'civi_approve_candidate',
			_ajax_nonce: nonce,
			post_id: postId
		}).done(function (res) {
			if (res && res.success) {
				$link.closest('tr').find('.column-title .row-title').after(' <span class="label civi-label-blue">' + civiApproveCandidate.i18n.approved + '</span>');
				$link.remove();
			} else {
