(function ($) {
	$(document).on('click', '.civi-approve-user-status', function (e) {
		e.preventDefault();
		var $link = $(this);
		...
		$.post(civiApproveUser.ajax_url, {
			action: 'civi_approve_user_status',
			_ajax_nonce: nonce,
			user_id: userId
		}).done(function (res) {
			if (res && res.success) {
				$link.closest('tr').find('.column-custom_column').html('<span class="label civi-label-blue">' + civiApproveUser.i18n.approved + '</span>');
				$link.remove();
			} else {
