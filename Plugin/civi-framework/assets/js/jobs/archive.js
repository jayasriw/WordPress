	...
	JOBS.elements = {
		...
		ajax_load: function (ajax_call, pagination) {
			...
			if (maptype == "google_map") {
				...
				var civi_osm_add_markers = function (props) {
					$(".maptype").remove();
				...
				var civi_mapbox_add_markers = function (props) {
			...
			$.ajax({
				...
				success: function (data) {
					...
					if (maptype == "google_map") {
						...
						$(".area-jobs .civi-jobs-item").removeClass("skeleton-loading");
						if (has_map) {
							civi_osm_add_markers(data.jobs);
						}
						if (data.success === true) {
							if (ajax_call == true) {
								if (
						...
						$(".area-jobs .civi-jobs-item").removeClass("skeleton-loading");
						if (has_map) {
							civi_mapbox_add_markers(data.jobs);
						}
						if (data.success === true) {
							if (ajax_call == true) {
								if (
					...
					if (pagination === "pagination") {
						JOBS.elements.scroll_to(".area-jobs .civi-jobs-item:first");
					}
				},
			});
		},
		...
		preview_job: function () {
			...
			$(".inner-content.layout-full .civi-jobs-item").off("click");
