(function ($) {
  ...
  $(document).ready(function () {
    ...
    $(".ai-generate").on("submit", function (e) {
      ...
      if (!keywords) {
        isValid = false;
        errorMsg += civi_submit_vars.enter_description + "\n";
      }

      if (keywords && keywords.length < 20) {
        isValid = false;
        errorMsg += civi_submit_vars.min_length + "\n";
      }

      if (keywords && keywords.length > 2000) {
        isValid = false;
        errorMsg += civi_submit_vars.max_length + "\n";
      }
      ...
      if (keywords) {
        ...
        if (hasPlaceholder) {
          isValid = false;
          errorMsg += civi_submit_vars.replace_placeholder + "\n";
        }

        if (containsSensitive && !hasPlaceholder) {
          sensitiveWarningMessage = civi_submit_vars.avoid_sensitive;
      ...
      if (!tone) {
        isValid = false;
        errorMsg += civi_submit_vars.select_tone + "\n";
      }
      if (!language) {
        isValid = false;
        errorMsg += civi_submit_vars.select_language + "\n";
      }
      ...
      var aiTimeout = parseInt(civi_submit_vars.ai_timeout, 10);
      if (isNaN(aiTimeout) || aiTimeout <= 0) {
        aiTimeout = 120000;
      }

      var warningDelay = parseInt(civi_submit_vars.ai_timeout_warning, 10);
      ...
      if (!sensitiveWarningMessage) {
        aiResponseWarningTimer = setTimeout(function () {
          _this.find(".field-notice p").text(civi_submit_vars.timeout_warning || "The AI request is taking longer than expected...");
          _this.find(".field-notice").addClass("warning").removeClass("error");
        }, warningDelay);
      }
      ...
      aiRequest = $.ajax({
        ...
        data: {
          ...
          security: civi_submit_vars.ai_nonce
        },
        timeout: aiTimeout,
        beforeSend: function () {
        ...
        success: function (response) {
          ...
          try {
            ...
            _this.find(".field-notice p").text(civi_submit_vars.connection_error || 'Invalid response from server.');
            _this.find(".field-notice").addClass("error");
            return;
          }

          if (!parsedResponse || typeof parsedResponse !== 'object') {
            _this.find(".field-notice p").text(civi_submit_vars.connection_error || 'Invalid response format.');
            _this.find(".field-notice").addClass("error");
            return;
          }
          ...
          if (parsedResponse.success && parsedResponse.message) {
            ...
            var errorMessage = parsedResponse.message || civi_submit_vars.connection_error || 'An error occurred. Please try again.';
        ...
        error: function (xhr, status, error) {
          ...
          if (status === "timeout") {
            _this.find(".field-notice p").text(civi_submit_vars.timeout_error || civi_submit_vars.connection_error);
          } else {
            _this.find(".field-notice p").text(civi_submit_vars.connection_error);
          }
