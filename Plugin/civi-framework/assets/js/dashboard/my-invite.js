(function ($) {
    ...
    var my_invite = $(".civi-my-invite");

    var ajax_url = civi_my_invite_vars.ajax_url,
        not_jobs = civi_my_invite_vars.not_jobs;
    ...
    $(document).ready(function () {
        ...
        my_invite.find("select.search-control").on("change", function () {
            my_invite.find(".civi-pagination").find('input[name="paged"]').val(1);
            ajax_load_invite();
        });
        ...
        $("body").on(
            "click",
            ".civi-my-invite .jobs-control .btn-delete",
            function (e) {
                e.preventDefault();
        ...
        $("body").on(
            "click",
            ".civi-my-invite .civi-pagination a.page-numbers",
            function (e) {
                e.preventDefault();
                my_invite
                ...
                var current_page = 1;
                if (
                    my_invite.find(".civi-pagination").find('input[name="paged"]').val()
                ) {
        ...
        function ajax_load_invite(item_id = "", action_click = "") {
            ...
            paged = my_invite.find('.civi-pagination input[name="paged"]').val();

            $.ajax({
                dataType: "json",
                url: ajax_url,
                data: {
                    action: "civi_filter_my_invite",
                ...
                beforeSend: function () {
                    my_invite.find(".civi-loading-effect").addClass("loading").fadeIn();
