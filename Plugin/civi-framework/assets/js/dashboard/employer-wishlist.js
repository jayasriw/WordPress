(function ($) {
    ...
    var employer_wishlist = $(".civi-employer-wishlist");

    var ajax_url = civi_employer_wishlist_vars.ajax_url,
        not_service = civi_employer_wishlist_vars.not_service;
    ...
    $(document).ready(function () {
        ...
        employer_wishlist.find("select.search-control").on("change", function () {
            employer_wishlist.find(".civi-pagination").find('input[name="paged"]').val(1);
            ajax_load_wishlist();
        });
        ...
        $("body").on(
            "click",
            ".civi-employer-wishlist .service-control .btn-delete",
            function (e) {
                e.preventDefault();
        ...
        $("body").on(
            "click",
            ".civi-employer-wishlist .civi-pagination a.page-numbers",
            function (e) {
                e.preventDefault();
                employer_wishlist
                ...
                var current_page = 1;
                if (
                    employer_wishlist.find(".civi-pagination").find('input[name="paged"]').val()
                ) {
        ...
        function ajax_load_wishlist(item_id = "", action_click = "") {
            ...
            paged = employer_wishlist.find('.civi-pagination input[name="paged"]').val();

            $.ajax({
                dataType: "json",
                url: ajax_url,
                data: {
                    action: "civi_filter_employer_wishlist",
                ...
                beforeSend: function () {
                    employer_wishlist.find(".civi-loading-effect").addClass("loading").fadeIn();
