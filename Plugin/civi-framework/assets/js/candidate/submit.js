...
jQuery(document).ready(function ($) {
  ...
  var text_present = civi_candidate_vars.text_present;
  var custom_field_candidate = civi_candidate_vars.custom_field_candidate;
  var date_format = civi_template_vars.date_format;
  ...
  if (tabTemplate.length > 0) {
    $.each(tabTemplate, function () {
      ...
      var $index = 1,
        ...
        item = tabID.find(".civi-candidate-wrapper .row").length,
  ...
  var $rowActive = submit_form.find(
    ".civi-candidate-wrapper > .row:first-child"
  );
  ...
  function saveProfileStrength() {
    ...
    $.ajax({
      type: "POST",
      url: civi_candidate_vars.ajax_url,
      data: {
        action: "save_candidate_profile_strength",
        nonce: civi_candidate_vars.save_strength_nonce,
  ...
  function markPoint() {
    ...
    try {
      ...
      var specialFieldSelectors = [
        ...
        'input[name="civi_map_address"]',
        'input[name="civi_map_location"]',
        'input[name="civi_latitude"]',
        'input[name="civi_longtitude"]',
      ];
      ...
      // Media Gallery
      var mediaGalleryContainer = submit_form.find("#civi_gallery_thumbs");
      ...
      var cvFilesList = submit_form.find("#civi_cv_files_list");
      ...
      if (percent !== lastSavedProfileStrength && typeof civi_candidate_vars !== "undefined") {
  ...
  function checkTabCompletion() {
    ...
    $.each(textTab, function (index, val) {
      ...
      if (val === "info") {
        ...
        var rows = tabID.find(".civi-candidate-wrapper .row");
  ...
  $(document).ready(function () {
    ...
    $(document).off("change", ".civi-select2");

    $(document).on("change keyup", ".point-mark", function () {
      debouncedMarkPoint();
    });

    $(document).on("change", ".civi-select2", function () {
      debouncedMarkPoint();
    });
    ...
    $(document).on(
      ...
      function () {
        ...
        if (
          $this.closest(
            ".avatar-container, .candidate-fields-avatar, #civi_avatar_container"
          ).length > 0
        ) {
          setTimeout(function () {
            debouncedMarkPoint();
    ...
    $(document).on(
      "change",
      'input[name="civi_map_location"], input[name="civi_map_address"]',
      function () {}
    );
  ...
  function ajax_submit() {
    ...
    $.ajax({
      ...
      data: {
        ...
        civi_security_candidate_submit: $("#civi_security_candidate_submit").val(),
        ...
        civi_map_address: candidate_map_address,
        civi_map_location: candidate_map_location,
        civi_latitude: candidate_latitude,
        civi_longtitude: candidate_longtitude,
