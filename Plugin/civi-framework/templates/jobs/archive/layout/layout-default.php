...
$items_amount = civi_get_option('archive_jobs_items_amount', '12');
...
$pagination_type = civi_get_option('jobs_pagination_type');
if ($pagination_type == 'loadpage') {
	$paged_load = isset($_GET['nagi-paged']) ? civi_clean(wp_unslash($_GET['nagi-paged'])) : '1';
...
if (!empty($jobs_location)) {
	$location_term = Civi_Location_Search::find_location_term($jobs_location, 'jobs-location');
...
$jobs_categories_param = isset($_GET['jobs-categories']) ? civi_clean(wp_unslash($_GET['jobs-categories'])) : '';
...
$jobs_type_param = isset($_GET['jobs-type']) ? civi_clean(wp_unslash($_GET['jobs-type'])) : '';
...
$jobs_skills_param = isset($_GET['jobs-skills']) ? civi_clean(wp_unslash($_GET['jobs-skills'])) : '';
...
$jobs_experience_param = isset($_GET['jobs-experience']) ? civi_clean(wp_unslash($_GET['jobs-experience'])) : '';
...
$jobs_career_param = isset($_GET['jobs-career']) ? civi_clean(wp_unslash($_GET['jobs-career'])) : '';
...
$jobs_gender_param = isset($_GET['jobs-gender']) ? civi_clean(wp_unslash($_GET['jobs-gender'])) : '';
...
$jobs_qualification_param = isset($_GET['jobs-qualification']) ? civi_clean(wp_unslash($_GET['jobs-qualification'])) : '';
...
$args = apply_filters('civi/archive-jobs/layout-default/query/args', $args);
		...
		<div class="filter-wrapper">
			<div class="entry-left">
				...
				<span class="result-count">
					<?php if (!empty($key)) { ?>
						<?php printf(_n('%1$s job for "%2$s"', '%1$s jobs for "%2$s"', $total_post_for_n, 'civi-framework'), '<span>' . $total_post . '</span>', esc_html($key)); ?>
					<?php } elseif (is_tax() && !empty($current_term_name)) { ?>
						<?php printf(_n('%1$s job for "%2$s"', '%1$s jobs for "%2$s"', $total_post_for_n, 'civi-framework'), '<span>' . $total_post . '</span>', esc_html($current_term_name)); ?>
					<?php } elseif (!empty($current_term_name)) { ?>
						<?php printf(_n('%1$s job for "%2$s"', '%1$s jobs for "%2$s"', $total_post_for_n, 'civi-framework'), '<span>' . $total_post . '</span>', esc_html($current_term_name)); ?>
					<?php } else { ?>
						<?php printf(_n('%1$s job', '%1$s jobs', $total_post_for_n, 'civi-framework'), '<span>' . $total_post . '</span>'); ?>
					<?php } ?>
				</span>
			</div>
			<div class="entry-right">
				<div class="entry-filter filter-wrapper inner-box">
					...
					<select name="sort_by" class="sort-by filter-control civi-select2">
						<option value="featured" selected><?php esc_html_e('Featured', 'civi-framework'); ?></option>
