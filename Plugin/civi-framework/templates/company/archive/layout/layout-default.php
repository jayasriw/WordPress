...
$item_amount = civi_get_option('archive_company_items_amount', '12');
...
$key          = isset($_GET['s']) ? urldecode(civi_clean(wp_unslash($_GET['s']))) : '';
$archive_class   = array();
...
if (!empty($company_location_values)) {
    ...
    foreach ($company_location_values as $loc) {
        ...
        $location_term = Civi_Location_Search::find_location_term($loc, 'company-location');
...
$company_categories_param = isset($_GET['company-categories']) ? civi_clean(wp_unslash($_GET['company-categories'])) : '';
...
$company_size_param = isset($_GET['company-size']) ? civi_clean(wp_unslash($_GET['company-size'])) : '';
...
$args = apply_filters('civi/archive-company/layout-default/query/args', $args);
...
<div class="inner-content container <?php echo join(' ', $class_inner); ?>">
    <div class="col-left">
        ...
        <div class="filter-wrapper">
            <div class="entry-left">
                ...
                <span class="result-count">
                    <?php if (!empty($key)) { ?>
                        <?php printf(_n('%1$s company for "%2$s"', '%1$s companies for "%2$s"', $total_post_for_n, 'civi-framework'), '<span>' . $total_post . '</span>', $key); ?>
                    <?php } elseif (is_tax()) { ?>
                        <?php printf(_n('%1$s company for "%2$s"', '%1$s companies for "%2$s"', $total_post_for_n, 'civi-framework'), '<span>' . $total_post . '</span>', $current_term_name); ?>
                    <?php } else { ?>
                        <?php printf(_n('%1$s company', '%1$s companies', $total_post_for_n, 'civi-framework'), '<span>' . $total_post . '</span>'); ?>
                    <?php } ?>
                </span>
            </div>
        ...
        $pagination_type = civi_get_option('company_pagination_type');
