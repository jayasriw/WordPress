...
if (!empty($service_location_values)) {
    ...
    foreach ($service_location_values as $loc) {
        ...
        $location_term = Civi_Location_Search::find_location_term($loc, 'service-location');
...
$service_skills_param = isset($_GET['service-skills']) ? civi_clean(wp_unslash($_GET['service-skills'])) : '';
...
$service_language_param = isset($_GET['service-language']) ? civi_clean(wp_unslash($_GET['service-language'])) : '';
...
$service_categories_param = isset($_GET['service-categories']) ? civi_clean(wp_unslash($_GET['service-categories'])) : '';
...
$args = apply_filters('civi/archive-service/layout-default/query/args', $args);
        ...
        <div class="filter-wrapper">
            <div class="entry-left">
                ...
                <span class="result-count">
                    <?php if (!empty($key)) { ?>
                        <?php printf(_n('%1$s service for "%2$s"', '%1$s services for "%2$s"', $total_post_for_n, 'civi-framework'), '<span>' . $total_post . '</span>', $key); ?>
                    <?php } elseif (is_tax()) { ?>
                        <?php printf(_n('%1$s service for "%2$s"', '%1$s services for "%2$s"', $total_post_for_n, 'civi-framework'), '<span>' . $total_post . '</span>', $current_term_name); ?>
                    <?php } else { ?>
                        <?php printf(_n('%1$s service', '%1$s services', $total_post_for_n, 'civi-framework'), '<span>' . $total_post . '</span>'); ?>
                    <?php } ?>
                </span>
            </div>
