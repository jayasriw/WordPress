...
$candidate_package_number_jobs_applys = intval(get_user_meta($user_id, CIVI_METABOX_PREFIX . 'candidate_package_number_jobs_apply', true));
...
<?php else : ?>
    ...
    <div class="entry-my-page packages-dashboard my-candidate-package">
                                ...
                                <?php } else { ?>
                                    <?php if (($current_date < $expired_date) || ($enable_package_service_unlimited_time == 1)) { ?>
                                        <?php if ($candidate_package_number_jobs_applys < 1) { ?>
                                            <span class="label label-close"><?php esc_html_e('Finished', 'civi-framework') ?></span>
                                        <?php } else { ?>
                            ...
                            <td>
                                <span class="remaining">
                                    <?php if ($enable_package_service_unlimited_time == 1) {
                                        ...
                                        echo sprintf(esc_html__('%s Day', 'civi-framework'), $expired_days);
                                    } ?>
                                </span>
                            </td>
                            <td>
        ...
        <?php endif; ?>
        <a href="<?php echo civi_get_permalink('candidate_package'); ?>" class="civi-button civi-new-package" id="civi-new-package-link">
    ...
    civi_get_template('dashboard/candidate/package-warning-modal.php');
    ?>
        ...
        ?>
            <div class="table-dashboard-wapper">
                <table class="table-dashboard">
                    <thead>
                        <tr>
                            <th><?php esc_html_e('Order #', 'civi-framework') ?></th>
                    ...
                    <tbody>
                        <?php while ($orders->have_posts()) : $orders->the_post();
                            $order_id = get_the_ID();
                            $order_package_id = get_post_meta($order_id, CIVI_METABOX_PREFIX . 'candidate_order_item_id', true);
                            $price = get_post_meta($order_id, CIVI_METABOX_PREFIX . 'candidate_order_price', true);
                            $order_date = get_post_meta($order_id, CIVI_METABOX_PREFIX . 'candidate_order_date', true);
                            ...
                            $payment_method = isset($order_meta['candidate_order_payment_method']) ? $order_meta['candidate_order_payment_method'] : get_post_meta($order_id, CIVI_METABOX_PREFIX . 'candidate_order_payment_method', true);
                            ...
                            if ($is_paid && $order_package_id) {
                                ...
                                $current_package_id = get_the_author_meta(CIVI_METABOX_PREFIX . 'candidate_package_id', $user_id);
                                ...
                                if ($order_package_id == $current_package_id) {
                                    ...
                                    $expired_date_result = $civi_candidate_package->get_expired_date($order_package_id, $user_id);
                                    ...
                                    if ($order_purchase_date) {
                                        ...
                                        $candidate_package_time_unit = get_post_meta($order_package_id, CIVI_METABOX_PREFIX . 'candidate_package_time_unit', true);
                                        $candidate_package_period = get_post_meta($order_package_id, CIVI_METABOX_PREFIX . 'candidate_package_period', true);
                                        $enable_package_service_unlimited_time = get_post_meta($order_package_id, CIVI_METABOX_PREFIX . 'enable_package_service_unlimited_time', true);
                        ...
                        ?>
                            <tr>
                                <td>
                                    <?php
                                    $wc_order_number = get_post_meta($order_id, CIVI_METABOX_PREFIX . 'candidate_order_wc_order_id', true);
                                ...
                                <td>
                                    <?php
                                    $payment_method_display = Civi_Candidate_Order::get_candidate_order_payment_method($payment_method);
                                    ...
                                    <?php if ($order_date) : ?>
                                        <small class="payment-date-label">
                                            <?php esc_html_e('Create at', 'civi-framework'); ?> <?php echo esc_html(date_i18n(get_option('date_format'), strtotime($order_date))); ?>
                                        </small>
                                    <?php endif; ?>
                                </td>
    ...
    wp_localize_script(CIVI_PLUGIN_PREFIX . 'user-package-warning', 'civi_package_vars', array(
        ...
        'nonce' => wp_create_nonce('civi_candidate_package_ajax_nonce')
    ));
    ?>
<?php endif; ?>
