...
$package_activate = get_user_meta($user_id, CIVI_METABOX_PREFIX . 'package_activate_date', true);
$package_activate_date = civi_convert_date_format($package_activate);
...
<?php else : ?>
    ...
    <div class="entry-my-page packages-dashboard">
                            ...
                            <td>
                                <span class="remaining">
                                    <?php if ($package_unlimited_time == 1) {
                                        ...
                                        echo sprintf(esc_html__('%s Day', 'civi-framework'), $expired_jobs);
                                    } ?>
                                </span>
                            </td>
                            <td>
                                ...
                                if (!empty($data_invoice->post)) :
                                    ...
                                    <?php } else { ?>
                                        <?php if (($current_date < $expired_date) || ($package_unlimited_time == 1)) { ?>
                                            <span class="label label-open"><?php esc_html_e('Activated', 'civi-framework') ?></span>
                                        <?php } else { ?>
        ...
        <?php endif; ?>
        <a href="<?php echo civi_get_permalink('package'); ?>" class="civi-button civi-new-package" id="civi-new-package-link">
    ...
    civi_get_template('dashboard/employer/package-warning-modal.php');
    ?>

    <!-- Purchase History Section -->
    <div class="entry-my-page packages-dashboard purchase-history">
        <div class="entry-title">
            <h4><?php esc_html_e('Purchase History', 'civi-framework') ?></h4>
        </div>
        ...
        // Payment page URL
        $payment_page_url = civi_get_permalink('payment');
        ...
        ?>
            <div class="table-dashboard-wapper">
                <table class="table-dashboard">
                    <thead>
                        <tr>
                            <th><?php esc_html_e('Invoice #', 'civi-framework') ?></th>
                            <th><?php esc_html_e('Package', 'civi-framework') ?></th>
                            <th><?php esc_html_e('Price', 'civi-framework') ?></th>
                            <th><?php esc_html_e('Payment', 'civi-framework') ?></th>
                            <th><?php esc_html_e('Date', 'civi-framework') ?></th>
                            <th><?php esc_html_e('Active Date', 'civi-framework') ?></th>
                            <th><?php esc_html_e('Expired Date', 'civi-framework') ?></th>
                            <th><?php esc_html_e('Status', 'civi-framework') ?></th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php while ($invoices->have_posts()) : $invoices->the_post();
                            ...
                            $invoice_package_id = get_post_meta($invoice_id, CIVI_METABOX_PREFIX . 'invoice_item_id', true);
                            $price = get_post_meta($invoice_id, CIVI_METABOX_PREFIX . 'invoice_price', true);
                            $invoice_date = get_post_meta($invoice_id, CIVI_METABOX_PREFIX . 'invoice_date', true);
                            $status = get_post_meta($invoice_id, CIVI_METABOX_PREFIX . 'invoice_payment_status', true);
                            $invoice_package_name = get_the_title($invoice_package_id);

                            // Get invoice meta (same as admin files)
                            $civi_invoice = new Civi_Invoice();
                            $invoice_meta = $civi_invoice->get_invoice_meta($invoice_id);
                            $payment_method = isset($invoice_meta['invoice_payment_method']) ? $invoice_meta['invoice_payment_method'] : get_post_meta($invoice_id, CIVI_METABOX_PREFIX . 'invoice_payment_method', true);
                            ...
                            if ($is_paid && $invoice_package_id) {
                                ...
                                $current_package_id = get_the_author_meta(CIVI_METABOX_PREFIX . 'package_id', $user_id);
                                ...
                                if ($invoice_package_id == $current_package_id) {
                                    ...
                                    $expired_date_result = $civi_package->get_expired_date($invoice_package_id, $user_id);
                                    if ($expired_date_result && $expired_date_result !== 'Never Expires') {
                                        ...
                                        $expired_date = esc_html__('Never Expires', 'civi-framework');
                                    }
                                } else {
                                    ...
                                    if ($invoice_purchase_date) {
                                        ...
                                        $package_time_unit = get_post_meta($invoice_package_id, CIVI_METABOX_PREFIX . 'package_time_unit', true);
                                        $package_period = get_post_meta($invoice_package_id, CIVI_METABOX_PREFIX . 'package_period', true);
                                        $package_unlimited_time = get_post_meta($invoice_package_id, CIVI_METABOX_PREFIX . 'package_unlimited_time', true);
                        ...
                        ?>
                            <tr>
                                <td>
                                    <?php
                                    $wc_order_number = get_post_meta($invoice_id, CIVI_METABOX_PREFIX . 'invoice_wc_order_id', true);
                                ...
                                <td>
                                    ...
                                    <?php else : ?>
                                        <span class="text-muted"><?php esc_html_e('Package Removed', 'civi-framework'); ?></span>
                                    <?php endif; ?>
                                </td>
                                <td><?php echo civi_get_format_money($price); ?></td>
                                <td>
                                    <?php
                                    $payment_method_display = Civi_Invoice::get_invoice_payment_method($payment_method);
                                    if (empty($payment_method_display)) {
                                        $payment_method_display = esc_html__('Unknown', 'civi-framework');
                                    }
                                    ?>
                                    <div class="payment-method-label"><?php echo esc_html($payment_method_display); ?></div>
                                    <?php if ($invoice_date) : ?>
                                        <small class="payment-date-label">
                                            <?php esc_html_e('Create at', 'civi-framework'); ?> <?php echo esc_html(date_i18n(get_option('date_format'), strtotime($invoice_date))); ?>
                                        </small>
                                    <?php endif; ?>
                                </td>
                                ...
                                <td>
                                    ...
                                    <?php elseif ($is_paid) : ?>
                                        <span class="label label-open"><?php esc_html_e('Paid', 'civi-framework') ?></span>
                                    <?php endif; ?>
                                </td>
                            </tr>
                        <?php endwhile; ?>
                    ...
                    <div class="civi-pagination dashboard" data-type="number">
                        ...
                        <div class="items-pagination" data-max-number="<?php echo esc_attr($total_invoices); ?>">
                            ...
                            <label class="text-pagination">
                                <?php echo sprintf(
                                    __('<span class="num-first">%d</span> - <span class="num-last">%d</span> of <span class="num-total">%d</span> items', 'civi-framework'),
                                    $num_first,
                                    $num_last,
                                    $total_invoices
                                ); ?>
            ...
            <?php endif; ?>
        <?php else : ?>
            <p class="item-not-found"><?php esc_html_e('No purchase history found.', 'civi-framework') ?></p>
        <?php endif;
        wp_reset_postdata();
        ?>
    </div>
    ...
    if (isset($total_invoices) && $total_invoices > 0) {
        wp_enqueue_script(CIVI_PLUGIN_PREFIX . 'user-package-pagination');
    }

    wp_enqueue_script(CIVI_PLUGIN_PREFIX . 'user-package-warning');
    wp_localize_script(CIVI_PLUGIN_PREFIX . 'user-package-warning', 'civi_package_vars', array(
        'ajax_url' => admin_url('admin-ajax.php'),
        'nonce' => wp_create_nonce('civi_employer_package_ajax_nonce'),
        'action' => 'civi_check_old_employer_package_before_activate'
    ));
    ?>
<?php endif; ?>
...
do_action('civi_dashboard_after_user_package');
?>
