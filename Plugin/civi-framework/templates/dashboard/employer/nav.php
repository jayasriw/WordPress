...
<div class="nav-dashboard-inner">
	...
	<div class="nav-dashboard-wapper custom-scrollbar">
		<div class="nav-dashboard nav-employer_dashboard">
			...
			<?php if (in_array('civi_user_employer', (array) $current_user->roles) || current_user_can('administrator')) : ?>
			...
			<?php if ($enable_post_your == 1) : ?>
				<div class="nav-dashboard-footer">
					<h4><?php esc_html_e('Post your first job!', 'civi-framework'); ?></h4>
					<p><?php esc_html_e('Your first 2 job postings for just $50 each.', 'civi-framework'); ?></p>
					<a href="<?php echo civi_get_permalink('jobs_submit'); ?>" class="civi-button"><i
							class="far fa-plus"></i><?php esc_html_e('Post a job', 'civi-framework'); ?></a>
				</div>
			<?php endif; ?>
		</div>
	</div>
