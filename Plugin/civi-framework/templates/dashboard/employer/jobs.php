...
if (!empty($jobs_id) && $pages == 'edit' && $verify_author) {
    ...
    $args_ex = apply_filters('civi_modify_author_query_args', $args_ex);
    ...
    $invoice_status = ($invoice_id > 0) ? get_post_meta($invoice_id, CIVI_METABOX_PREFIX . 'invoice_payment_status', true) : '';
?>
    <?php
    ...
    ?>
            <p class="notice">
                ...
                <?php esc_html_e("No package found. Please buy a package to continue.", 'civi-framework'); ?>
                <a href="<?php echo civi_get_permalink('package'); ?>">
                    <?php esc_html_e('Buy Package', 'civi-framework'); ?>
                </a>
            </p>
        <?php
        ...
        ?>
            <p class="notice">
                <i class="fal fa-exclamation-circle"></i>
                <?php esc_html_e("Your package is not activated yet. Please complete the payment to start using it.", 'civi-framework'); ?>
                <a href="<?php echo civi_get_permalink('package'); ?>">
                            ...
                            <tr>
                                ...
                                <td class="action-setting jobs-control">
                                    ...
                                    if ($check_package == -1 && $paid_submission_type == 'per_package') : ?>
                                        <a href="#" class="icon-setting btn-add-to-message" data-text="<?php echo esc_attr__('Your package has expired. Please buy a new one.', 'civi-framework'); ?>">
                                            <i class="fal fa-ellipsis-h"></i>
                                        </a>
                                    <?php elseif ($invoice_status == '0' && $paid_submission_type == 'per_package') : ?>
                                        <a href="#" class="icon-setting btn-add-to-message" data-text="<?php echo esc_attr__('Your package is not active. Please complete payment.', 'civi-framework'); ?>">
                                            <i class="fal fa-ellipsis-h"></i>
                                        </a>
                                                ...
                                                case 'publish':
                                                    if ($paid_submission_type == 'per_package') {
                                                        ...
                                                        if ($check_package != -1 && $check_package != 0) { ?>
                                                            <li><a class="btn-edit" href="<?php echo esc_url($jobs_dashboard_link); ?><?php echo strpos(esc_url($jobs_dashboard_link), '?') ? '&' : '?' ?>pages=edit&jobs_id=<?php echo esc_attr($id); ?>"><?php esc_html_e('Edit', 'civi-framework'); ?></a></li>
                                                        <?php }
                                    ...
                                    <?php else : ?>
                                        ...
                                        <?php else: ?>
                                            <a href="#" class="icon-setting btn-add-to-message" data-text="<?php echo esc_attr__('Jobs has expired so you can not change it', 'civi-framework'); ?>"><i class="fal fa-ellipsis-h"></i></a>
                                            </a>
                                        <?php endif; ?>
                                    <?php endif; ?>
                                </td>
