...
class Widget_Search_Vertical extends Widget_Base
{
	...
	{
		return [
			CIVI_PLUGIN_PREFIX . 'search-vertical',
			CIVI_PLUGIN_PREFIX . 'search-location',
	...
	private function add_layout_section()
	{
		...
		$this->add_control(
			'show_service',
			[
				'label'   => esc_html__('Show Service', 'civi-framework'),
				'type'    => Controls_Manager::SWITCHER,
				'default' => 'yes',
			]
		);
	...
	protected function render()
	{
		...
		if ($settings['show_service'] == 'yes') {
			...
			$search_placeholder_service = esc_attr__('Service title or keywords', 'civi-framework');
			$taxonomies_field_service   = array(
				esc_html__('Locations', 'civi-framework')  => "service-location",
...
?>
				...
				<ul class="tab-list">
					<?php if ($settings['show_jobs'] == 'yes') { ?>
						<li class="tab-item tab-jobs-item"><a
								href="#tab-jobs"><?php esc_html_e('For Jobs', 'civi-framework'); ?></a></li>
					<?php } ?>
					<?php if ($settings['show_company'] == 'yes') { ?>
						<li class="tab-item tab-company-item"><a
								href="#tab-company"><?php esc_html_e('For Companies', 'civi-framework'); ?></a>
						</li>
					<?php } ?>
					<?php if ($settings['show_candidate'] == 'yes') { ?>
						<li class="tab-item tab-candidate-item"><a
								href="#tab-candidate"><?php esc_html_e('For Candidates', 'civi-framework'); ?></a>
						</li>
					<?php } ?>
					<?php if ($settings['show_service'] == 'yes') { ?>
						<li class="tab-item tab-service-item"><a
								href="#tab-service"><?php esc_html_e('For Services', 'civi-framework'); ?></a>
						</li>
					<?php } ?>
				</ul>
	...
	private function print_content_form($post_type, array $settings, $taxonomy_key, $search_placeholder, $taxonomies_field)
	{
		...
		$link_redirect = apply_filters('civi_search_vertical_redirect_url', $link_redirect, $settings, $this);
	?>
		<form action="<?php echo esc_url($link_redirect); ?>" method="get" class="form-search-vertical">
			<div class="search-vertical-inner">
				...
				<div class="form-group">
					<input class="search-vertical-<?php echo $post_type; ?> civi-ajax-ui" data-key='<?php echo $post_type_keyword ?>'
				...
				?>
					<div class="form-group civi-form-location">
						<?php if ($term_count <= 150): ?>
							<input class="input-search-location civi-ajax-ui" type="text" name="<?php echo $field ?>"
								placeholder="<?php esc_attr_e('All Cities', 'civi-framework') ?>" autocomplete="off" data-taxonomy="<?php echo esc_attr($taxonomy); ?>"
								value="<?php
								if (isset($_GET[$field])) {
								...
								?>">
							<select class="civi-select2 hide">
								<option value=""><?php esc_html_e('All Cities', 'civi-framework') ?></option>
						...
						<?php else: ?>
							<input class="input-search-location civi-ajax-ui" type="text" name="<?php echo $field ?>"
								...
								?>">
							<select class="civi-ajax-select2 hide" data-taxonomy="<?php echo esc_attr($taxonomy); ?>" data-placeholder="<?php esc_attr_e('Select location', 'civi-framework'); ?>">
									<?php
						...
						?>
							<div class="form-group">
								...
								<?php else: ?>
									<select name="<?php echo $field ?>" class="civi-ajax-select2" data-taxonomy="<?php echo esc_attr($taxonomy); ?>" data-placeholder="<?php echo sprintf(esc_attr__('Select %s', 'civi-framework'), $label_field); ?>">
										<?php
