							...
							if (!empty($category_slug)) {
								...
								if ($cate && !is_wp_error($cate) && !empty($cate->term_id)) {
									...
									if (function_exists('civi_get_term_post_count_for_sidebar')) {
										$term_count = civi_get_term_post_count_for_sidebar($cate->term_id, 'jobs-categories');
									} else {
										...
										<?php if ($has_icon && $settings['show_icon'] == 'yes') : ?>
											...
											<div class="cate-content">
												...
												<?php if ($settings['show_count'] == 'yes' && isset($term_count)) : ?>
													<p class="cate-count"><?php echo sprintf( _n( '%s job', '%s jobs', $term_count, 'civi-framework' ), number_format_i18n( $term_count ) ); ?></p>
												<?php endif; ?>
												<?php
