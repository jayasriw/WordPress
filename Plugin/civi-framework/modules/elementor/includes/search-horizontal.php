...
class Widget_Search_Horizontal extends Widget_Base
{
    ...
    private function add_layout_section()
    {
        ...
        $this->add_control(
            'choose_options',
            [
                'label' => esc_html__('Choose Options', 'civi-framework'),
    ...
    protected function render()
    {
        ...
        $link_redirect = apply_filters('civi_search_horizontal_redirect_url', $link_redirect, $settings, $this);
?>
        <div <?php echo $this->get_render_attribute_string('wrapper') ?>>
            <form action="<?php echo esc_url($link_redirect); ?>" method="get" class="form-search-horizontal">
                <div class="search-horizontal-inner">
                    ...
                    $id = apply_filters('civi/search-control/id', 'search-horizontal_filter_search');
                    ?>
                    <div class="form-group">
                        <input class="search-horizontal-control civi-ajax-ui"
                    ...
                    <?php foreach ($taxonomies_field as $label_field => $field) {
                        if ($settings['show_' . $field]) {
                            if ($field == 'jobs-location' || $field == 'company-location' || $field == 'candidate_locations' || $field == 'service-location') {
                                ...
                                $custom_location_field = apply_filters('civi_search_horizontal_location_field', '', $field, $settings);
                    ...
                    ?>
                                    <div class="form-group civi-form-location">
                                        ...
                                        <?php if ($term_count <= 150): ?>
                                            <select class="civi-select2" data-placeholder="<?php esc_attr_e('All Cities', 'civi-framework') ?>">
                                                <option value=""><?php esc_html_e('All Cities', 'civi-framework') ?></option>
                                                <?php civi_get_taxonomy($field, true, false); ?>
                                            </select>
                                        <?php else: ?>
                                            <select class="civi-ajax-select2" data-taxonomy="<?php echo esc_attr($taxonomy); ?>" data-placeholder="<?php esc_attr_e('Select location', 'civi-framework'); ?>">
                                                <?php
                                                ...
                                                if (!empty($selected_location)) {
                                                    ...
                                                    foreach ($location_tokens as $token) {
                                                        ...
                                                        $term = \Civi_Location_Search::find_location_term($token, $taxonomy);
                                ...
                                <div class="form-group">
                                    ...
                                    <?php if ($term_count <= 150): ?>
                                        <select name="<?php echo $field ?>" class="civi-select2" data-placeholder="<?php echo sprintf(esc_attr__('All %s', 'civi-framework'), $label_field); ?>">
