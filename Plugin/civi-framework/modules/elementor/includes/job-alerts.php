...
class Widget_Job_Alerts extends Widget_Base
{
	...
	private function add_form_section()
	{
		$this->start_controls_section('form_section', [
			'label' => esc_html__('Form', 'civi-framework'),
		]);

		$this->add_control('heading', [
			'label'       => esc_html__('Heading', 'civi-framework'),
			'type'        => Controls_Manager::TEXT,
			'placeholder' => esc_html__('Enter form heading', 'civi-framework'),
			'default'     => esc_html__('Create Job Alert', 'civi-framework'),
		]);
		...
		$this->add_control(
			'show_phone',
			[
				'label' => esc_html__('Show Phone', 'civi-framework'),
				'type' => Controls_Manager::SWITCHER,
				'default' => 'yes',
			]
		);
		...
		$this->add_control('submit_text', [
			'label'       => esc_html__('Submit Text', 'civi-framework'),
			'type'        => Controls_Manager::TEXT,
			'placeholder' => esc_html__('Enter Submit Text', 'civi-framework'),
			'default'     => esc_html__('Create job alert', 'civi-framework'),
		]);
	...
	private function add_form_style_section()
	{
		$this->start_controls_section('form_style_section', [
			'label'     => esc_html__('Form Style', 'civi-framework'),
	...
	protected function render()
	{
		...
		if (!empty($email) && !empty($frequency)) {
			...
			$existing_alert = get_posts(array(
				'post_type' => $post_type,
				'meta_key' => CIVI_METABOX_PREFIX . 'job_alerts_email',
				'meta_value' => $email,
				'posts_per_page' => 1
			));
			...
			if (!empty($existing_alert)) {
				echo '<div class="job-alerts-notice error">
					<div class="notice error"><i class="fal fa-times"></i>' . esc_html__('This email is already subscribed to job alerts', 'civi-framework') . '</div>
				</div>';
			} else {
				...
				if ($post_id) {
					...
					update_post_meta($post_id, CIVI_METABOX_PREFIX . 'job_alerts_phone', $phone);
					...
					echo '<div class="job-alerts-notice">
						<div class="notice"><i class="fal fa-check"></i>' . esc_html__('You have successfully subscribed', 'civi-framework') . '</div>
						</div>';
				}
			}
		}
?>
		...
		?>
			<div class="job-alerts-notice">
				<div class="notice"><i class="fal fa-check"></i><?php echo esc_html__('You have successfully unsubscribed', 'civi-framework'); ?></div>
			</div>
		<?php
		}
		?>
		<div class="job-alerts-wrapper">
			...
			<form action="" method="GET" class="job-alerts-form">
				<div class="field-input">
					...
					<input type="text" id="email" name="email" required placeholder="<?php esc_html_e('Your email', 'civi-framework'); ?>" value="<?php echo $mail; ?>" required>
				</div>
				<?php if ($settings['show_phone']) : ?>
					<div class="field-input">
						<label for="tel"><?php esc_html_e('Your phone ', 'civi-framework'); ?></label>
						<input type="tel" id="tel" name="phone" placeholder="<?php esc_html_e('Your phone', 'civi-framework'); ?>" value="">
					</div>
				<?php endif; ?>
				<?php if ($settings['show_job_category']) : ?>
					<div class="field-select">
						...
						<div class="form-select">
							...
							<?php else: ?>
								<select name="category" id="category" class="civi-ajax-select2" data-taxonomy="<?php echo esc_attr($taxonomy); ?>" data-placeholder="<?php esc_attr_e('Select categories', 'civi-framework'); ?>" multiple="multiple">
								</select>
							<?php endif; ?>
						</div>
					</div>
				...
				<?php if ($settings['show_location']) : ?>
					<?php
					$custom_location_field = apply_filters('civi_job_alerts_location_field', '', $settings, $_GET);
					...
					if (!empty($custom_location_field)) {
						...
						<div class="field-select">
							...
							<div class="form-select">
								...
								<?php else: ?>
									<select name="location" id="location" class="civi-ajax-select2" data-taxonomy="<?php echo esc_attr($taxonomy); ?>" data-placeholder="<?php esc_attr_e('Select location', 'civi-framework'); ?>">
										<?php
				...
				<?php if ($settings['show_job_experience']) : ?>
					<div class="field-select">
						...
						<div class="form-select">
							...
							<?php else: ?>
								<select name="experience" id="experience" class="civi-ajax-select2" data-taxonomy="<?php echo esc_attr($taxonomy); ?>" data-placeholder="<?php esc_attr_e('Select experience', 'civi-framework'); ?>">
									<?php
				...
				<?php if ($settings['show_job_skills']) : ?>
					<div class="field-input">
						...
						<div class="form-select">
							...
							<?php else: ?>
								<select data-placeholder="<?php esc_attr_e('Select skills', 'civi-framework'); ?>" multiple="multiple" class="civi-ajax-select2" name="skills" data-taxonomy="<?php echo esc_attr($taxonomy); ?>">
									<?php
				...
				<?php if ($settings['show_job_type']) : ?>
					<div class="field-select">
						...
						<div class="form-select">
							...
							<?php else: ?>
								<select data-placeholder="<?php esc_attr_e('Select job types', 'civi-framework'); ?>" multiple="multiple" class="civi-ajax-select2" name="types" id="type" data-taxonomy="<?php echo esc_attr($taxonomy); ?>">
									<?php
