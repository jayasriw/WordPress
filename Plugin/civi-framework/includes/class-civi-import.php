...
class Civi_Demo_Importer
{
    ...
    public function __construct()
    {
        ...
        add_action('civi_freelancer_basic_cleanup', [$this, 'run_freelancer_basic_cleanup']);
        add_action('civi_freelancer_full_cleanup', [$this, 'run_freelancer_full_cleanup']);
        add_action('civi_standard_cleanup', [$this, 'run_standard_cleanup']);
        ...
        if (!empty($selected_import['theme_options_file'])) {
            ...
            if (file_exists($file)) {
                ...
                if (is_array($data)) {
                    ...
                    $existing_options = get_option(CIVI_OPTIONS_NAME, array());
                    $merged_options = array_merge($existing_options, $cleaned_data);
                    update_option(CIVI_OPTIONS_NAME, $merged_options);
                }
            }
        }
    }
    ...
    private function clean_theme_options_urls($data, $selected_import)
    {
        ...
        $old_domain = untrailingslashit($this->civi_get_old_domain($selected_import));

        $cleanup_patterns = [];

        if ($selected_import['import_file_name'] === 'Civi Freelancer') {
            $cleanup_patterns = [
     ...
     * Currently supports:
     * - Civi Jobs Board (main demo)
     * - Civi Freelancer (alternative demo)
     *
     * @since 1.0.0
    ...
    public function getImportFiles(): array
    {
        $import_files = array(
            array(
                'import_file_name'              => 'Civi Jobs Board',
                ...
                'import_notice'                 => __('After importing this demo, please go to Settings > Permalinks and click "Save Changes" to update your permalink structure.', 'civi-framework'),
                'preview_url'                   => 'https://civi.uxper.co/',
                'theme_options_file'           => plugin_dir_path(__DIR__) . 'assets/import/01/theme-options.json',
            ),
            array(
                'import_file_name'              => 'Civi Freelancer',
                ...
                <div class="civi-box__body">
                    <ol>
                        <li>
                            ...
                            <a href="http://civi.local/wp-admin/plugin-install.php?tab=plugin-information&amp;plugin=wordpress-reset&amp;TB_iframe=true&amp;width=800&amp;height=550" class="thickbox" title="Install WordPress Reset">WordPress Reset</a> plugin.
                        </li>
    ...
    private function setElementorSettings($selected_import)
    {
        ...
        if (!empty($selected_import['import_file_name'])) {
            ...
            if ($import_file_name == 'Civi Jobs Board') {
                $elementor_settings = plugin_dir_path(__DIR__) . 'assets/import/01/elementor.json';
            } elseif ($import_file_name == 'Civi Freelancer') {
    ...
    private function setPages($selected_import)
    {
        ...
        $pages = [
            ...
            'civi_update_profile_page_id'       => ($page = $this->get_page_by_title_safe('Profile')) ? $page->ID : 0,
            'civi_add_jobs_page_id'             => ($page = $this->get_page_by_title_safe('Post Job')) ? $page->ID : 0,
            'civi_add_jobs_not_page_id'         => ($page = $this->get_page_by_title_safe('Employer Landing')) ? $page->ID : 0,

            // Employer Pages
            'civi_dashboard_page_id'                 => ($page = $this->get_page_by_title_safe('Dashboard Employers')) ? $page->ID : 0,
            'civi_jobs_dashboard_page_id'            => ($page = $this->get_page_by_title_safe('Job Employer')) ? $page->ID : 0,
            'civi_jobs_submit_page_id'               => ($page = $this->get_page_by_title_safe('Post Job')) ? $page->ID : 0,
            'civi_applicants_page_id'                => ($page = $this->get_page_by_title_safe('Applicants')) ? $page->ID : 0,
            'civi_candidates_page_id'                => ($page = $this->get_page_by_title_safe('Candidate')) ? $page->ID : 0,
            'civi_user_package_page_id'              => ($page = $this->get_page_by_title_safe('User Package')) ? $page->ID : 0,
            'civi_company_page_id'                   => ($page = $this->get_page_by_title_safe('Company Employer')) ? $page->ID : 0,
            'civi_submit_company_page_id'            => ($page = $this->get_page_by_title_safe('Submit Company')) ? $page->ID : 0,
            'civi_messages_page_id'                  => ($page = $this->get_page_by_title_safe('Messages')) ? $page->ID : 0,
            'civi_meetings_page_id'                  => ($page = $this->get_page_by_title_safe('Meetings Dashboard')) ? $page->ID : 0,
            'civi_settings_page_id'                  => ($page = $this->get_page_by_title_safe('Settings')) ? $page->ID : 0,
            'civi_package_page_id'                   => ($page = $this->get_page_by_title_safe('Packages')) ? $page->ID : 0,
            'civi_payment_page_id'                   => ($page = $this->get_page_by_title_safe('Payment')) ? $page->ID : 0,
            'civi_payment_completed_page_id'         => ($page = $this->get_page_by_title_safe('Payment Completed')) ? $page->ID : 0,

            // Candidate Pages
            'civi_candidate_dashboard_page_id'           => ($page = $this->get_page_by_title_safe('Dashboard Candidates')) ? $page->ID : 0,
            'civi_candidate_profile_page_id'             => ($page = $this->get_page_by_title_safe('Profile')) ? $page->ID : 0,
            'civi_my_jobs_page_id'                       => ($page = $this->get_page_by_title_safe('My Jobs')) ? $page->ID : 0,
            'civi_candidate_user_package_page_id'        => ($page = $this->get_page_by_title_safe('Candidate User Package')) ? $page->ID : 0,
            'civi_candidate_reviews_page_id'             => ($page = $this->get_page_by_title_safe('My Review')) ? $page->ID : 0,
            'civi_candidate_company_page_id'             => ($page = $this->get_page_by_title_safe('Company Candidate')) ? $page->ID : 0,
            'civi_candidate_messages_page_id'            => ($page = $this->get_page_by_title_safe('Message')) ? $page->ID : 0,
            'civi_candidate_meetings_page_id'            => ($page = $this->get_page_by_title_safe('Candidate Meetings')) ? $page->ID : 0,
            'civi_candidate_settings_page_id'            => ($page = $this->get_page_by_title_safe('Setting Candidate')) ? $page->ID : 0,
            'civi_candidate_package_page_id'             => ($page = $this->get_page_by_title_safe('Candidate Package')) ? $page->ID : 0,
            'civi_candidate_payment_page_id'             => ($page = $this->get_page_by_title_safe('Candidate Payment')) ? $page->ID : 0,
            'civi_candidate_payment_completed_page_id'   => ($page = $this->get_page_by_title_safe('Candidate Payment Completed')) ? $page->ID : 0,
        ];

        update_option(CIVI_OPTIONS_NAME, $pages);
    }
    ...
    private function civi_replace_demo_domain($selected_import)
    {
        $new_domain = untrailingslashit(home_url());
        $old_domain = untrailingslashit($this->civi_get_old_domain($selected_import));
     ...
     * Usage: Civi_Demo_Importer::manual_cleanup_freelancer_paths();
     */
     ...
     * Usage: Civi_Demo_Importer::test_theme_options_cleaning();
     */
    public static function test_theme_options_cleaning()
    {
        ...
        $test_data = [
            'jobs_search_image' => [
                'url' => 'https://civi.uxper.co/freelance/wp-content/uploads/sites/2/2023/04/Banner-de-LinkedIn-Trabajo-Sencillo.png'
            ],
            'image_employer_dashboard' => [
                'url' => 'https://civi.uxper.co/freelance/wp-content/uploads/sites/2/2022/10/dashboard.svg'
            ]
        ];
     ...
     * Usage: Civi_Demo_Importer::create_missing_menus();
     */
     ...
     * Usage: Civi_Demo_Importer::check_menu_status();
     */
    ...
    private function update_all_jobs_expiration()
    {
        ...
        if (!defined('CIVI_METABOX_PREFIX')) {
            define('CIVI_METABOX_PREFIX', 'civi_');
        }
    ...
    private function fix_company_logos()
    {
        ...
        foreach ($companies as $company_id) {
            $logo_meta = get_post_meta($company_id, CIVI_METABOX_PREFIX . 'company_logo', true);
            ...
            if ($new_attachment_id && $new_attachment_id != $old_id) {
                ...
                update_post_meta($company_id, CIVI_METABOX_PREFIX . 'company_logo', $updated_logo_meta);
                $fixed_count++;
            }
        }
        return $fixed_count;
    ...
    private function fix_candidate_avatars()
    {
        ...
        foreach ($candidates as $candidate_id) {
            $avatar_meta = get_post_meta($candidate_id, CIVI_METABOX_PREFIX . 'candidate_avatar', true);
            ...
            if ($new_attachment_id && $new_attachment_id != $old_id) {
                ...
                update_post_meta($candidate_id, CIVI_METABOX_PREFIX . 'candidate_avatar', $updated_avatar_meta);
                $fixed_count++;
            }
        }
    ...
    public function civi_get_old_domain($selected_import)
    {
        $import_file_name = isset($selected_import['import_file_name']) ? $selected_import['import_file_name'] : '';

        switch ($import_file_name) {
            case 'Civi Jobs Board':
                return 'https://civi.uxper.co';
            case 'Civi Freelancer':
                return 'https://civi.uxper.co/freelance';
            default:
                return 'https://civi.uxper.co';
        }
    }
    ...
    public function ensure_service_enabled_on_import()
    {
        ...
        if (isset($_GET['page']) && $_GET['page'] === 'one-click-demo-import') {
            $current_options = get_option(CIVI_OPTIONS_NAME, array());
            ...
            if (!isset($current_options['enable_post_type_service']) || $current_options['enable_post_type_service'] !== '1') {
                ...
                update_option(CIVI_OPTIONS_NAME, $current_options);
            }
        }
    }
    ...
    public function afterImportSetup(array $selected_import): void
    {
        try {
            ...
            error_log("Civi Import Error in afterImportSetup: " . $e->getMessage());
        }
    }
    ...
    private function setup_basic_configuration($selected_import)
    {
        try {
            ...
            error_log('Civi Import Error in setup_basic_configuration: ' . $e->getMessage());
        }
    }
    ...
    {
        wp_schedule_single_event(time() + self::CLEANUP_DELAY_BASIC, 'civi_freelancer_basic_cleanup');
        wp_schedule_single_event(time() + self::CLEANUP_DELAY_FULL, 'civi_freelancer_full_cleanup');
        if (!has_action('civi_freelancer_basic_cleanup', [$this, 'run_freelancer_basic_cleanup'])) {
            add_action('civi_freelancer_basic_cleanup', [$this, 'run_freelancer_basic_cleanup']);
        }
        if (!has_action('civi_freelancer_full_cleanup', [$this, 'run_freelancer_full_cleanup'])) {
            add_action('civi_freelancer_full_cleanup', [$this, 'run_freelancer_full_cleanup']);
        }
    }
    ...
    {
        wp_schedule_single_event(time() + self::CLEANUP_DELAY_STANDARD, 'civi_standard_cleanup', [$selected_import]);
        if (!has_action('civi_standard_cleanup', [$this, 'run_standard_cleanup'])) {
            add_action('civi_standard_cleanup', [$this, 'run_standard_cleanup']);
        }
    }
    ...
    public function run_freelancer_basic_cleanup(): void
    {
        try {
            ...
            // Domain replacement
            $this->civi_replace_demo_domain($selected_import);

            // Basic jobs update
            $this->update_all_jobs_expiration();
        } catch (Exception $e) {
            error_log('Civi Import Error in run_freelancer_basic_cleanup: ' . $e->getMessage());
        }
    }
    ...
    public function run_freelancer_full_cleanup(): void
    {
        try {
            ...
            error_log('Civi Import Error in run_freelancer_full_cleanup: ' . $e->getMessage());
        }
    }
    ...
    public function run_standard_cleanup(array $selected_import): void
    {
        try {
            ...
            error_log('Civi Import Error in run_standard_cleanup: ' . $e->getMessage());
        }
    }
    ...
    public function prepare_media_import()
    {
        ...
        if (function_exists('wp_cache_delete')) {
            wp_cache_delete('upload_errors', 'civi_import');
        }
    }
    ...
    public function after_content_import($selected_import)
    {
        ...
        if ($pages_after == 0) {
            error_log("Civi Import: WARNING - No pages were imported! Check XML file and import process.");
        }
    }
    ...
    private function log_upload_error($file)
    {
        ...
        $upload_errors = wp_cache_get('upload_errors', 'civi_import') ?: [];
        $upload_errors[] = [
            'file' => $file['name'],
            'error' => $error_message,
            'time' => current_time('mysql')
        ];
        wp_cache_set('upload_errors', $upload_errors, 'civi_import', 3600);
    }
    ...
    private function create_placeholder_image($file)
    {
        try {
            ...
            error_log("Civi Import: Failed to create placeholder: " . $e->getMessage());
        }
    ...
    private function retry_single_media_import($filename)
    {
        try {
            ...
            error_log("Civi Import: Error retrying media import for {$filename}: " . $e->getMessage());
            return false;
        }
    }
