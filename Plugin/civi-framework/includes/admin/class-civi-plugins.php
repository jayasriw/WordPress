    ...
    class Civi_Plugins
    {
        ...
        public static function get_plugin_action($plugin)
        {
            ...
            if (!isset($installed_plugins[$plugin['file_path']])) {
                ...
                $actions = sprintf(
                    __('<a href="%1$s" title="Install %2$s">Install</a>', 'civi-framework'),
                    esc_url(
                        wp_nonce_url(
                            add_query_arg(
                                array(
                ...
                // Display update link.
                $actions = sprintf(
                    __('<a href="%1$s" title="Update %2$s">Update</a>', 'civi-framework'),
                    wp_nonce_url(
                        add_query_arg(
                            array(
                                'page'         => rawurlencode(TGM_Plugin_Activation::$instance->menu),
                ...
                // Display Active link.
                $actions = sprintf(
                    __('<a href="%1$s" title="Activate %2$s" data-slug="%3$s" data-source="%4$s" data-plugin-action="activate-plugin" data-nonce="%5$s" class="civi-plugin-action plugin-activate">Activate</a>', 'civi-framework'),
                    '#',
                    $plugin['name'],
                    $plugin['slug'],
                    $plugin['file_path'],
                ...
                $actions = sprintf(
                    __('<a href="%1$s" title="Deactivate %2$s" data-slug="%3$s" data-source="%4$s" data-plugin-action="deactivate-plugin" data-nonce="%5$s" class="civi-plugin-action plugin-deactivate">Deactivate</a>', 'civi-framework'),
                    '#',
                    $plugin['name'],
                    $plugin['slug'],
                    $plugin['file_path'],
        ...
        public function process_plugin_actions()
        {
            if (! current_user_can('activate_plugins')) {
                wp_send_json_error(esc_html__('Permission denied.', 'civi-framework'), 403);
            }

            if (empty($_POST['_wpnonce']) || ! wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['_wpnonce'])), 'process_plugin_actions_nonce')) {
                wp_send_json_error(esc_html__('Invalid nonce.', 'civi-framework'), 400);
            }
            ...
            if (!class_exists('TGM_Plugin_Activation')) {
                wp_send_json_error(esc_html__('TGM_Plugin_Activation does not exist', 'civi-framework'));
            }
            ...
            if (empty($source)) {
                wp_send_json_error(esc_html__('Installation package not available.', 'civi-framework'));
            }
            ...
            if (in_array($plugin_action, ['activate-plugin', 'deactivate-plugin'], true)) {
                if ('activate-plugin' === $plugin_action) {
                    ...
                    wp_send_json_success(array('nonce' => $nonce, 'message' => __('Plugin activated.', 'civi-framework')));
                }

                if ('deactivate-plugin' === $plugin_action) {
                    deactivate_plugins($source);
                    $nonce = wp_create_nonce('process_plugin_actions_nonce');
                    wp_send_json_success(array('nonce' => $nonce, 'message' => __('Plugin deactivated.', 'civi-framework')));
                }
            } else {
                wp_send_json_error(__('Invalid plugin action.', 'civi-framework'));
            }

            wp_send_json_error(esc_html__('Invalid action.', 'civi-framework'), 400);
        }
    }
}
