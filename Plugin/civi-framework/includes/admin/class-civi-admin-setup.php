...
require_once CIVI_PLUGIN_DIR . 'includes/admin/class-civi-exporter.php';
if (!class_exists('Civi_Admin_Setup')) {
	...
	class Civi_Admin_Setup
	{
		...
		public function admin_menu()
		{
			...
			if (civi_get_option('enable_post_type_service') === '1') {
				add_submenu_page(
					'civi_employer',
					esc_html__('Service Order', 'civi-framework'),
					esc_html__('Service Order', 'civi-framework'),
					'manage_options',
				...
				add_submenu_page(
					'civi_employer',
					esc_html__('Service Withdraw', 'civi-framework'),
					esc_html__('Service Withdraw', 'civi-framework'),
					'manage_options',
			...
			add_submenu_page(
				'civi_candidate',
				esc_html__('Candidates', 'civi-framework'),
				esc_html__('Candidates', 'civi-framework'),
				'manage_options',
			...
			if (civi_get_option('enable_post_type_service') === '1') {
				add_submenu_page(
					'civi_candidate',
					esc_html__('Service', 'civi-framework'),
					esc_html__('Service', 'civi-framework'),
					'manage_options',
			...
			add_submenu_page(
				'civi_candidate',
				esc_html__('Order', 'civi-framework'),
				esc_html__('Order', 'civi-framework'),
				'manage_options',
			...
			add_submenu_page(
				'civi_extensions',
				esc_html__('Package Coupon', 'civi-framework'),
				esc_html__('Package Coupon', 'civi-framework'),
				'manage_options',
			...
			add_submenu_page(
				'civi_builder',
				esc_html__('Header', 'civi-framework'),
				esc_html__('Header', 'civi-framework'),
				'manage_options',
				'edit.php?post_type=civi_header'
			);
	...
	public function reorder_admin_menu()
	{
		...
		$this->safe_add_menu_page(esc_html__('Posts', 'civi-framework'), esc_html__('Posts', 'civi-framework'), 'edit_posts', 'edit.php', '', 'dashicons-admin-post', 26);
		$this->safe_add_menu_page(esc_html__('Media', 'civi-framework'), esc_html__('Media', 'civi-framework'), 'manage_options', 'upload.php', '', 'dashicons-admin-media', 27);
		$this->safe_add_menu_page(esc_html__('Pages', 'civi-framework'), esc_html__('Pages', 'civi-framework'), 'edit_pages', 'edit.php?post_type=page', '', 'dashicons-admin-page', 28);
		$this->safe_add_menu_page(esc_html__('Appearance', 'civi-framework'), esc_html__('Appearance', 'civi-framework'), 'edit_theme_options', 'themes.php', '', 'dashicons-admin-appearance', 30);
		$this->safe_add_menu_page(esc_html__('Plugins', 'civi-framework'), esc_html__('Plugins', 'civi-framework'), 'activate_plugins', 'plugins.php', '', 'dashicons-admin-plugins', 31);
		$this->safe_add_menu_page(esc_html__('Users', 'civi-framework'), esc_html__('Users', 'civi-framework'), 'promote_users', 'users.php', '', 'dashicons-admin-users', 32);
		$this->safe_add_menu_page(esc_html__('Tools', 'civi-framework'), esc_html__('Tools', 'civi-framework'), 'manage_options', 'tools.php', '', 'dashicons-admin-tools', 33);
	}
				...
				<div class="wrap-column wrap-column-2 col-started">
					<div class="panel-column column-content">
						...
						<div class="entry-detail">
							...
							<a href="<?php echo esc_url(admin_url('admin.php?page=one-click-demo-import')); ?>" class="button button-primary"><?php esc_html_e('Install Sample Data', 'civi-framework'); ?></a>
		...
		private function create_page($title, $content, $option)
		{
			...
			if ($option) {
				if (function_exists('pll_the_languages')) {
					$config = get_option(pll_current_language() . '_civi-framework');
					$config[$option] = $page_id;
					update_option(pll_current_language() . '_civi-framework', $config);
				} else if (defined('ICL_SITEPRESS_VERSION')) {
					$current_language = apply_filters('wpml_current_language', NULL);
					if ($current_language) {
						$config = get_option($current_language . '_civi-framework');
						$config[$option] = $page_id;
						update_option($current_language . '_civi-framework', $config);
					} else {
						$config = get_option('civi-framework');
						$config[$option] = $page_id;
						update_option('civi-framework', $config);
					}
				} else {
					$config = get_option('civi-framework');
					$config[$option] = $page_id;
		...
		public function setup_page()
		{
			...
			if (3 === $step && !empty($_POST)) {
				...
				$pages_to_create = array(
					'dashboard' => '[civi_dashboard]',
					'candidate_dashboard' => '[civi_candidates]',
					...
					'payment_service' => '[civi_service_payment]',
					'service_payment_completed' => '[civi_service_payment_completed]',
					...
					'candidate_user_package' => '[civi_candidate_user_package]',
					'employer_service' => '[civi_employer_service]',
					'candidate_service' => '[civi_candidate_service]',
					'submit_service' => '[civi_submit_service]',
					'candidate_package' => '[civi_candidate_package]',
					'candidate_payment' => '[civi_candidate_payment]',
					'candidate_payment_completed' => '[civi_candidate_payment_completed]',
				);
					...
					<form action="<?php echo esc_url(add_query_arg('step', 3)); ?>" method="post">
						<table class="civi-shortcodes widefat">
							...
							<tbody>
								...
								<tr>
									<td><input type="checkbox" checked="checked" name="civi-create-page[candidate_user_package]" /></td>
									<td><input type="text" value="<?php echo esc_attr(_x('Candidate User Package', 'Default page title (wizard)', 'civi-framework')); ?>" name="civi-page-title[candidate_user_package]" /></td>
									<td>
										<p><?php esc_html_e('This page allows users to view their own "Candidate User Package" via the front-end.', 'civi-framework'); ?></p>
									</td>
									<td><code>[civi_candidate_user_package]</code></td>
								</tr>
								<tr>
									<td><input type="checkbox" checked="checked" name="civi-create-page[candidate_package]" /></td>
									<td><input type="text" value="<?php echo esc_attr(_x('Candidate Package', 'Default page title (wizard)', 'civi-framework')); ?>" name="civi-page-title[candidate_package]" /></td>
									<td>
										<p><?php esc_html_e('This page allows users to view their own "Candidate Package" via the front-end.', 'civi-framework'); ?></p>
									</td>
									<td><code>[civi_candidate_package]</code></td>
								</tr>
								<tr>
									<td><input type="checkbox" checked="checked" name="civi-create-page[candidate_payment]" /></td>
									<td><input type="text" value="<?php echo esc_attr(_x('Candidate Payment', 'Default page title (wizard)', 'civi-framework')); ?>" name="civi-page-title[candidate_payment]" /></td>
									<td>
										<p><?php esc_html_e('This page allows users to view their own "Candidate Payment" via the front-end.', 'civi-framework'); ?></p>
									</td>
									<td><code>[civi_candidate_payment]</code></td>
								</tr>
								<tr>
									<td><input type="checkbox" checked="checked" name="civi-create-page[candidate_payment_completed]" /></td>
									<td><input type="text" value="<?php echo esc_attr(_x('Candidate Payment Completed', 'Default page title (wizard)', 'civi-framework')); ?>" name="civi-page-title[candidate_payment_completed]" /></td>
									<td>
										<p><?php esc_html_e('This page allows users to view their own "Candidate Payment Completed" via the front-end.', 'civi-framework'); ?></p>
									</td>
									<td><code>[civi_candidate_payment_completed]</code></td>
								</tr>
								<tr>
									<td><input type="checkbox" checked="checked" name="civi-create-page[employer_service]" /></td>
									<td><input type="text" value="<?php echo esc_attr(_x('Employer Service', 'Default page title (wizard)', 'civi-framework')); ?>" name="civi-page-title[employer_service]" /></td>
									<td>
										<p><?php esc_html_e('This page allows users to view their own "Employer Service" via the front-end.', 'civi-framework'); ?></p>
									</td>
									<td><code>[civi_employer_service]</code></td>
								</tr>
								<tr>
									<td><input type="checkbox" checked="checked" name="civi-create-page[candidate_service]" /></td>
									<td><input type="text" value="<?php echo esc_attr(_x('Candidate Service', 'Default page title (wizard)', 'civi-framework')); ?>" name="civi-page-title[candidate_service]" /></td>
									<td>
										<p><?php esc_html_e('This page allows users to view their own "Candidate Service" via the front-end.', 'civi-framework'); ?></p>
									</td>
									<td><code>[civi_candidate_service]</code></td>
								</tr>
								<tr>
									<td><input type="checkbox" checked="checked" name="civi-create-page[submit_service]" /></td>
									<td><input type="text" value="<?php echo esc_attr(_x('Service Submit', 'Default page title (wizard)', 'civi-framework')); ?>" name="civi-page-title[submit_service]" /></td>
									<td>
										<p><?php esc_html_e('This page allows users to view their own "Service Submit" via the front-end.', 'civi-framework'); ?></p>
									</td>
									<td><code>[civi_submit_service]</code></td>
								</tr>
								<tr>
									<td><input type="checkbox" checked="checked" name="civi-create-page[payment_service]" /></td>
									<td><input type="text" value="<?php echo esc_attr(_x('Service Payment', 'Default page title (wizard)', 'civi-framework')); ?>" name="civi-page-title[payment_service]" /></td>
									<td>
										<p><?php esc_html_e('This page allows users to view their own "Service Payment" via the front-end.', 'civi-framework'); ?></p>
									</td>
									<td><code>[civi_service_payment]</code></td>
								</tr>
								<tr>
									<td><input type="checkbox" checked="checked" name="civi-create-page[service_payment_completed]" /></td>
									<td><input type="text" value="<?php echo esc_attr(_x('Service Payment Completed', 'Default page title (wizard)', 'civi-framework')); ?>" name="civi-page-title[service_payment_completed]" /></td>
									<td>
										<p><?php esc_html_e('This page allows users to view their own "Service Payment Completed" via the front-end.', 'civi-framework'); ?></p>
									</td>
									<td><code>[civi_service_payment_completed]</code></td>
								</tr>
							</tbody>
							<tfoot>
								<tr>
