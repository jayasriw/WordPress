    ...
    class Civi_Admin_Jobs
    {
        ...
        private function format_salary_rate($rate)
        {
            ...
            if (isset(self::$salary_rate_labels[$rate])) {
                return esc_html__(self::$salary_rate_labels[$rate], 'civi-framework');
            }
        ...
        private function get_status_label($status)
        {
            if (isset(self::$status_labels[$status])) {
                return esc_html__(self::$status_labels[$status], 'civi-framework');
            }
        ...
        private function display_thumb_column($post_id)
        {
            ...
            if (!empty($logo_url)) {
                ...
                $placeholder_url = CIVI_PLUGIN_URL . 'assets/images/job-img.webp';
                $style = sprintf('max-width: %dpx; height: auto; border-radius: 50%%; border: 3px solid #eee;', self::LOGO_MAX_WIDTH);
                echo '<img src="' . esc_url($placeholder_url) . '" alt="' . esc_attr__('No image', 'civi-framework') . '" style="' . esc_attr($style) . '"/>';
            }
        }
        ...
        {
            $salary_show = get_post_meta($post_id, CIVI_METABOX_PREFIX . 'jobs_salary_show', true);
            ...
            $currency = get_post_meta($post_id, CIVI_METABOX_PREFIX . 'jobs_currency_type', true) ?: '';
            $rate = get_post_meta($post_id, CIVI_METABOX_PREFIX . 'jobs_salary_rate', true);
            ...
            switch ($salary_show) {
                ...
                case 'agree':
                    echo esc_html__('Negotiable', 'civi-framework');
                    break;
        ...
        {
            $minimum = get_post_meta($post_id, CIVI_METABOX_PREFIX . 'jobs_salary_minimum', true);
            $maximum = get_post_meta($post_id, CIVI_METABOX_PREFIX . 'jobs_salary_maximum', true);
            ...
            if ($minimum && $maximum) {
                ...
                $min_formatted = $this->format_salary_amount($minimum);
                echo esc_html(esc_html__('From', 'civi-framework') . ' ' . $currency . $min_formatted . $rate_text);
            } elseif ($maximum) {
                $max_formatted = $this->format_salary_amount($maximum);
                echo esc_html(esc_html__('Up to', 'civi-framework') . ' ' . $currency . $max_formatted . $rate_text);
            } else {
                echo '&ndash;';
            }
        }
        ...
        {
            $maximum = get_post_meta($post_id, CIVI_METABOX_PREFIX . 'jobs_maximum_price', true);
        ...
        {
            $minimum = get_post_meta($post_id, CIVI_METABOX_PREFIX . 'jobs_minimum_price', true);
        ...
        private function change_job_status($post_id, $new_status, $email_template = '')
        {
            ...
            if (!empty($email_template)) {
                ...
                if ($user) {
                    ...
                    civi_send_email($user->user_email, $email_template, $args);
                }
            }
        ...
        public function register_custom_column_titles($columns)
        {
            ...
            $columns['company']  = esc_html__('Company', 'civi-framework');
            $columns['type']     = esc_html__('Type', 'civi-framework');
            $columns['location'] = esc_html__('Location', 'civi-framework');
            $columns['salary']   = esc_html__('Salary', 'civi-framework');
        ...
        {
            $nonce = wp_create_nonce('civi_' . $action . '_job_' . $post_id);
            return '<a href="#" class="civi-ajax-action" data-action="' . esc_attr($action) . '" data-post-id="' . esc_attr($post_id) . '" data-nonce="' . esc_attr($nonce) . '">' . esc_html($label) . '</a>';
        }
        ...
        public function modify_list_row_actions($actions, $post)
        {
            ...
            if (in_array($status, array('pending', 'expired'))) {
                $actions['jobs-approve'] = $this->create_ajax_action_link('approve', $post_id, esc_html__('Approve', 'civi-framework'));
            }

            // Expire action
            if (in_array($status, array('publish', 'pending'))) {
                $actions['jobs-expired'] = $this->create_ajax_action_link('expire', $post_id, esc_html__('Expire', 'civi-framework'));
            }

            // Hide action
            if ($status === 'publish') {
                $actions['jobs-hidden'] = $this->create_ajax_action_link('hide', $post_id, esc_html__('Hide', 'civi-framework'));
            }

            // Show action
            if ($status === 'hidden') {
                $actions['jobs-show'] = $this->create_ajax_action_link('show', $post_id, esc_html__('Show', 'civi-framework'));
            }
            ...
            switch ($vars['orderby']) {
                ...
                case 'company':
                    $vars = array_merge($vars, array(
                        'meta_key' => CIVI_METABOX_PREFIX . 'jobs_select_company',
                ...
                case 'salary':
                    ...
                    $vars = array_merge($vars, array(
                        'meta_key' => CIVI_METABOX_PREFIX . 'jobs_salary_minimum',
                        'orderby' => 'meta_value_num',
                    ));
                    break;
        ...
        {
            $url_slug = civi_get_option($option_key);
        ...
        {
            $this->handle_ajax_status_change('approve', 'publish', 'mail_approved_listing', esc_html__('Job approved successfully.', 'civi-framework'), esc_html__('Failed to approve job.', 'civi-framework'));
        }
        ...
        {
            $this->handle_ajax_status_change('expire', 'expired', 'mail_expired_listing', esc_html__('Job expired successfully.', 'civi-framework'), esc_html__('Failed to expire job.', 'civi-framework'));
        }
        ...
        {
            $this->handle_ajax_status_change('hide', 'hidden', '', esc_html__('Job hidden successfully.', 'civi-framework'), esc_html__('Failed to hide job.', 'civi-framework'));
        }
        ...
        {
            $this->handle_ajax_status_change('show', 'publish', '', esc_html__('Job shown successfully.', 'civi-framework'), esc_html__('Failed to show job.', 'civi-framework'));
        }
        ...
        private function handle_ajax_status_change($action, $new_status, $email_template, $success_message, $error_message)
        {
            if (!isset($_POST['post_id']) || !isset($_POST['nonce'])) {
                wp_send_json_error(array('message' => esc_html__('Missing required parameters.', 'civi-framework')));
            }

            $post_id = absint(civi_clean(wp_unslash($_POST['post_id'])));
            check_ajax_referer('civi_' . $action . '_job_' . $post_id, 'nonce');

            if (!current_user_can('publish_post', $post_id)) {
                wp_send_json_error(array('message' => esc_html__('You do not have permission to perform this action.', 'civi-framework')));
            }
        ...
        public function enqueue_admin_scripts($hook)
        {
            ...
            wp_enqueue_style('select2', CIVI_PLUGIN_URL . 'assets/libs/select2/css/select2.min.css', array(), '4.0.13');
            ...
            $custom_css = '
                ...
                .civi-filter-wrapper .select2-container {
                    margin-right: 0;
                }
            ';
            ...
            $select2_config = array(
                ...
                'search_companies' => esc_js(__('Search companies...', 'civi-framework')),
                'search_authors' => esc_js(__('Search authors...', 'civi-framework')),
            );
        ...
        {
            echo '<span class="civi-filter-wrapper" style="display: inline-block; margin-right: 10px; margin-bottom: 10px;">';
            echo '<select name="' . esc_attr($name) . '" id="' . esc_attr($id) . '" class="civi-select2" data-placeholder="' . esc_attr($placeholder) . '">';
        ...
        public function filter_restrict_manage_jobs()
        {
            ...
            // Filter by Status
            $selected_status = isset($_GET['jobs_status']) ? civi_clean(wp_unslash($_GET['jobs_status'])) : '';
            $statuses = array(
                'publish' => esc_html__('Published', 'civi-framework'),
                'pending' => esc_html__('Pending', 'civi-framework'),
                'draft' => esc_html__('Draft', 'civi-framework'),
                'expired' => esc_html__('Expired', 'civi-framework'),
                'hidden' => esc_html__('Hidden', 'civi-framework'),
            );
            $this->render_filter_dropdown('jobs_status', 'jobs_status', $statuses, $selected_status, esc_html__('All Statuses', 'civi-framework'));
            ...
            if (!empty($companies)) {
                ...
                $this->render_filter_dropdown('jobs_company', 'jobs_company', $company_options, $selected_company, esc_html__('All Companies', 'civi-framework'));
            }
            ...
            foreach ($taxonomy_arr as $taxonomy) {
                ...
                echo '<span class="civi-filter-wrapper" style="display: inline-block; margin-right: 10px; margin-bottom: 10px;">';
                echo '<select name="' . esc_attr($taxonomy) . '" id="' . esc_attr($taxonomy) . '" class="civi-select2" data-placeholder="' . esc_attr(sprintf(__('All %s', 'civi-framework'), $taxonomy_label)) . '">';
                echo '<option value="">' . esc_html(sprintf(__('All %s', 'civi-framework'), $taxonomy_label)) . '</option>';
            ...
            // Filter by Featured
            $selected_featured = isset($_GET['jobs_featured']) ? civi_clean(wp_unslash($_GET['jobs_featured'])) : '';
            $featured_options = array(
                '1' => esc_html__('Featured', 'civi-framework'),
                '0' => esc_html__('Not Featured', 'civi-framework'),
            );
            $this->render_filter_dropdown('jobs_featured', 'jobs_featured', $featured_options, $selected_featured, esc_html__('All Featured', 'civi-framework'));
            ...
            $this->render_filter_dropdown('jobs_author', 'jobs_author', $author_options, $selected_author, esc_html__('All Authors', 'civi-framework'));
        }
        ...
        public function jobs_filter($query)
        {
            ...
            if (isset($_GET['jobs_status']) && !empty($_GET['jobs_status'])) {
                $q_vars['post_status'] = civi_clean(wp_unslash($_GET['jobs_status']));
            }
        ...
        public function auto_description_generate()
        {
            ...
            check_ajax_referer('civi_ai_generate_nonce', 'security');

            if (!isset($_POST['keywords'])) {
                echo json_encode(array('success' => false, 'message' => esc_html__('Missing required parameters.', 'civi-framework')));
                wp_die();
            }
            ...
            // Validate keywords
            if (empty($keywords)) {
                echo json_encode(array('success' => false, 'message' => esc_html__('Please enter a description.', 'civi-framework')));
                wp_die();
            }

            if (strlen($keywords) < 20) {
                echo json_encode(array('success' => false, 'message' => esc_html__('The description should be at least 20 characters.', 'civi-framework')));
                wp_die();
            }

            if (strlen($keywords) > 2000) {
                echo json_encode(array('success' => false, 'message' => esc_html__('Description too long. Please keep under 2000 characters.', 'civi-framework')));
                wp_die();
            }

            // Get AI configuration
            $ai_model = civi_get_option('ai_model', 'gpt-3.5-turbo');
            $ai_temperature = civi_get_option('ai_temperature', '0.7');
            $ai_max_tokens = civi_get_option('ai_max_tokens', '2048');
            $ai_key = civi_get_option('ai_key', '');

            // Validate API key
            if (empty($ai_key)) {
                echo json_encode(array('success' => false, 'message' => esc_html__('OpenAI API Key is not configured. Please configure it in admin settings.', 'civi-framework')));
                wp_die();
            }
            ...
            if (is_wp_error($response)) {
                ...
                echo json_encode(array(
                    'success' => false,
                    'message' => esc_html__('Connection error: ', 'civi-framework') . esc_html($error_message)
                ));
                wp_die();
            }
            ...
            if (empty($response_body_raw)) {
                echo json_encode(array(
                    'success' => false,
                    'message' => esc_html__('Empty response from API. Please try again.', 'civi-framework')
                ));
                wp_die();
            }
            ...
            if (json_last_error() !== JSON_ERROR_NONE) {
                echo json_encode(array(
                    'success' => false,
                    'message' => esc_html__('Invalid response format from API.', 'civi-framework')
                ));
                wp_die();
            }
            ...
            if (isset($response_body->error)) {
                ...
                $error_message = isset($response_body->error->message) ? $response_body->error->message : esc_html__('Unknown API error', 'civi-framework');

                if ($error_code === 'invalid_api_key') {
                    echo json_encode(array(
                        'success' => false,
                        'message' => esc_html__('Invalid API Key. Please check your OpenAI API key in admin settings.', 'civi-framework')
                    ));
                } else {
                    echo json_encode(array(
                        'success' => false,
            ...
            if (!isset($response_body->choices) || !is_array($response_body->choices) || empty($response_body->choices)) {
                echo json_encode(array(
                    'success' => false,
                    'message' => esc_html__('Invalid response structure from API.', 'civi-framework')
                ));
                wp_die();
            }
            ...
            if (!isset($response_body->choices[0]->message->content) || empty($response_body->choices[0]->message->content)) {
                echo json_encode(array(
                    'success' => false,
                    'message' => esc_html__('Empty content in API response.', 'civi-framework')
                ));
                wp_die();
            }
