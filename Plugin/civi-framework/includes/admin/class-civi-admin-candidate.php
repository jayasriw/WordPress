...
class Civi_Admin_Candidate
{
	...
	public function register_custom_column_titles($columns)
	{
		...
		$columns['title']  = esc_html__('Full Name', 'civi-framework');
		$columns['position'] = esc_html__('Position', 'civi-framework');
		$columns['location'] = esc_html__('Location', 'civi-framework');
		$columns['contact'] = esc_html__('Contact', 'civi-framework');
		...
		switch ($column) {
			case 'thumb':
				...
				if (null === $placeholder_url) {
					$placeholder_url = CIVI_PLUGIN_URL . 'assets/images/default-user-image.png';
				}
				...
				if (!empty($candidate_avatar)) {
					echo '<img src="' . esc_url($candidate_avatar) . '" alt="' . esc_attr__('Candidate Avatar', 'civi-framework') . '" style="' . esc_attr($avatar_style) . '" />';
				} else {
					echo '<img src="' . esc_url($placeholder_url) . '" alt="' . esc_attr__('No Avatar', 'civi-framework') . '" style="' . esc_attr($avatar_style) . ' opacity: 0.7;" />';
				}
				break;
			...
			case 'title':
				...
				if (function_exists('civi_get_candidate_display_name')) {
					$full_name = civi_get_candidate_display_name($candidate_id);
			...
			case 'position':
				$current_position = get_post_meta($candidate_id, CIVI_METABOX_PREFIX . 'candidate_current_position', true);
			...
			case 'contact':
				...
				$candidate_email = isset($meta_values[CIVI_METABOX_PREFIX . 'candidate_email']) ? $meta_values[CIVI_METABOX_PREFIX . 'candidate_email'][0] : '';
				$candidate_phone = isset($meta_values[CIVI_METABOX_PREFIX . 'candidate_phone']) ? $meta_values[CIVI_METABOX_PREFIX . 'candidate_phone'][0] : '';
			...
			case 'cate':
				echo civi_admin_taxonomy_terms($candidate_id, 'candidate_categories', 'candidate');
				break;

			case 'skills':
				echo civi_admin_taxonomy_terms($candidate_id, 'candidate_skills', 'candidate');
				break;
			...
			case 'status':
				$approval_status = get_post_meta($candidate_id, CIVI_METABOX_PREFIX . 'candidate_approval_status', true);
				...
				if ($approval_status === 'locked') {
					echo '<span class="civi-status-badge civi-status-locked" title="' . esc_attr__('Profile locked - User role changed to Employer', 'civi-framework') . '">';
					echo '<span class="dashicons dashicons-lock" style="color: #d63638;"></span> ';
					echo esc_html__('Locked', 'civi-framework');
					echo '</span>';
				} elseif ($post_status === 'publish') {
					echo '<span class="civi-status-badge civi-status-published">';
					echo '<span class="dashicons dashicons-yes-alt" style="color: #00a32a;"></span> ';
					echo esc_html__('Published', 'civi-framework');
					echo '</span>';
				} elseif ($post_status === 'pending') {
					echo '<span class="civi-status-badge civi-status-pending">';
					echo '<span class="dashicons dashicons-clock" style="color: #dba617;"></span> ';
					echo esc_html__('Pending', 'civi-framework');
					echo '</span>';
				} else {
					echo '<span class="civi-status-badge civi-status-draft">';
					echo '<span class="dashicons dashicons-edit" style="color: #646970;"></span> ';
					echo esc_html__('Draft', 'civi-framework');
					echo '</span>';
				}
		...
		switch ($vars['orderby']) {
			case 'position':
				$vars = array_merge($vars, array(
					'meta_key' => CIVI_METABOX_PREFIX . 'candidate_current_position',
					'orderby' => 'meta_value',
				));
				break;
			...
			case 'contact':
				...
				$vars = array_merge($vars, array(
					'meta_key' => CIVI_METABOX_PREFIX . 'candidate_email',
					'orderby' => 'meta_value',
				));
				break;
			...
			case 'status':
				$vars = array_merge($vars, array(
					'meta_key' => CIVI_METABOX_PREFIX . 'candidate_approval_status',
					'orderby' => 'meta_value',
				));
				break;
	...
	{
		$candidate_gender_url_slug = civi_get_option('candidate_gender_url_slug');
	...
	public function approve_candidate()
	{
		if (!empty($_GET['approve_candidate']) && wp_verify_nonce($_REQUEST['_wpnonce'], 'approve_candidate') && current_user_can('publish_post', $_GET['approve_candidate'])) {
			$post_id = absint(civi_clean(wp_unslash($_GET['approve_candidate'])));
			$listing_data = array(
				'ID' => $post_id,
				'post_status' => 'publish'
			);
			...
			if ($user) {
				...
				$company_post_id = get_user_meta($author_id, 'civi-company_id', true);
				...
				civi_send_email($user_email, 'mail_approved_candidate', $args);
				update_user_meta($author_id, CIVI_METABOX_PREFIX . 'user_status', 'approve');
				...
				if ($is_employer) {
					...
					civi_send_email($user_email, 'mail_approved_user_status_company', $args_company);
				} else {
					civi_send_email($user_email, 'mail_approved_user_status', $args);
				}
			}

			update_post_meta($post_id, CIVI_METABOX_PREFIX . 'candidate_approval_status', 'approved');
	...
	{
		check_ajax_referer('civi_approve_candidate');
		...
		update_post_meta($post_id, CIVI_METABOX_PREFIX . 'candidate_approval_status', 'approved');
		wp_send_json_success(array('message' => esc_html__('Approved', 'civi-framework')));
	}
	...
	public function modify_list_row_actions($actions, $post)
	{
		...
		if ($post->post_type == 'candidate') {
			if (in_array($post->post_status, array('pending'))) {
				$nonce = wp_create_nonce('civi_approve_candidate');
				$actions['candidate-approve'] = '<a href="#" class="civi-approve-candidate" data-post-id="' . esc_attr($post->ID) . '" data-nonce="' . esc_attr($nonce) . '">' . esc_html__('Approve', 'civi-framework') . '</a>';
			}
		}
		return $actions;
	}
	...
	public function filter_restrict_manage_candidate()
	{
		...
		if ($typenow == $post_type) {
			...
			?>
			<a href="<?php echo esc_url($export_url); ?>" id="civi-export-csv-btn" class="button" style="margin-left: 10px;">
				<span class="dashicons dashicons-download" style="vertical-align: middle; margin-top: 3px;"></span>
				<?php esc_html_e('Export CSV', 'civi-framework'); ?>
			</a>
			<?php
		}
	}
	...
	public function enqueue_candidate_admin_scripts($hook)
	{
		...
		wp_enqueue_script('civi-approve-candidate', CIVI_PLUGIN_URL . 'assets/js/admin/approve-candidate.js', array('jquery'), CIVI_THEME_VERSION, true);
		wp_localize_script('civi-approve-candidate', 'civiApproveCandidate', array(
	...
	public function export_candidates_csv()
	{
		...
		if (!current_user_can('export')) {
			wp_die(esc_html__('You do not have permission to export candidates', 'civi-framework'));
		}
		...
		// CSV Headers
		$headers = array(
			esc_html__('ID', 'civi-framework'),
			esc_html__('Full Name', 'civi-framework'),
			esc_html__('Position', 'civi-framework'),
			esc_html__('Location', 'civi-framework'),
			esc_html__('Email', 'civi-framework'),
			esc_html__('Phone', 'civi-framework'),
			esc_html__('Categories', 'civi-framework'),
			esc_html__('Skills', 'civi-framework'),
			esc_html__('Author', 'civi-framework'),
			esc_html__('Status', 'civi-framework'),
			esc_html__('Approval Status', 'civi-framework'),
			esc_html__('Date Created', 'civi-framework'),
			esc_html__('Date Modified', 'civi-framework'),
			esc_html__('URL', 'civi-framework'),
		);
		...
		$author_cache = array();
		$meta_keys = array(
			CIVI_METABOX_PREFIX . 'candidate_current_position',
			CIVI_METABOX_PREFIX . 'candidate_email',
			CIVI_METABOX_PREFIX . 'candidate_phone',
			CIVI_METABOX_PREFIX . 'candidate_approval_status',
		);
		...
		do {
			...
			foreach ($query->posts as $candidate) {
				...
				$position = isset($batch_meta[$candidate_id][CIVI_METABOX_PREFIX . 'candidate_current_position'])
					? $batch_meta[$candidate_id][CIVI_METABOX_PREFIX . 'candidate_current_position']
					: '';
				...
				$email = isset($batch_meta[$candidate_id][CIVI_METABOX_PREFIX . 'candidate_email'])
					? $batch_meta[$candidate_id][CIVI_METABOX_PREFIX . 'candidate_email']
					: '';
				$phone = isset($batch_meta[$candidate_id][CIVI_METABOX_PREFIX . 'candidate_phone'])
					? $batch_meta[$candidate_id][CIVI_METABOX_PREFIX . 'candidate_phone']
					: '';
				...
				$approval_status = isset($batch_meta[$candidate_id][CIVI_METABOX_PREFIX . 'candidate_approval_status'])
					? $batch_meta[$candidate_id][CIVI_METABOX_PREFIX . 'candidate_approval_status']
					: '';
