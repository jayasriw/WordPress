...
if (!class_exists('Civi_Core')) {
	...
	class Civi_Core
	{
		...
		private function include_library()
		{
			...
			require_once CIVI_PLUGIN_DIR . 'includes/class-civi-header.php';
			require_once CIVI_PLUGIN_DIR . 'includes/class-civi-footer.php';
			require_once CIVI_PLUGIN_DIR . 'includes/class-civi-search.php';
			...
			require_once CIVI_PLUGIN_DIR . 'includes/admin/class-civi-admin-candidate-package.php';
			require_once CIVI_PLUGIN_DIR . 'includes/admin/class-civi-admin-candidate-order.php';
			require_once CIVI_PLUGIN_DIR . 'includes/admin/class-civi-admin-service.php';
			require_once CIVI_PLUGIN_DIR . 'includes/admin/class-civi-admin-service-order.php';
			require_once CIVI_PLUGIN_DIR . 'includes/admin/class-civi-admin-service-withdraw.php';
			...
			include_once CIVI_PLUGIN_DIR . 'includes/partials/candidate/class-civi-candidate-order.php';
			include_once CIVI_PLUGIN_DIR . 'includes/partials/candidate/class-civi-candidate-package.php';
			include_once CIVI_PLUGIN_DIR . 'includes/partials/candidate/class-civi-candidate-payment.php';
			include_once CIVI_PLUGIN_DIR . 'includes/partials/candidate/class-civi-candidate-trans-log.php';
			include_once CIVI_PLUGIN_DIR . 'includes/partials/service/class-civi-service.php';
			include_once CIVI_PLUGIN_DIR . 'includes/partials/service/class-civi-service-order.php';
			include_once CIVI_PLUGIN_DIR . 'includes/partials/service/class-civi-service-payment.php';
			include_once CIVI_PLUGIN_DIR . 'includes/partials/service/class-civi-service-withdraw.php';

			// Init Search Optimization
			if (class_exists('Civi_Search')) {
				Civi_Search::instance();
			}
		}
		...
		private function admin_hooks()
		{
			...
			add_filter('civi_jobs_gender_slug', array($civi_admin_jobs, 'modify_jobs_gender_slug'));
			add_action('restrict_manage_posts', array($civi_admin_jobs, 'filter_restrict_manage_jobs'));
			add_action('admin_enqueue_scripts', array($civi_admin_jobs, 'enqueue_admin_scripts'));

			add_filter('parse_query', array($civi_admin_jobs, 'jobs_filter'));
			add_filter('posts_clauses', array($civi_admin_jobs, 'posts_clauses'), 10, 1);
			...
			add_action('admin_init', array($civi_admin_jobs, 'add_badge_menu'));
			...
			add_action('wp_ajax_civi_ajax_approve_job', array($civi_admin_jobs, 'ajax_approve_jobs'));
			add_action('wp_ajax_civi_ajax_expire_job', array($civi_admin_jobs, 'ajax_expire_jobs'));
			add_action('wp_ajax_civi_ajax_hide_job', array($civi_admin_jobs, 'ajax_hide_jobs'));
			add_action('wp_ajax_civi_ajax_show_job', array($civi_admin_jobs, 'ajax_show_jobs'));

			add_action('wp_ajax_auto_description_generate', array($civi_admin_jobs, 'auto_description_generate'));
			add_action('wp_ajax_nopriv_auto_description_generate', array($civi_admin_jobs, 'auto_description_generate'));
			...
			/**
			 * Hook Civi_Admin_candidate_package
			 */
			$civi_admin_candidate_package = new Civi_Admin_candidate_package();
			add_filter('civi_candidate_package_slug', array($civi_admin_candidate_package, 'modify_candidate_package_slug'));
			...
			add_action('admin_init', array($civi_admin_invoice, 'invoice_active'));
			add_action('admin_init', array($civi_admin_invoice, 'invoice_pending'));
			...
			/**
			 * Hook Civi_Admin_candidate_order
			 */
			$civi_admin_candidate_order = new Civi_Admin_candidate_order();
			add_action('parse_query', array($civi_admin_candidate_order, 'candidate_order_filter'));
			add_action('restrict_manage_posts', array($civi_admin_candidate_order, 'filter_restrict_manage_candidate_order'));
			add_action('admin_init', array($civi_admin_candidate_order, 'candidate_order_active'));
			add_action('admin_init', array($civi_admin_candidate_order, 'candidate_order_pending'));

			/**
			 * Hook Civi_Admin_service_order
			 */
			$civi_admin_service_order = new Civi_Admin_service_order();
			add_action('parse_query', array($civi_admin_service_order, 'service_order_filter'));
			add_action('restrict_manage_posts', array($civi_admin_service_order, 'filter_restrict_manage_service_order'));
			add_action('admin_init', array($civi_admin_service_order, 'service_order_inprogress'));
			add_action('admin_init', array($civi_admin_service_order, 'service_order_pending'));

			/**
			 * Hook Civi_Admin_Service_Withdraw
			 */
			$civi_admin_service_withdraw = new Civi_Admin_Service_Withdraw();
			add_action('parse_query', array($civi_admin_service_withdraw, 'service_withdraw_filter'));
			add_action('restrict_manage_posts', array($civi_admin_service_withdraw, 'filter_restrict_manage_service_withdraw'));
			add_action('admin_init', array($civi_admin_service_withdraw, 'service_withdraw_active'));
			add_action('admin_init', array($civi_admin_service_withdraw, 'service_withdraw_pending'));
			add_action('admin_init', array($civi_admin_service_withdraw, 'service_withdraw_canceled'));
			...
			add_action('admin_init', array($civi_admin_company, 'add_badge_menu'));
			...
			add_filter('civi_candidate_gender_slug', array($civi_admin_candidate, 'modify_candidate_gender_url_slug'));
			...
			add_action('admin_post_export_candidates_csv', array($civi_admin_candidate, 'export_candidates_csv'));
			add_action('admin_enqueue_scripts', array($civi_admin_candidate, 'enqueue_candidate_admin_scripts'));
			add_action('admin_init', array($civi_admin_candidate, 'show_candidates'));
			add_action('admin_init', array($civi_admin_candidate, 'approve_candidate'));
			add_action('admin_init', array($civi_admin_candidate, 'add_badge_menu'));

			/**
			 * Hook Civi_Admin_Candidate
			 */
			$civi_admin_service = new Civi_Admin_Service();
			add_filter('civi_service_slug', array($civi_admin_service, 'modify_service_slug'));
			add_filter('civi_service_has_archive', array($civi_admin_service, 'modify_service_has_archive'));
			add_filter('civi_service_categories_slug', array($civi_admin_service, 'modify_service_categories_url_slug'));
			add_filter('civi_service_skills_slug', array($civi_admin_service, 'modify_service_skills_url_slug'));
			add_filter('civi_service_location_slug', array($civi_admin_service, 'modify_service_location_url_slug'));
			add_filter('civi_service_language_slug', array($civi_admin_service, 'modify_service_language_url_slug'));
			add_action('restrict_manage_posts', array($civi_admin_service, 'filter_restrict_manage_service'));
			add_action('parse_query', array($civi_admin_service, 'service_filter'));
			add_action('admin_init', array($civi_admin_service, 'approve_service'));
			...
			if (is_admin()) {
				...
				if ($pagenow == 'edit.php' && isset($_GET['post_type']) && esc_attr($_GET['post_type']) == 'candidate') {
					...
					add_filter('request', array($civi_admin_candidate, 'column_orderby'));
					add_filter('posts_clauses', array($civi_admin_candidate, 'posts_clauses'), 10, 1);
					add_filter('post_row_actions', array($civi_admin_candidate, 'modify_list_row_actions'), 10, 2);
				}

				// service custom columns
				if ($pagenow == 'edit.php' && isset($_GET['post_type']) && esc_attr($_GET['post_type']) == 'service') {
					add_filter('manage_edit-service_columns', array($civi_admin_service, 'register_custom_column_titles'));
					add_action('manage_posts_custom_column', array($civi_admin_service, 'display_custom_column'));
					add_filter('manage_edit-service_sortable_columns', array($civi_admin_service, 'sortable_columns'));
					add_filter('post_row_actions', array($civi_admin_service, 'modify_list_row_actions'), 10, 2);
				}
				...
				if ($pagenow == 'edit.php' && isset($_GET['post_type']) && esc_attr($_GET['post_type']) == 'candidate_order') {
					add_filter('manage_edit-candidate_order_columns', array($civi_admin_candidate_order, 'register_custom_column_titles'));
					add_action('manage_posts_custom_column', array($civi_admin_candidate_order, 'display_custom_column'));
					add_filter('manage_edit-candidate_order_sortable_columns', array($civi_admin_candidate_order, 'sortable_columns'));
					add_filter('request', array($civi_admin_candidate_order, 'column_orderby'));
					add_filter('post_row_actions', array($civi_admin_candidate_order, 'modify_list_row_actions'), 10, 2);
				}

				// Candidate package custom columns
				if ($pagenow == 'edit.php' && isset($_GET['post_type']) && esc_attr($_GET['post_type']) == 'candidate_package') {
					add_filter('manage_edit-candidate_package_columns', array($civi_admin_candidate_package, 'register_custom_column_titles'));
					add_action('manage_posts_custom_column', array($civi_admin_candidate_package, 'display_custom_column'));
					add_filter('post_row_actions', array($civi_admin_candidate_package, 'modify_list_row_actions'), 10, 2);
				}

				// Service order custom columns
				if ($pagenow == 'edit.php' && isset($_GET['post_type']) && esc_attr($_GET['post_type']) == 'service_order') {
					add_filter('manage_edit-service_order_columns', array($civi_admin_service_order, 'register_custom_column_titles'));
					add_action('manage_posts_custom_column', array($civi_admin_service_order, 'display_custom_column'));
					add_filter('manage_edit-service_order_sortable_columns', array($civi_admin_service_order, 'sortable_columns'));
					add_filter('request', array($civi_admin_service_order, 'column_orderby'));
					add_filter('post_row_actions', array($civi_admin_service_order, 'modify_list_row_actions'), 10, 2);
				}

				// Service withdraw custom columns
				if ($pagenow == 'edit.php' && isset($_GET['post_type']) && esc_attr($_GET['post_type']) == 'service_withdraw') {
					add_filter('manage_edit-service_withdraw_columns', array($civi_admin_service_withdraw, 'register_custom_column_titles'));
					add_action('manage_posts_custom_column', array($civi_admin_service_withdraw, 'display_custom_column'));
					add_filter('manage_edit-service_withdraw_sortable_columns', array($civi_admin_service_withdraw, 'sortable_columns'));
					add_filter('request', array($civi_admin_service_withdraw, 'column_orderby'));
					add_filter('post_row_actions', array($civi_admin_service_withdraw, 'modify_list_row_actions'), 10, 2);
				}
			}
		}
		...
		private function template_hooks()
		{
			...
			add_action('wp_footer', array($civi_template_loader, 'civi_form_setting_deactive'));
			add_action('wp_footer', array($civi_template_loader, 'civi_form_candidate_user_package'));
			add_action('wp_footer', array($civi_template_loader, 'civi_form_employer_user_package'));
			add_action('wp_footer', array($civi_template_loader, 'civi_form_service_order_refund'));
			add_action('wp_footer', array($civi_template_loader, 'civi_form_service_view_reason'));
			add_action('wp_footer', array($civi_template_loader, 'civi_form_service_withdraw'));
			add_action('wp_head', array($civi_template_loader, 'civi_add_google_job_schema'));
			add_action('wp_head', array($civi_template_loader, 'civi_add_company_schema'));
			add_action('wp_head', array($civi_template_loader, 'civi_add_candidate_schema'));
			add_action('wp_head', array($civi_template_loader, 'civi_add_service_schema'));
			add_action('send_meeting_notification', array($civi_template_loader, 'send_meeting_notification'));
			add_action('init', array($civi_template_loader, 'setup_meeting_notifications'));
			...
			add_action('wp_ajax_civi_service_archive_ajax', array($civi_ajax, 'civi_service_archive_ajax'));
			add_action('wp_ajax_nopriv_civi_service_archive_ajax', array($civi_ajax, 'civi_service_archive_ajax'));
			...
			add_action('wp_ajax_civi_read_mess_ajax_load', array($civi_ajax, 'civi_read_mess_ajax_load'));
			add_action('wp_ajax_nopriv_civi_read_mess_ajax_load', array($civi_ajax, 'civi_read_mess_ajax_load'));

			add_action('wp_ajax_civi_realy_mess_ajax_load', array($civi_ajax, 'civi_realy_mess_ajax_load'));
			add_action('wp_ajax_nopriv_civi_realy_mess_ajax_load', array($civi_ajax, 'civi_realy_mess_ajax_load'));
			...
			add_action('wp_ajax_civi_filter_employer_wishlist', array($civi_ajax, 'civi_filter_employer_wishlist'));
			add_action('wp_ajax_nopriv_civi_filter_employer_wishlist', array($civi_ajax, 'civi_filter_employer_wishlist'));
			...
			//update payout
			add_action('wp_ajax_civi_update_payout_ajax', array($civi_ajax, 'civi_update_payout_ajax'));
			add_action('wp_ajax_nopriv_civi_update_payout_ajax', array($civi_ajax, 'civi_update_payout_ajax'));
			...
			add_action('wp_ajax_civi_service_wishlist', array($civi_ajax, 'civi_service_wishlist'));
			add_action('wp_ajax_nopriv_civi_service_wishlist', array($civi_ajax, 'civi_service_wishlist'));

			// Add to service addons
			add_action('wp_ajax_civi_service_addons', array($civi_ajax, 'civi_service_addons'));
			add_action('wp_ajax_nopriv_civi_service_addons', array($civi_ajax, 'civi_service_addons'));
			...
			add_action('wp_ajax_civi_candidate_download_cv', array($civi_ajax, 'civi_candidate_download_cv'));
			add_action('wp_ajax_nopriv_civi_candidate_download_cv', array($civi_ajax, 'civi_candidate_download_cv'));
			...
			// Verify ID
			add_action('wp_ajax_civi_verify_id_before_upload_ajax', array($civi_ajax, 'civi_verify_id_before_upload_ajax'));
			add_action('wp_ajax_nopriv_civi_verify_id_before_upload_ajax', array($civi_ajax, 'civi_verify_id_before_upload_ajax'));

			add_action('wp_ajax_civi_verify_id_before_remove_ajax', array($civi_ajax, 'civi_verify_id_before_remove_ajax'));
			add_action('wp_ajax_nopriv_civi_verify_id_before_remove_ajax', array($civi_ajax, 'civi_verify_id_before_remove_ajax'));

			add_action('wp_ajax_civi_verify_id_after_upload_ajax', array($civi_ajax, 'civi_verify_id_after_upload_ajax'));
			add_action('wp_ajax_nopriv_civi_verify_id_after_upload_ajax', array($civi_ajax, 'civi_verify_id_after_upload_ajax'));

			add_action('wp_ajax_civi_verify_id_after_remove_ajax', array($civi_ajax, 'civi_verify_id_after_remove_ajax'));
			add_action('wp_ajax_nopriv_civi_verify_id_after_remove_ajax', array($civi_ajax, 'civi_verify_id_after_remove_ajax'));

			add_action('wp_ajax_civi_verify_id_selfie_upload_ajax', array($civi_ajax, 'civi_verify_id_selfie_upload_ajax'));
			add_action('wp_ajax_nopriv_civi_verify_id_selfie_upload_ajax', array($civi_ajax, 'civi_verify_id_selfie_upload_ajax'));

			add_action('wp_ajax_civi_verify_id_selfie_remove_ajax', array($civi_ajax, 'civi_verify_id_selfie_remove_ajax'));
			add_action('wp_ajax_nopriv_civi_verify_id_selfie_remove_ajax', array($civi_ajax, 'civi_verify_id_selfie_remove_ajax'));

			// Ajax Custom Image
			add_action('wp_ajax_civi_custom_image_upload_ajax', array($civi_ajax, 'civi_custom_image_upload_ajax'));
			add_action('wp_ajax_nopriv_civi_custom_image_upload_ajax', array($civi_ajax, 'civi_custom_image_upload_ajax'));

			add_action('wp_ajax_civi_custom_image_remove_ajax', array($civi_ajax, 'civi_custom_image_remove_ajax'));
			add_action('wp_ajax_nopriv_civi_custom_image_remove_ajax', array($civi_ajax, 'civi_custom_image_remove_ajax'));

			// Ajax Mess Image
			add_action('wp_ajax_civi_mess_image_upload_ajax', array($civi_ajax, 'civi_mess_image_upload_ajax'));
			add_action('wp_ajax_nopriv_civi_mess_image_upload_ajax', array($civi_ajax, 'civi_mess_image_upload_ajax'));

			add_action('wp_ajax_civi_mess_image_remove_ajax', array($civi_ajax, 'civi_mess_image_remove_ajax'));
			add_action('wp_ajax_nopriv_civi_mess_image_remove_ajax', array($civi_ajax, 'civi_mess_image_remove_ajax'));
			...
			// Service
			add_action('wp_ajax_civi_filter_my_service', array($civi_ajax, 'civi_filter_my_service'));
			add_action('wp_ajax_nopriv_civi_filter_my_service', array($civi_ajax, 'civi_filter_my_service'));

			add_action('wp_ajax_civi_employer_order_service', array($civi_ajax, 'civi_employer_order_service'));
			add_action('wp_ajax_nopriv_civi_employer_order_service', array($civi_ajax, 'civi_employer_order_service'));

			add_action('wp_ajax_civi_candidate_order_service', array($civi_ajax, 'civi_candidate_order_service'));
			add_action('wp_ajax_nopriv_civi_candidate_order_service', array($civi_ajax, 'civi_candidate_order_service'));

			add_action('wp_ajax_civi_candidate_wallet_service', array($civi_ajax, 'civi_candidate_wallet_service'));
			add_action('wp_ajax_nopriv_civi_candidate_wallet_service', array($civi_ajax, 'civi_candidate_wallet_service'));

			add_action('wp_ajax_civi_submit_withdraw', array($civi_ajax, 'civi_submit_withdraw'));
			add_action('wp_ajax_nopriv_civi_submit_withdraw', array($civi_ajax, 'civi_submit_withdraw'));

			// Locations
			add_action('wp_ajax_civi_select_country', array($civi_ajax, 'civi_select_country'));
			add_action('wp_ajax_nopriv_civi_select_country', array($civi_ajax, 'civi_select_country'));

			add_action('wp_ajax_civi_select_state', array($civi_ajax, 'civi_select_state'));
			add_action('wp_ajax_nopriv_civi_select_state', array($civi_ajax, 'civi_select_state'));

			// Apply Package Coupon
			add_action('wp_ajax_civi_apply_package_coupon', array($civi_ajax, 'civi_apply_package_coupon'));
			add_action('wp_ajax_nopriv_civi_apply_package_coupon', array($civi_ajax, 'civi_apply_package_coupon'));
			...
			add_action('wp_ajax_civi_stripe_create_invoice_per_package', array($civi_payment, 'stripe_create_invoice_per_package_ajax'));

			add_action('wp_ajax_civi_razor_package_create_order', array($civi_payment, 'civi_razor_package_create_order'));
			add_action('wp_ajax_civi_razor_payment_verify', array($civi_payment, 'civi_razor_payment_verify'));
			...
			add_action('wp_ajax_save_candidate_profile_strength', array($civi_candidate, 'save_candidate_profile_strength_ajax'));

			add_action('wp_ajax_civi_candidate_print_ajax', array($civi_ajax, 'civi_candidate_print_ajax'));
			add_action('wp_ajax_nopriv_civi_candidate_print_ajax', array($civi_ajax, 'civi_candidate_print_ajax'));
			/**
			 * Hook Civi_Service
			 */
			$civi_service = new Civi_Service();
			add_action('wp_ajax_civi_service_submit_review_ajax', array($civi_service, 'submit_review_ajax'));
			add_action('wp_ajax_nopriv_civi_service_submit_review_ajax', array($civi_service, 'submit_review_ajax'));

			add_filter('civi_service_rating_meta', array($civi_service, 'rating_meta_filter'), 4, 9);

			add_action('wp_ajax_civi_service_submit_reply_ajax', array($civi_service, 'submit_reply_ajax'));
			add_action('wp_ajax_nopriv_service_submit_reply_ajax', array($civi_service, 'submit_reply_ajax'));

			add_action('wp_ajax_service_submit_ajax', array($civi_service, 'service_submit_ajax'));
			add_action('wp_ajax_nopriv_service_submit_ajax', array($civi_service, 'service_submit_ajax'));
			/**
			 * Hook Civi_Meetings
			 */
			...
			add_action('wp_ajax_civi_load_more_messages', array($civi_messages, 'civi_load_more_messages'));
			add_action('wp_ajax_nopriv_civi_load_more_messages', array($civi_messages, 'civi_load_more_messages'));
			...
			/**
			 * Hook Civi Candidate Payment
			 */
			$civi_candidate_payment = new Civi_Candidate_Payment();
			add_action('wp_ajax_civi_candidate_paypal_payment_per_package_ajax', array($civi_candidate_payment, 'candidate_paypal_payment_per_package_ajax'));
			add_action('wp_ajax_nopriv_civi_candidate_paypal_payment_per_package_ajax', array($civi_candidate_payment, 'candidate_paypal_payment_per_package_ajax'));

			add_action('wp_ajax_civi_candidate_wire_transfer_per_package_ajax', array($civi_candidate_payment, 'candidate_wire_transfer_per_package_ajax'));
			add_action('wp_ajax_nopriv_civi_candidate_wire_transfer_per_package_ajax', array($civi_candidate_payment, 'candidate_wire_transfer_per_package_ajax'));

			add_action('wp_ajax_civi_razor_package_candidate_create_order', array($civi_candidate_payment, 'civi_razor_package_candidate_create_order'));
			add_action('wp_ajax_civi_candidate_razor_package_verify', array($civi_candidate_payment, 'civi_candidate_razor_package_verify'));

			add_action('wp_ajax_civi_candidate_free_package_ajax', array($civi_candidate_payment, 'candidate_free_package_ajax'));
			add_action('wp_ajax_nopriv_civi_candidate_free_package_ajax', array($civi_candidate_payment, 'candidate_free_package_ajax'));

			add_action('wp_ajax_civi_candidate_woocommerce_payment_per_package_ajax', array($civi_candidate_payment, 'candidate_woocommerce_payment_per_package_ajax'));
			add_action('wp_ajax_nopriv_civi_candidate_woocommerce_payment_per_package_ajax', array($civi_candidate_payment, 'candidate_woocommerce_payment_per_package_ajax'));

			/**
			 * Hook Civi Candidate Package Check
			 */
			add_action('wp_ajax_civi_check_old_package_before_activate', array($civi_ajax, 'civi_check_old_package_before_activate'));

			/**
			 * Hook Civi Candidate Package Impact Check (for warning modal)
			 */
			$civi_candidate_package = new Civi_candidate_package();
			add_action('wp_ajax_civi_check_candidate_package_impact', array($civi_candidate_package, 'ajax_check_package_impact'));

			// Employer package impact check
			$civi_package = new Civi_Package();
			add_action('wp_ajax_civi_check_employer_package_impact', array($civi_package, 'ajax_check_package_impact'));

			/**
			 * Hook Civi Employer Package Check
			 */
			add_action('wp_ajax_civi_check_old_employer_package_before_activate', array($civi_ajax, 'civi_check_old_employer_package_before_activate'));

			/**
			 * Hook Civi Service Payment
			 */
			$civi_service_payment = new Civi_Service_Payment();
			add_action('wp_ajax_civi_paypal_payment_service_addons', array($civi_service_payment, 'civi_paypal_payment_service_addons'));
			add_action('wp_ajax_nopriv_civi_paypal_payment_service_addons', array($civi_service_payment, 'civi_paypal_payment_service_addons'));

			add_action('wp_ajax_civi_wire_transfer_service_addons', array($civi_service_payment, 'civi_wire_transfer_service_addons'));
			add_action('wp_ajax_nopriv_civi_wire_transfer_service_addons', array($civi_service_payment, 'civi_wire_transfer_service_addons'));

			add_action('wp_ajax_civi_razor_service_create_order', array($civi_service_payment, 'civi_razor_service_create_order'));
			add_action('wp_ajax_civi_razor_service_payment_verify', array($civi_service_payment, 'civi_razor_service_payment_verify'));

			add_action('wp_ajax_civi_woocommerce_payment_service_addons', array($civi_service_payment, 'civi_woocommerce_payment_service_addons'));
			add_action('wp_ajax_nopriv_civi_woocommerce_payment_service_addons', array($civi_service_payment, 'civi_woocommerce_payment_service_addons'));
		}
		...
		{
			wp_localize_script('jquery', 'civi_ajax_nonce', array(
				// Withdraw
				'withdraw' => wp_create_nonce('civi_withdraw_nonce'),

				// Wishlist & Follow
				'wishlist' => wp_create_nonce('civi_wishlist_nonce'),
				'service_wishlist' => wp_create_nonce('civi_service_wishlist_nonce'),
				'follow' => wp_create_nonce('civi_follow_nonce'),
				'invite' => wp_create_nonce('civi_invite_nonce'),
				'follow_candidate' => wp_create_nonce('civi_follow_candidate_nonce'),

				// Service
			'service_addons'   => wp_create_nonce('civi_service_addons_nonce'),
			'submit_review'    => wp_create_nonce('civi_submit_review_ajax_nonce'),
			'submit_reply'     => wp_create_nonce('civi_submit_reply_ajax_nonce'),
			'create_company'   => wp_create_nonce('create_company_nonce'),
			'submit_service'   => wp_create_nonce('civi_service_submit_nonce'),
			'send_message'     => wp_create_nonce('send_message_nonce'),
			'write_message'    => wp_create_nonce('write_message_nonce'),
			'update_profile'   => wp_create_nonce('civi_update_profile_ajax_nonce'),
			'change_password'  => wp_create_nonce('civi_change_password_ajax_nonce'),
            'download_cv_candidate' => wp_create_nonce('civi_download_cv_candidate_nonce'),
            'civi_candidate_print' => wp_create_nonce('civi_candidate_print_nonce'),
            'update_payout' => wp_create_nonce('civi_update_payout_ajax_nonce'),
            'civi_chart_nonce' => wp_create_nonce('civi_chart_nonce'),
            ));
		}
