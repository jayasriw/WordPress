...
if (!class_exists('Civi_Service')) {
	...
	class Civi_Service
	{
		...
		public function submit_review_ajax()
		{
			...
			if ($service_id <= 0) {
				wp_send_json_error(['message' => __('Invalid service ID.', 'civi-framework')], 422);
			}
         ...
         LIMIT 1",
			...
			if (null === $my_review) {
				...
				if (!$comment_id || is_wp_error($comment_id)) {
					wp_send_json_error(['message' => __('Could not save review.', 'civi-framework')], 500);
				}
				...
				if (!$ok) {
					wp_send_json_error(['message' => __('Could not update review.', 'civi-framework')], 500);
				}
		...
		public function submit_reply_ajax()
		{
			...
			if (! is_user_logged_in()) {
				wp_send_json_error(['message' => __('You must be logged in to reply.', 'civi-framework')], 401);
			}
		...
		{
			check_ajax_referer('civi_service_submit_ajax_nonce', 'civi_security_service_submit');
