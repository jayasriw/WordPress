...
if (!class_exists('Civi_Package')) {
    ...
    class Civi_Package
    {
        ...
        public function insert_user_package($user_id, $package_id, $invoice_pdf_id = '', $skip_cleanup = false)
        {
            ...
            $package_number_follow = get_post_meta($package_id, CIVI_METABOX_PREFIX . 'company_package_number_candidate_follow', true);
            $package_follow_unlimited = get_post_meta($package_id, CIVI_METABOX_PREFIX . 'enable_package_candidate_follow_unlimited', true);
            $package_number_download_cv = get_post_meta($package_id, CIVI_METABOX_PREFIX . 'company_package_number_download_cv', true);
            $package_download_cv_unlimited = get_post_meta($package_id, CIVI_METABOX_PREFIX . 'enable_package_download_cv_unlimited', true);
            ...
            update_user_meta($user_id, CIVI_METABOX_PREFIX . 'package_number_job', $remaining_jobs);

            // Featured jobs - deduct current featured count
            $active_featured_count = $this->count_active_featured_jobs($user_id);
            $remaining_featured = max(0, intval($package_number_featured) - $active_featured_count);
            update_user_meta($user_id, CIVI_METABOX_PREFIX . 'package_number_featured', $remaining_featured);

            // Candidate follow - deduct current follow count
            $follows = get_user_meta($user_id, 'follow_candidate', true) ?: [];
            $current_follow_count = is_array($follows) ? count($follows) : 0;
            $remaining_follow = max(0, intval($package_number_follow) - $current_follow_count);
            update_user_meta($user_id, CIVI_METABOX_PREFIX . 'package_number_candidate_follow', $remaining_follow);

            // Action-based - just reset
            update_user_meta($user_id, CIVI_METABOX_PREFIX . 'package_number_download_cv', $package_number_download_cv);

            do_action('civi_ajax_field_package_jobs', $user_id, $package_id);
            ...
            // civi_send_email($user_email, 'mail_activated_package', $package_args);
        ...
        public function get_package_quotas($package_id)
        {
            ...
            $get_meta = function($key) use ($all_meta) {
                $full_key = CIVI_METABOX_PREFIX . $key;
        ...
        {
            check_ajax_referer('civi_package_nonce', 'security');
        ...
        public function get_active_package_info($user_id)
        {
            ...
            $activate_date = get_user_meta($user_id, CIVI_METABOX_PREFIX . 'package_activate_date', true);

            // Get remaining quota
            $remaining_quota = array();

            $remaining_quota['jobs'] = intval(get_user_meta($user_id, CIVI_METABOX_PREFIX . 'package_number_job', true));
            $remaining_quota['jobs_featured'] = intval(get_user_meta($user_id, CIVI_METABOX_PREFIX . 'package_number_featured', true));
            $remaining_quota['candidate_follow'] = intval(get_user_meta($user_id, CIVI_METABOX_PREFIX . 'package_number_candidate_follow', true));
            $remaining_quota['download_cv'] = intval(get_user_meta($user_id, CIVI_METABOX_PREFIX . 'package_number_download_cv', true));
