...
if (!class_exists('Civi_Invoice')) {
	...
	class Civi_Invoice
	{
		...
		public function insert_invoice($payment_type, $item_id, $user_id, $payment_for, $payment_method, $paid = 0, $payment_id = '', $payer_id = '', $force_new = false)
		{
			...
			$package_coupon = get_user_meta($user_id, CIVI_METABOX_PREFIX . 'package_coupon', true);
			$package_coupon_id = get_user_meta($user_id, CIVI_METABOX_PREFIX . 'package_coupon_id', true);
			...
			$rw = civi_get_page_by_title($posttitle, 'invoice');
			...
			if ($package_coupon_id) {
				$counpon_used = get_post_meta($package_coupon_id, CIVI_METABOX_PREFIX . 'coupon_used', true);
				if (empty($counpon_used)) {
					update_post_meta($package_coupon_id, CIVI_METABOX_PREFIX . 'coupon_used', 1);
				} else {
					update_post_meta($package_coupon_id, CIVI_METABOX_PREFIX . 'coupon_used', $counpon_used + 1);
				}
			}

			$pdf_id = generate_pdf_with_fpdf($invoice_id);
			update_post_meta($invoice_id, CIVI_METABOX_PREFIX . 'invoice_pdf_id', $pdf_id);

			delete_user_meta($user_id, CIVI_METABOX_PREFIX . 'package_coupon');
			delete_user_meta($user_id, CIVI_METABOX_PREFIX . 'package_coupon_id');
		...
		public static function get_invoice_payment_method($payment_method)
		{
			switch ($payment_method) {
				case 'Razor':
					return esc_html__('Razor', 'civi-framework');
					break;
				case 'Paypal':
