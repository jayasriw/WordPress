    ...
    GLF_MapClass.prototype = {
        ...
        bindMap: function () {
            ...
            if( this.$maptype == 'google_map' ){
                ...
                if (typeof mapboxgl === 'undefined' || !this.$mapbox.length || !this.$mapbox[0]) {
                    return;
                }
                ...
                var lat = parseFloat($.trim(String(locationValue[0])));
                var lng = parseFloat($.trim(String(locationValue[1])));
                if (isNaN(lat) || isNaN(lng)) {
                    lat = 40;
                    lng = -74.5;
                }

                var styleSlug = this.$mapstyle || 'streets-v11';
                var mapStyle = (typeof styleSlug === 'string' && styleSlug.indexOf('mapbox://') === 0)
                    ? styleSlug
                    : 'mapbox://styles/mapbox/' + styleSlug;

                var zoomNum = parseInt(this.$mapzoom, 10);
                if (isNaN(zoomNum)) {
                    zoomNum = 12;
                }

                var config_default = {
                    container: this.$mapbox[0],
                    style: mapStyle,
                    center: [lng, lat],
                    zoom: zoomNum
                };
                ...
                this.mapbox = new mapboxgl.Map(config_default);
