...
$item_amount = jobportal_get_option('archive_candidate_items_amount', '12');
wp_enqueue_script(JOBPORTAL_PLUGIN_PREFIX . 'select-location');
wp_enqueue_script(JOBPORTAL_PLUGIN_PREFIX . 'candidate-archive');
wp_localize_script(
    JOBPORTAL_PLUGIN_PREFIX . 'candidate-archive',
    'jobportal_candidate_archive_vars',
    array(
...
$hide_candidate_top_filter_fields = jobportal_get_option('hide_candidate_top_filter_fields');
$enable_candidate_filter_top = jobportal_get_option('enable_candidate_filter_top');
$candidate_filter_sidebar_option = jobportal_get_option('candidate_filter_sidebar_option');
...
$candidate_filter_sidebar_option = !empty($_GET['filter']) ? JobPortal_Helper::jobportal_clean(wp_unslash($_GET['filter'])) : $candidate_filter_sidebar_option;
...
if (!empty($candidate_location_values)) {
    ...
    foreach ($candidate_location_values as $loc) {
        ...
        $location_term = JobPortal_Location_Search::find_location_term($loc, 'candidate_locations');
...
$candidate_categories_param = isset($_GET['candidate_categories']) ? jobportal_clean(wp_unslash($_GET['candidate_categories'])) : '';
...
$candidate_ages_param = isset($_GET['candidate_ages']) ? jobportal_clean(wp_unslash($_GET['candidate_ages'])) : '';
...
$candidate_languages_param = isset($_GET['candidate_languages']) ? jobportal_clean(wp_unslash($_GET['candidate_languages'])) : '';
...
$candidate_qualification_param = isset($_GET['candidate_qualification']) ? jobportal_clean(wp_unslash($_GET['candidate_qualification'])) : '';
...
$candidate_yoe_param = isset($_GET['candidate_yoe']) ? jobportal_clean(wp_unslash($_GET['candidate_yoe'])) : '';
...
$candidate_education_levels_param = isset($_GET['candidate_education_levels']) ? jobportal_clean(wp_unslash($_GET['candidate_education_levels'])) : '';
...
$candidate_skills_param = isset($_GET['candidate_skills']) ? jobportal_clean(wp_unslash($_GET['candidate_skills'])) : '';
...
$args = apply_filters('jobportal/archive-candidate/layout-default/query/args', $args);
...
<?php if ($enable_candidate_filter_top == 1) { ?>
    <?php do_action('jobportal_archive_candidate_top_filter', $current_term, $total_post); ?>
<?php } ?>

<div class="inner-content container <?php echo join(' ', $class_inner); ?>">
    <div class="col-left">
        <?php if ($candidate_filter_sidebar_option !== 'filter-right') {
            do_action('jobportal_archive_candidate_sidebar_filter', $current_term, $total_post);
        } ?>
        ...
        <div class="filter-wrapper">
            <div class="entry-left">
                ...
                <span class="result-count">
                    <?php if (!empty($key)) { ?>
                        <?php printf(_n('%1$s candidate for "%2$s"', '%1$s candidates for "%2$s"', $total_post_for_n, 'jobportal-framework'), '<span>' . $total_post . '</span>', $key); ?>
                    <?php } elseif (is_tax()) { ?>
                        <?php printf(_n('%1$s candidate for "%2$s"', '%1$s candidates for "%2$s"', $total_post_for_n, 'jobportal-framework'), '<span>' . $total_post . '</span>', $current_term_name); ?>
                    <?php } else { ?>
                        <?php printf(_n('%1$s candidate', '%1$s candidates', $total_post_for_n, 'jobportal-framework'), '<span>' . $total_post . '</span>'); ?>
                    <?php } ?>
                </span>
            </div>
        ...
        $pagination_type = jobportal_get_option('candidate_pagination_type');
