...
$candidate_video_url   = isset($candidate_meta_data[JOBPORTAL_METABOX_PREFIX . 'candidate_video_url']) ? $candidate_meta_data[JOBPORTAL_METABOX_PREFIX . 'candidate_video_url'][0] : '';
$candidate_video_image = isset($candidate_meta_data[JOBPORTAL_METABOX_PREFIX . 'candidate_video_image']) ? $candidate_meta_data[JOBPORTAL_METABOX_PREFIX . 'candidate_video_image'][0] : '';
?>
<?php if (!empty($candidate_video_url)) : ?>
    <div class="block-archive-inner candidate-single-field">
        <h4 class="title-candidate"><?php esc_html_e('Video', 'jobportal-framework') ?></h4>
        <div class="entry-candidate-element">
            <div class="entry-thumb-wrap">
                <?php if (wp_oembed_get($candidate_video_url)) : ?>
                    <?php
                    $image_src = jobportal_image_resize_id($candidate_video_image, 870, 420, true);
                    $width = '870';
                    $height = '420';
                    if (!empty($image_src)) : ?>
