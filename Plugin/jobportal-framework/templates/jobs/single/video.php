...
$jobs_video_url   = isset($jobs_meta_data[JOBPORTAL_METABOX_PREFIX . 'jobs_video_url']) ? $jobs_meta_data[JOBPORTAL_METABOX_PREFIX . 'jobs_video_url'][0] : '';
$jobs_video_image = isset($jobs_meta_data[JOBPORTAL_METABOX_PREFIX . 'jobs_video_image']) ? $jobs_meta_data[JOBPORTAL_METABOX_PREFIX . 'jobs_video_image'][0] : '';
?>
<?php if (!empty($jobs_video_url)) : ?>
    <div class="block-archive-inner jobs-video-details">
        <h4 class="title-jobs"><?php esc_html_e('Video', 'jobportal-framework') ?></h4>
        <div class="entry-jobs-element">
            <div class="entry-thumb-wrap">
                <?php if (wp_oembed_get($jobs_video_url)) : ?>
                    <?php
                    $image_src = jobportal_image_resize_id($jobs_video_image, 870, 420, true);
                    $width = '870';
                    $height = '420';
                    if (!empty($image_src)) : ?>
