...
$jobs_item_class[] = 'jobportal-jobs-item';
if (!empty($layout)) {
    $jobs_item_class[] = $layout;
}

if (jobportal_get_expiration_apply($jobs_id) !== 0) {
    ...
    if ($jobs_featured == '1') {
        $jobs_item_class[] = 'jobportal-jobs-featured';
    } else {
        ...
        if ($number_status_urgent > jobportal_get_expiration_apply($jobs_id) && $enable_status_urgent == '1' && $number_status_urgent !== '') {
            $jobs_item_class[] = 'jobportal-jobs-urgent';
        }
    }
}
$jobs_item_class[] = 'jobs-' . $id;
$enable_jobs_des = jobportal_get_option('enable_jobs_show_des');
?>
<div class="<?php echo join(' ', $jobs_item_class); ?>" data-jobid="<?php echo esc_attr($id); ?>">
    <div class="jobs-archive-header">
        ...
        <div class="jobs-header-right">
            <span class="jobs-status"><?php echo jobportal_get_icon_status($jobs_id); ?></span>
    ...
    <div class="jobs-archive-footer">
        <div class="jobs-footer-left">
            ...
            if (
                ($jobs_salary_active && $jobs_salary_show == 'range' && ($jobs_salary_minimum !== '' || $jobs_salary_maximum !== ''))
                || ($jobs_salary_active && $jobs_salary_show == 'starting_amount' && $jobs_minimum_price !== '')
                || ($jobs_salary_active && $jobs_salary_show == 'maximum_amount' && $jobs_maximum_price !== '')
                || ($jobs_salary_active && $jobs_salary_show == 'agree')
            ) : ?>
    ...
    <a class="jobportal-link-item" href="<?php echo get_post_permalink($jobs_id) ?>"></a>
</div>