...
global $current_user;
wp_get_current_user();
wp_enqueue_script(JOBPORTAL_PLUGIN_PREFIX . 'wishlist');
...
$check_package = jobportal_get_field_check_candidate_package('jobs_wishlist');
...
<?php if (is_user_logged_in() && in_array('jobportal_user_candidate', (array)$current_user->roles)) { ?>
    <?php if ($check_package == -1 || $check_package == 0 || ($candidate_paid_submission_type == 'candidate_per_package' && $candidate_package_number_jobs_wishlist < 1)) { ?>
        <a href="#" class="btn-add-to-message btn-add-to-wishlist tooltip <?php echo esc_attr($css_class); ?>"
            data-title="<?php esc_attr_e('Wishlist', 'jobportal-framework') ?>"
    ...
    <?php } else { ?>
        <a href="#" class="jobportal-add-to-wishlist btn-add-to-wishlist tooltip <?php echo esc_attr($css_class); ?>"
           data-jobs-id="<?php echo intval($id) ?>" data-title="<?php esc_attr_e('Wishlist', 'jobportal-framework') ?>">
            <span class="icon-heart">
                <i class="fas fa-heart"></i>
            </span>
        </a>
