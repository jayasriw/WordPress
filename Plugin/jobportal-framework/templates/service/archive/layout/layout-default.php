...
$items_amount = jobportal_get_option('archive_service_items_amount', '12');
wp_enqueue_script(JOBPORTAL_PLUGIN_PREFIX . 'select-location');
wp_enqueue_script(JOBPORTAL_PLUGIN_PREFIX . 'service-archive');
wp_localize_script(
    JOBPORTAL_PLUGIN_PREFIX . 'service-archive',
    'jobportal_service_archive_vars',
    array(
...
$hide_service_top_filter_fields = jobportal_get_option('hide_service_top_filter_fields');
$enable_service_filter_top = jobportal_get_option('enable_service_filter_top');
$service_filter_sidebar_option = jobportal_get_option('service_filter_sidebar_option');
$content_service = !empty($_GET['layout']) ? jobportal_validate_layout($_GET['layout'], 'service') : $content_service;
if ($content_service === false) {
    $content_service = jobportal_get_option('archive_service_layout', 'layout-list');
}
$service_filter_sidebar_option = !empty($_GET['filter']) ? JobPortal_Helper::jobportal_clean(wp_unslash($_GET['filter'])) : $service_filter_sidebar_option;
...
$author          = isset($_GET['service_author']) ? jobportal_clean(wp_unslash($_GET['service_author'])) : '';
...
//Current term - ONLY when actually on a taxonomy page (not from URL parameters)
// Check if we're on a real taxonomy archive page, not just a filtered search
if (is_tax() && !is_search()) {
    ...
    // For non-taxonomy pages (main archive, search, filtered results), keep empty
    $current_term = null;
}
...
$service_location_values = array();
if (isset($_SERVER['QUERY_STRING'])) {
    $query_string = $_SERVER['QUERY_STRING'];
    $params = explode('&', $query_string);

    foreach ($params as $param) {
        if (strpos($param, 'service-location=') === 0) {
            $value = substr($param, strlen('service-location='));
            $value = urldecode($value);
            if (!empty($value)) {
                $service_location_values[] = $value;
            }
        }
    }
}

if (empty($service_location_values) && isset($_GET['service-location'])) {
    $val = $_GET['service-location'];
    if (is_array($val)) {
        foreach ($val as $v) {
            if (!empty($v)) {
                $service_location_values[] = $v;
            }
        }
    } elseif (!empty($val)) {
        $service_location_values = array($val);
    }
}

if (!empty($service_location_values)) {
    $location_ids = array();

    foreach ($service_location_values as $loc) {
        ...
        $location_term = JobPortal_Location_Search::find_location_term($loc, 'service-location');
...
$service_skills_param = isset($_GET['service-skills']) ? jobportal_clean(wp_unslash($_GET['service-skills'])) : '';
if (!empty($service_skills_param)) {
    $ids = $__resolve_terms($service_skills_param, 'service-skills');
    if (!empty($ids)) {
        $tax_query[] = array(
...
$service_language_param = isset($_GET['service-language']) ? jobportal_clean(wp_unslash($_GET['service-language'])) : '';
if (!empty($service_language_param)) {
    $ids = $__resolve_terms($service_language_param, 'service-language');
    if (!empty($ids)) {
        $tax_query[] = array(
...
$service_categories_param = isset($_GET['service-categories']) ? jobportal_clean(wp_unslash($_GET['service-categories'])) : '';
if (!empty($service_categories_param)) {
    $tokens = is_array($service_categories_param) ? $service_categories_param : explode(',', (string) $service_categories_param);
    $cat_ids = array();
    foreach ($tokens as $token) {
        $token = trim((string) $token);
        if ($token === '') {
        ...
        if (preg_match('/^\d+$/', $token)) {
            $term = get_term((int) $token, 'service-categories');
        } else {
            $term = get_term_by('slug', $token, 'service-categories');
            if (!$term || is_wp_error($term)) {
                $term = get_term_by('name', $token, 'service-categories');
            }
        }
        if ($term && !is_wp_error($term)) {
...
$args = apply_filters('jobportal/archive-service/layout-default/query/args', $args);
...
<?php if ($enable_service_filter_top == 1) { ?>
    <?php do_action('jobportal_archive_service_top_filter', $current_term, $total_post); ?>
<?php } ?>

<div class="inner-content container <?php echo join(' ', $class_inner); ?>">
    <div class="col-left">
        <?php if ($service_filter_sidebar_option !== 'filter-right') {
            do_action('jobportal_archive_service_sidebar_filter', $current_term, $total_post);
        } ?>
        ...
        <div class="filter-wrapper">
            <div class="entry-left">
                ...
                <span class="result-count">
                    <?php if (!empty($key)) { ?>
                        <?php printf(_n('%1$s service for "%2$s"', '%1$s services for "%2$s"', $total_post_for_n, 'jobportal-framework'), '<span>' . $total_post . '</span>', $key); ?>
                    <?php } elseif (is_tax()) { ?>
                        <?php printf(_n('%1$s service for "%2$s"', '%1$s services for "%2$s"', $total_post_for_n, 'jobportal-framework'), '<span>' . $total_post . '</span>', $current_term_name); ?>
                    <?php } else { ?>
                        <?php printf(_n('%1$s service', '%1$s services', $total_post_for_n, 'jobportal-framework'), '<span>' . $total_post . '</span>'); ?>
                    <?php } ?>
                </span>
            </div>
        ...
        $pagination_type = jobportal_get_option('service_pagination_type');
