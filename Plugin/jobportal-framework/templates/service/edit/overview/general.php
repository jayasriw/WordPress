...
$service_price = isset($service_meta_data[JOBPORTAL_METABOX_PREFIX . 'service_price']) ? $service_meta_data[JOBPORTAL_METABOX_PREFIX . 'service_price'][0] : '';
$service_time = isset($service_meta_data[JOBPORTAL_METABOX_PREFIX . 'service_number_time']) ? $service_meta_data[JOBPORTAL_METABOX_PREFIX . 'service_number_time'][0] : '';
$service_time_type = isset($service_meta_data[JOBPORTAL_METABOX_PREFIX . 'service_time_type']) ? $service_meta_data[JOBPORTAL_METABOX_PREFIX . 'service_time_type'][0] : '';
$service_language_level = isset($service_meta_data[JOBPORTAL_METABOX_PREFIX . 'service_language_level']) ? $service_meta_data[JOBPORTAL_METABOX_PREFIX . 'service_language_level'][0] : '';
        ...
        <div class="form-group col-md-12">
            ...
            <div class="select2-field">
                <select name="service_categories" class="jobportal-select2">
                    <?php jobportal_get_taxonomy_by_post_id($service_data->ID, 'service-categories', true, false, true, 'service_categories_order'); ?>
                </select>
            </div>
        </div>
    <?php endif; ?>
            ...
            <div class="select2-field">
                <select name="service_time_type" class="jobportal-select2">
                    <option value="hr" <?php if ($service_time_type == 'hr') {
                        echo 'selected';
                    } ?>><?php esc_html_e('Hour', 'jobportal-framework'); ?></option>
                    <option value="day" <?php if ($service_time_type == 'day') {
                        echo 'selected';
                    } ?>><?php esc_html_e('Day', 'jobportal-framework'); ?></option>
                    <option value="week" <?php if ($service_time_type == 'week') {
                        echo 'selected';
                    } ?>><?php esc_html_e('Week', 'jobportal-framework'); ?></option>
                    <option value="month" <?php if ($service_time_type == 'month') {
                        echo 'selected';
                    } ?>><?php esc_html_e('Month', 'jobportal-framework'); ?></option>
                </select>
            </div>
        </div>
    <?php endif; ?>
        ...
        <div class="form-group col-md-6">
            ...
            <div class="form-select">
                <div class="select2-field select2-multiple">
                    <select data-placeholder="<?php esc_attr_e('Select language', 'jobportal-framework'); ?>"
                            multiple="multiple"
                            class="jobportal-select2" name="service_languages">
                        <?php jobportal_get_taxonomy_by_post_id($service_data->ID, 'service-language', false, false, true, 'service_language_order'); ?>
                    </select>
                </div>
            ...
            <div class="select2-field">
                <select name="service_languages_level" class="jobportal-select2">
                    <option value="basic" <?php if ($service_language_level == 'basic') {
                        echo 'selected';
                    } ?>><?php esc_html_e('Basic', 'jobportal-framework'); ?></option>
                    <option value="conversational" <?php if ($service_language_level == 'conversational') {
                        echo 'selected';
                    } ?>><?php esc_html_e('Conversational', 'jobportal-framework'); ?></option>
                    <option value="fluent" <?php if ($service_language_level == 'fluent') {
                        echo 'selected';
                    } ?>><?php esc_html_e('Fluent', 'jobportal-framework'); ?></option>
                    <option value="native" <?php if ($service_language_level == 'native') {
                        echo 'selected';
                    } ?>><?php esc_html_e('Native or Bilingual', 'jobportal-framework'); ?></option>
                    <option value="professional" <?php if ($service_language_level == 'professional') {
                        echo 'selected';
                    } ?>><?php esc_html_e('Professional', 'jobportal-framework'); ?></option>
                </select>
            </div>
        </div>
    <?php endif; ?>
