...
$service_video_url   = isset($service_meta_data[JOBPORTAL_METABOX_PREFIX . 'service_video_url']) ? $service_meta_data[JOBPORTAL_METABOX_PREFIX . 'service_video_url'][0] : '';
$service_video_image = isset($service_meta_data[JOBPORTAL_METABOX_PREFIX . 'service_video_image']) ? $service_meta_data[JOBPORTAL_METABOX_PREFIX . 'service_video_image'][0] : '';
?>
<?php if (!empty($service_video_url)) : ?>
    <div class="jobportal-block-inner block-archive-inner service-video-details">
        <h4 class="title-service"><?php esc_html_e('Video', 'jobportal-framework') ?></h4>
        <div class="entry-service-element">
            <div class="entry-thumb-wrap">
                <?php if (wp_oembed_get($service_video_url)) : ?>
                    <?php
                    $image_src = jobportal_image_resize_id($service_video_image, 870, 420, true);
                    $width = '870';
                    $height = '420';
                    if (!empty($image_src)) : ?>
