...
wp_enqueue_script(JOBPORTAL_PLUGIN_PREFIX . 'candidates-dashboard');
wp_localize_script(
    JOBPORTAL_PLUGIN_PREFIX . 'candidates-dashboard',
    'jobportal_candidates_dashboard_vars',
    array(
        'ajax_url' => JOBPORTAL_AJAX_URL,
        'not_candidates' => esc_html__('No candidates found', 'jobportal-framework'),
    )
);
global $current_user;
...
$users_candidates = get_users(array('role__in' => array('jobportal_user_candidate')));
$user_follow = $company = array();
foreach ($users_candidates as $user) {
    $my_follow = get_user_meta($user->ID, JOBPORTAL_METABOX_PREFIX . 'my_follow', true);
    ...
    <div class="search-dashboard-wrapper">
        <div class="search-left">
            <div class="action-search">
                <input class="search-control" type="text" name="candidates_search" placeholder="<?php esc_attr_e('Find by name', 'jobportal-framework') ?>">
        ...
        <div class="search-right">
            ...
            <div class="select2-field">
				<select class="search-control action-sorting jobportal-select2" name="candidates_sort_by">
                        ...
                        <tr>
                            ...
                            <td class="action-setting">
                                <div class="list-action">
                                    ...
                                    <?php else : ?>
                                        <a href="#" class="action btn-delete tooltip" athour-id="<?php echo esc_attr($author_id) ?>" follow_company="<?php echo $user_follow_athour; ?>" items-id="<?php echo esc_attr($id); ?>" data-title="<?php echo esc_attr__('Delete', 'jobportal-framework') ?>"><i class="far fa-trash-alt"></i></a>
                                    <?php endif; ?>
