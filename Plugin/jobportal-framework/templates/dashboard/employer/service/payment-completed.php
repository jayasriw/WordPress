...
if ($service_payment_method == 1) {
    $jobportal_service_payment->paypal_payment_completed();
} elseif ($service_payment_method == 2) {
    $jobportal_service_payment->stripe_payment_completed();
}elseif ($service_payment_method == 4) {
    $jobportal_service_payment->razor_payment_completed();
}
?>
<div class="jobportal-payment-completed-wrap">
    <div class="inner-payment-completed">
        <?php
        do_action('jobportal_before_service_payment_completed');
        if (isset($_GET['order_id']) && $_GET['order_id'] != '') :
            $order_id = absint(wp_unslash($_GET['order_id']));
            $jobportal_service_order = new JobPortal_service_order();
            $order_meta = $jobportal_service_order->get_service_order_meta($order_id);
            ...
            $service_wire_transfer_card_number = $is_wire_transfer ? jobportal_get_option('service_wire_transfer_card_number', '') : '';
            $service_wire_transfer_card_name = $is_wire_transfer ? jobportal_get_option('service_wire_transfer_card_name', '') : '';
            $service_wire_transfer_bank_name = $is_wire_transfer ? jobportal_get_option('service_wire_transfer_bank_name', '') : '';
            $service_wire_transfer_bank_address = $is_wire_transfer ? jobportal_get_option('service_wire_transfer_bank_address', '') : '';
            $service_wire_transfer_swift_bic = $is_wire_transfer ? jobportal_get_option('service_wire_transfer_swift_bic', '') : '';
            $service_wire_transfer_iban = $is_wire_transfer ? jobportal_get_option('service_wire_transfer_iban', '') : '';
            $service_wire_transfer_routing_number = $is_wire_transfer ? jobportal_get_option('service_wire_transfer_routing_number', '') : '';
            $service_wire_transfer_info = $is_wire_transfer ? jobportal_get_option('service_wire_transfer_info', '') : '';
            ?>
            ...
            <div class="panel panel-default">
                ...
                <ul class="list-group">
                    ...
                    <li class="list-group-item">
                        ...
                        <strong class="pull-right">
                            ...
                            $payment_method_display = isset($order_meta['service_order_payment_method'])
                                ? JobPortal_service_order::get_service_order_payment_method($order_meta['service_order_payment_method'])
                    ...
                    <li class="list-group-item">
                        ...
                        <strong class="pull-right"><?php echo isset($order_meta['service_order_item_price']) ? jobportal_get_format_money($order_meta['service_order_item_price']) : ''; ?></strong>
                    </li>
                </ul>
            </div>
        ...
        <?php else : ?>
            ...
            <a href="<?php echo jobportal_get_permalink('dashboard'); ?>" class="jobportal-button"> <?php esc_html_e('Go to Dashboard', 'jobportal-framework'); ?> </a>
        <?php endif;
        do_action('jobportal_after_service_payment_completed');
        ?>
    </div>
</div>
