...
if ($data_applicants->have_posts()) {
	while ($data_applicants->have_posts()) : $data_applicants->the_post();
		...
		if (! empty($jobs_filter_id)) {
			$jobs_filter[] = get_post_meta($jobs_filter_id, JOBPORTAL_METABOX_PREFIX . 'applicants_jobs_id')[0];
		}
	endwhile;
}
...
<div class="entry-my-page applicants-dashboard mettings-action-dashboard">
	<div class="entry-title">
		<h4><?php esc_html_e('All applicants', 'jobportal-framework') ?></h4>
	</div>
	<div class="search-dashboard-wrapper">
		<div class="search-left">
			<div class="select2-field">
				<select class="search-control jobportal-select2" name="applicants_filter_jobs">
					<option value=""><?php esc_html_e('All Jobs', 'jobportal-framework') ?></option>
					<?php if ($data_jobs_filter->have_posts() && ! empty($jobs_employer_id) && !empty($jobs_filter)) { ?>
			...
			<div class="action-search">
				<input class="search-control" type="text" name="applicants_search"
					placeholder="<?php esc_attr_e('Find by jobs', 'jobportal-framework') ?>">
	...
	if ($total_post > $posts_per_page && ! empty($jobs_employer_id)) { ?>
		<div class="pagination-dashboard">
			...
			jobportal_get_template('global/pagination.php', array(
