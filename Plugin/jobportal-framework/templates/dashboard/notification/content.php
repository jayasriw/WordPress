...
<?php if (!empty($data_notification)) { ?>
    <div class="head-noti">
        <div class="head-left">
            <span class="noti-refresh">
                ...
                <?php esc_html_e('Refresh', 'jobportal-framework'); ?>
            </span>
            <span class="noti-clear">
                <i class="far fa-trash-alt"></i>
                <?php esc_html_e('Clear All', 'jobportal-framework'); ?>
            </span>
        </div>
    ...
    <ul>
        <?php foreach ($data_notification as $data) {
            $post_id = $data->ID;
            $user_send_id = get_post_meta($post_id, JOBPORTAL_METABOX_PREFIX . 'user_send_noti', true);
            ...
            $user_send = jobportal_get_user_display_name($user_send_id);
            // Check if notification is read (must be exactly '1')
            $is_read = get_post_meta($post_id, JOBPORTAL_METABOX_PREFIX . 'notification_read', true) === '1';

            // Get notification message
            $mess_noti = get_post_meta($post_id, JOBPORTAL_METABOX_PREFIX . 'mess_noti', true);
            if (empty($mess_noti)) {
                $mess_noti = esc_html__('Notification', 'jobportal-framework');
            }
            ...
            if ($post_time) {
                ...
                $time = esc_html__('recently', 'jobportal-framework');
            }

            $link = get_post_meta($post_id, JOBPORTAL_METABOX_PREFIX . 'link_post_noti', true);
            ...
            $page_link = get_post_meta($post_id, JOBPORTAL_METABOX_PREFIX . 'link_page_noti', true);
            ...
            <li class="<?php echo $is_read ? '' : 'is-unread'; ?>">
                ...
                <span class="content-wrapper">
                    <span class="content">
                        ...
                        $notification_message .= ' ' . esc_html__('by', 'jobportal-framework') . ' ';
                    ...
                    <span class="date">
                        <?php echo sprintf(esc_html__('%s ago', 'jobportal-framework'), esc_html($time)); ?>
                    </span>
                </span>
                <?php if (!$is_read) : ?>
                    <a href="#" class="btn-quick-read" data-noti-id="<?php echo esc_attr($post_id); ?>" title="<?php esc_attr_e('Mark as Read', 'jobportal-framework'); ?>">
                        <span class="unread-dot"></span>
                    </a>
                <?php endif; ?>
                <span class="action action-setting">
                    <a href="#" class="icon-setting-jobportal"><i class="fal fa-ellipsis-v"></i></a>
                    <span class="action-dropdown">
                        <?php if (!$is_read) : ?>
                            <a class="btn-mark-read" data-noti-id="<?php echo esc_attr($post_id); ?>" href="#">
                                <i class="far fa-check"></i>
                                <?php esc_html_e('Mark as Read', 'jobportal-framework'); ?>
                            </a>
                        <?php else : ?>
                            <a class="btn-mark-unread" data-noti-id="<?php echo esc_attr($post_id); ?>" href="#">
                                <i class="far fa-circle"></i>
                                <?php esc_html_e('Mark as Unread', 'jobportal-framework'); ?>
                            </a>
                        <?php endif; ?>
                        <a class="btn-delete" data-noti-id="<?php echo esc_attr($post_id); ?>" href="#">
                            <i class="far fa-trash-alt"></i>
                            <?php esc_html_e('Delete', 'jobportal-framework'); ?>
                        </a>
                    </span>
                </span>
...
<?php } else { ?>
    <span class="empty"><?php esc_html_e('You do not have any notifications.', 'jobportal-framework'); ?></span>
<?php } ?>
