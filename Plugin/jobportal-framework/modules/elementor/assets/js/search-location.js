(function ($) {
  ...
  const HorizontalSearchHandler = function ($scope, $) {
    const map_api_key = jobportal_template_vars.map_api_key;
    const map_type = jobportal_template_vars.map_type;

    const search_form = $scope.find(".jobportal-form-location"),
      input = search_form.find(".input-search-location"),
      archive_input = search_form.find(".archive-search-location"),
      field_select = search_form.find(".jobportal-select2");
    ...
    if (archive_input.length && !archive_input.hasClass('jobportal-ajax-ui')) {
