 ...
 * @since emo 1.0
 */
if (!class_exists('JOBPORTAL_Widget_Areas')) {
	class JOBPORTAL_Widget_Areas
	{
		...
		public function __construct()
		{
			if (is_admin()) {
				...
				add_action('wp_ajax_jobportal_delete_widget_area', array($this, 'delete_widget_area'));
			}
		...
		{
			wp_enqueue_script(JOBPORTAL_PLUGIN_PREFIX . 'widget-areas', plugins_url(JOBPORTAL_PLUGIN_NAME . '/widgets/widget-areas/assets/js/widget-areas.min.js'), array('jquery'), $this->version);
			wp_enqueue_style(JOBPORTAL_PLUGIN_PREFIX . 'widget-areas', plugins_url(JOBPORTAL_PLUGIN_NAME . '/widgets/widget-areas/assets/css/widget-areas.min.css'), array(), $this->version, 'screen');
			wp_localize_script(
				JOBPORTAL_PLUGIN_PREFIX . 'widget-areas',
				'jobportal_widget_areas_variable',
				array(
					'ajax_url' => admin_url('admin-ajax.php'),
					'confirm_delete' => esc_html__('Are you sure to delete this widget areas?', 'jobportal-framework')
				)
			);
		}
		...
		public function register_custom_widget_areas()
		{
			...
			$args = apply_filters('jobportal_custom_widget_args', $args);
		...
		public function add_widget_area_area()
		{
			if (!empty($_POST['bin-add-widget-input'])) {
				// Security checks
				if (!isset($_POST['_wpnonce']) || !check_ajax_referer('bin-add-widget-area-nonce', '_wpnonce', false)) {
