...
if (!class_exists('GLF_Theme_Options')) {
	class GLF_Theme_Options
	{
		...
		public function binder_page()
		{
			...
			$page = jobportal_clean(wp_unslash($_GET['page']));
		...
		public function save_theme_options()
		{
			...
			try {
				$page = isset($_POST['_current_page']) ? $_POST['_current_page'] : '';
				$configs = &glf_get_options_config($page);
				if (empty($configs) || !is_array($configs) || !isset($configs['option_name'])) {
					throw new Exception('Invalid options config.');
				}
				$options_name = $configs['option_name'];

				if (!isset($_POST['_wpnonce']) || !wp_verify_nonce($_POST['_wpnonce'], $configs['option_name'])) {
					return;
				}
				...
				$message = esc_html__('Settings saved', 'jobportal-framework');
				add_settings_error('gf-options', 'update-options', $message, 'updated');
			} catch (Exception $e) {
				$message = esc_html__('There has been an error when update options', 'jobportal-framework');
