...
if (!class_exists('GLF_Field_Map')) {
	class GLF_Field_Map extends GLF_Field
	{
		public function enqueue()
		{
			...
			$map_script_deps = array();

			if ($map_type == 'mapbox') {
				wp_enqueue_script($pfx . 'mapbox-promise', GLF_BASE_URL . 'assets/libs/mapbox/es6-promise.min.js', array(), GLF_VER, true);
				wp_enqueue_script($pfx . 'mapbox-promise-auto', GLF_BASE_URL . 'assets/libs/mapbox/es6-promise.auto.min.js', array($pfx . 'mapbox-promise'), GLF_VER, true);
				wp_enqueue_script($pfx . 'mapbox', GLF_BASE_URL . 'assets/libs/mapbox/mapbox-gl.js', array($pfx . 'mapbox-promise-auto'), GLF_VER, true);
				wp_enqueue_script($pfx . 'mapbox-geocoder', GLF_BASE_URL . 'assets/libs/mapbox/mapbox-gl-geocoder.min.js', array($pfx . 'mapbox'), GLF_VER, true);
				wp_enqueue_style($pfx . 'mapbox', GLF_BASE_URL . 'assets/libs/mapbox/mapbox-gl.css', array(), GLF_VER);
				wp_enqueue_style($pfx . 'mapbox-geocoder', GLF_BASE_URL . 'assets/libs/mapbox/mapbox-gl-geocoder.css', array(), GLF_VER);
				$map_script_deps = array($pfx . 'mapbox-geocoder');
			}

			if ($map_type == 'openstreetmap') {
				wp_enqueue_script($pfx . 'leaflet', GLF_BASE_URL . 'assets/libs/leaflet/leaflet.js', array(), GLF_VER, true);
				wp_enqueue_script($pfx . 'leaflet-src', GLF_BASE_URL . 'assets/libs/leaflet/leaflet-src.js', array(), GLF_VER, true);
				wp_enqueue_script($pfx . 'esri-leaflet', GLF_BASE_URL . 'assets/libs/leaflet/esri-leaflet.js', array(), GLF_VER, true);
				wp_enqueue_script($pfx . 'esri-leaflet-geocoder', GLF_BASE_URL . 'assets/libs/leaflet/esri-leaflet-geocoder.js', array(), GLF_VER, true);
				wp_enqueue_style($pfx . 'leaflet', GLF_BASE_URL . 'assets/libs/leaflet/leaflet.css', array(), GLF_VER);
				wp_enqueue_style($pfx . 'esri-leaflet-geocoder', GLF_BASE_URL . 'assets/libs/leaflet/esri-leaflet-geocoder.css', array(), GLF_VER);
				$map_script_deps = array('google-map', $pfx . 'esri-leaflet-geocoder');
			}

			if ($map_type == 'google_map') {
				$map_script_deps = array('google-map');
			}

			wp_enqueue_script($pfx . 'map', GLF_BASE_URL . 'fields/map/assets/map.js', $map_script_deps, '1.4.9', true);
			wp_enqueue_style($pfx . 'map', GLF_BASE_URL . 'fields/map/assets/map.css', array(), '1.4.6');
		}
		...
		function render_content($content_args = '')
		{
			...
			$googlemap_type = jobportal_get_option('googlemap_type', 'roadmap');
			...
			$mapbox_api_key = jobportal_get_option('mapbox_api_key', 'pk.eyJ1Ijoic2F5aTc3NDciLCJhIjoiY2tpcXRmYW1tMWpjMjJzbGllbThieTFlaCJ9.eDj6zNLBZpG-veFqXiyVPw');
			$openstreetmap_api_key = jobportal_get_option('openstreetmap_api_key', 'pk.eyJ1Ijoic2F5aTc3NDciLCJhIjoiY2tpcXRmYW1tMWpjMjJzbGllbThieTFlaCJ9.eDj6zNLBZpG-veFqXiyVPw');
			...
			$placeholder = isset($this->params['placeholder']) ? $this->params['placeholder'] : esc_html__('Enter an address...', 'jobportal-framework');
?>
			<div class="glf-field-map-inner">
				...
				<?php if ((!isset($this->params['show_address']) || $this->params['show_address']) && $map_type == 'google_map') : ?>
					<div class="glf-map-address">
						...
						<button type="button" class="button"><?php echo esc_html__('Find Address', 'jobportal-framework'); ?></button>
