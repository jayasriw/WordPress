		...
		function render_content($content_args = '')
		{
			...
			<div class="glf-field-select-inner">
				<select data-field-control="" class="glf-select"
                    ...
                    >
					<?php
						if( $this->params['id'] === 'redirect_for_admin' || $this->params['id'] === 'redirect_for_candidate' || $this->params['id'] === 'redirect_for_employer' ) {
							?>
							<option value="reload" <?php glf_the_selected('reload', $field_value) ?>><?php esc_html_e('Reload Page', 'jobportal-framework'); ?></option>
							<?php
						}
					?>
							...
							<optgroup label="<?php echo esc_attr($key); ?>">
								<?php foreach ($value as $opt_key => $opt_value): ?>
									<option <?php glf_the_selected($opt_key, $field_value) ?>
										value="<?php echo esc_attr($opt_key); ?>"><?php esc_html_e($opt_value); ?></option>
								<?php endforeach; ?>
							</optgroup>
						<?php else:; ?>
