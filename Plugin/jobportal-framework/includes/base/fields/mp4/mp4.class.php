...
if (!class_exists('GLF_Field_Mp4')) {
	class GLF_Field_Mp4 extends GLF_Field
	{
		function enqueue()
		{
			...
			wp_localize_script(GLF_BASE_RESOURCE_PREFIX . 'file', 'sfFileFieldMeta', array(
				'title'   => esc_html__('Select File', 'jobportal-framework'),
				'button'  => esc_html__('Use these files', 'jobportal-framework')
			));
		}
		...
		function render_content($content_args = '')
		{
			...
			$remove_text = esc_html__('Remove', 'jobportal-framework');
?>
			<div class="glf-field-file-inner" data-remove-text="<?php echo esc_attr($remove_text); ?>" <?php if (isset($this->params['lib_filter']) && !empty($this->params['lib_filter'])) : ?> data-lib-filter="<?php echo esc_attr($this->params['lib_filter']) ?>" <?php endif; ?>>
				...
				<div class="glf-file-add">
					<button class="button" type="button"><?php esc_html_e('+ Add File', 'jobportal-framework'); ?></button>
				</div>
			</div>
<?php
		}
