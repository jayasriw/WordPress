...
if (!class_exists('JobPortal_Admin_Service')) {
    /**
     * Class JobPortal_Admin_Service
     */
    class JobPortal_Admin_Service
    {
        /**
        ...
        public function register_custom_column_titles($columns)
        {
            ...
            $columns['thumb']  = esc_html__('Thumbnail', 'jobportal-framework');
        ...
        public function display_custom_column($column)
        {
            ...
            switch ($column) {
                case 'thumb':
                    ...
                    break;
                case 'skills':
                    echo jobportal_admin_taxonomy_terms($post->ID, 'service-skills', 'service');
                    break;
                case 'author':
        ...
        public function modify_list_row_actions($actions, $post)
        {
            ...
            if ($post->post_type == 'service') {
                if (in_array($post->post_status, array('pending'))) {
                    $actions['service-approve'] = '<a href="' . wp_nonce_url(add_query_arg('approve_service', $post->ID), 'approve_service') . '">' . esc_html__('Approve', 'jobportal-framework') . '</a>';
                }
            }
            return $actions;
        }
        ...
        public function approve_service()
        {
            if (!empty($_GET['approve_service']) && wp_verify_nonce($_REQUEST['_wpnonce'], 'approve_service') && current_user_can('publish_post', $_GET['approve_service'])) {
                $post_id = absint(jobportal_clean(wp_unslash($_GET['approve_service'])));
                $listing_data = array(
                    'ID' => $post_id,
                    'post_status' => 'publish'
                );
                wp_update_post($listing_data);
                update_post_meta($post_id, JOBPORTAL_METABOX_PREFIX . 'enable_service_package_expires', 0);
                update_post_meta($post_id, JOBPORTAL_METABOX_PREFIX . 'service_featured', 0);
        ...
        {
            $service_skills_url_slug = jobportal_get_option('service_skills_url_slug');
        ...
        {
            $service_location_url_slug = jobportal_get_option('service_location_url_slug');
        ...
        {
            $service_language_url_slug = jobportal_get_option('service_language_url_slug');
        ...
        {
            $service_url_slug = jobportal_get_option('service_url_slug');
        ...
        {
            $service_tags_url_slug = jobportal_get_option('service_tags_url_slug');
