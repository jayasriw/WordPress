...
if (!class_exists('JobPortal_Admin_candidate_package')) {
    /**
     * Class JobPortal_Admin_candidate_package
     */
    class JobPortal_Admin_candidate_package
    {
        /**
        ...
        {
            $candidate_package_url_slug = jobportal_get_option('candidate_package_url_slug');
        ...
        public function register_custom_column_titles($columns)
        {
            ...
            $columns['title']    = esc_html__('Name', 'jobportal-framework');
            ...
            $custom_order = ['cb', 'title', 'price', 'featured', 'date'];
            $new_columns = [];
        ...
        public function display_custom_column($column)
        {
            ...
            switch ($column) {
                case 'price':
                    $candidate_package_free = get_post_meta($post->ID, JOBPORTAL_METABOX_PREFIX . 'candidate_package_free', true);
                    if ($candidate_package_free == 1) {
                        esc_html_e('Free', 'jobportal-framework');
                    } else {
                        $candidate_package_price = get_post_meta($post->ID, JOBPORTAL_METABOX_PREFIX . 'candidate_package_price', true);
                        if ($candidate_package_price > 0) {
                            echo jobportal_get_format_money($candidate_package_price);
                        } else {
                            esc_html_e('Free', 'jobportal-framework');
                        }
                    }
                    ...
                    break;
                case 'featured':
                    $featured = get_post_meta($post->ID, JOBPORTAL_METABOX_PREFIX . 'candidate_package_featured', true);
                    if ($featured == 1) {
