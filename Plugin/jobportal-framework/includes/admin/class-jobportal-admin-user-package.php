...
if (!class_exists('JobPortal_User_Package_Admin')) {
    /**
     * Class JobPortal_User_Package_Admin
     */
    class JobPortal_User_Package_Admin
    {
        /**
        ...
        public function register_custom_column_titles($columns)
        {
            ...
            $columns['title']          = esc_html__('ID', 'jobportal-framework');
            $columns['user_id']        = esc_html__('Name', 'jobportal-framework');
            $columns['package']        = esc_html__('Package', 'jobportal-framework');
            $columns['num_job']        = esc_html__('Number Jobs', 'jobportal-framework');
            $columns['num_featured']   = esc_html__('Number Featured', 'jobportal-framework');
            $columns['activate_date']  = esc_html__('Activate Date', 'jobportal-framework');
            $columns['expire_date']    = esc_html__('Expires Date', 'jobportal-framework');
        ...
        public function display_custom_column($column)
        {
            ...
            $package_num_job = get_user_meta($package_user_id, JOBPORTAL_METABOX_PREFIX . 'package_number_job', true);
            $package_num_featured_job = get_user_meta($package_user_id, JOBPORTAL_METABOX_PREFIX . 'package_number_featured', true);
            ...
            $expired_date = $jobportal_package->get_expired_date($package_id, $package_user_id);
        ...
        {
            $user_package_url_slug = jobportal_get_option('user_package_url_slug');
        ...
        public function filter_restrict_manage_user_package()
        {
            ...
            if ($typenow == $post_type) { ?>
                <input type="text" placeholder="<?php esc_html_e('Buyer', 'jobportal-framework'); ?>" name="package_user" value="<?php echo (isset($_GET['package_user']) ? esc_attr(jobportal_clean(wp_unslash($_GET['package_user']))) : ''); ?>">
<?php }
        }
        ...
        public function user_package_filter($query)
        {
            ...
            if ($pagenow == 'edit.php' && isset($q_vars['post_type']) && $q_vars['post_type'] == $post_type) {
                if (isset($_GET['package_user']) && $_GET['package_user'] != '') {
                    $user = get_user_by('login', jobportal_clean(wp_unslash($_GET['package_user'])));
                    $user_id = -1;
                    if ($user) {
                        $user_id = $user->ID;
                    }
        ...
        public function action_delete_post()
        {
            ...
            if ('user_package' == get_post_type($postID)) {
                $package_user_id = get_post_meta($postID, JOBPORTAL_METABOX_PREFIX . 'package_user_id', true);
                update_user_meta($package_user_id, JOBPORTAL_METABOX_PREFIX . 'package_id', '');
                update_user_meta($package_user_id, JOBPORTAL_METABOX_PREFIX . 'paypal_transfer', '');
                update_user_meta($package_user_id, JOBPORTAL_METABOX_PREFIX . 'free_package', '');
            }
        }
    }
}
