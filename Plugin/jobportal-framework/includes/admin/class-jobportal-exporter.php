...
/**
 * JobPortal Exporter Class
 */
class JobPortal_Exporter
{

	/**
	 * Instance
	 *
	 * @var JobPortal_Exporter The single instance of the class.
	 */
	 ...
	 *
	 * @return JobPortal_Exporter An instance of the class.
	 */
	...
	public static function get_export_items()
	{
		...
		$export_items = array(
			array(
				'name'        => esc_html__('Content', 'jobportal-framework'),
				'action'      => 'export_content',
				'icon'        => 'lab la-wordpress-simple',
				'description' => esc_html__('Create an XML file containing your posts, pages, comments, custom fields, categories, and tags', 'jobportal-framework'),
			),
			array(
				'name'        => esc_html__('Widgets', 'jobportal-framework'),
				'action'      => 'export_widgets',
				'icon'        => 'las la-shapes',
				'description' => esc_html__('Create a text file containing your widgets', 'jobportal-framework'),
			),
			array(
				'name'        => esc_html__('Customizer Settings', 'jobportal-framework'),
				'action'      => 'export_customizer_settings',
				'icon'        => 'las la-tools',
				'description' => esc_html__('Create a text file containing your customizer settings (in Appearance > Customize)', 'jobportal-framework'),
			),

			array(
				'name'        => esc_html__('Theme Options', 'jobportal-framework'),
				'action'      => 'export_theme_options',
				'icon'        => 'lab la-themeisle',
				'description' => esc_html__('Create a text file containing your theme options', 'jobportal-framework'),
			),

			array(
				'name'        => esc_html__('Elementor', 'jobportal-framework'),
				'action'      => 'export_elementor',
				'icon'        => 'lab la-elementor',
				'description' => esc_html__('Create a text file containing your elementor', 'jobportal-framework'),
			),

			array(
				'name'        => esc_html__('Menus', 'jobportal-framework'),
				'action'      => 'export_menus',
				'icon'        => 'las la-bars',
				'description' => esc_html__('Create a text file containing your menus', 'jobportal-framework'),
			),
			array(
				'name'        => esc_html__('Page Options', 'jobportal-framework'),
				'action'      => 'export_page_options',
				'icon'        => 'lar la-file-alt',
				'description' => esc_html__('Create a text file containing the Homepage & Post Page settings', 'jobportal-framework'),
			),
		);

		if (class_exists('ZipArchive')) {
			$export_items[] = array(
				'name'              => esc_html__('Media Package', 'jobportal-framework'),
				'action'            => 'export_media_package',
				'icon'              => 'lar la-image',
				'input_file_name'   => true,
				'default_file_name' => 'media-01',
				'description'       => esc_html__('Create a zip package containing all files in the wp-content/uploads directory', 'jobportal-framework'),
			);
		}

		return apply_filters('jobportal_export_items', $export_items);
	}
	...
	protected function get_allowed_exts()
	{
		...
		$allowed_exts = apply_filters('jobportal_export_allowed_exts', $allowed_exts);
	...
	public function export_content()
	{
		...
		if (!verify_nonce('export_content')) {
			wp_die(esc_html__('Invalid nonce', 'jobportal-framework'));
		}
	...
	public function export_widgets()
	{
		...
		$data = apply_filters('jobportal_export_widgets_data', $sidebars_widget_instances);
	...
	public function export_customizer_settings()
	{
		...
		$data = apply_filters('jobportal_export_customizer_settings_data', $data);
	...
	public function export_theme_options()
	{
		...
		$data = get_option('jobportal-framework');

		$data = apply_filters('jobportal_export_theme_options_data', $data);
	...
	public function export_elementor()
	{
		...
		$elementor_options = apply_filters('jobportal_export_elementor_data', $elementor_options);
	...
	public function export_menus()
	{
		...
		$data = apply_filters('jobportal_export_menus_data', $data);
	...
	public function export_page_options()
	{
		...
		$data = apply_filters('jobportal_export_page_options_data', $data);
	...
	public function export_media_package()
	{
		...
		$file_name = apply_filters('jobportal_export_archive_file_name', $file_name);
		...
		if (!is_writeable(WP_CONTENT_DIR)) {
			wp_die(__('Could not write files into \'wp-content\' directory, permission denined.', 'jobportal-framework')); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		}

		// Action: jobportal_before_create_media_package.
		do_action('jobportal_before_create_media_package');

		if (!$this->create_media_package($source, $file_path)) {
			wp_die(esc_html__('Could not create the media package, please try again.', 'jobportal-framework'));
		}

		// Action: jobportal_after_create_media_package.
		do_action('jobportal_after_create_media_package');
		...
		if (file_exists($file_path)) {
			...
			if (!readfile("{$file_path}")) { // phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_read_readfile
				...
				wp_die(sprintf(esc_html__('Could not read file %s.', 'jobportal-framework'), esc_html($file_path)));
			}
			...
			// translators: $s: Archive file name.
			wp_die(sprintf(esc_html__('The file %s does not exists.', 'jobportal-framework'), esc_html($file_path)));
		}
	}
	...
	protected function create_media_package($source, $dest)
	{
		...
		if (!class_exists('ZipArchive')) {
			wp_die(__('Could not create zip file. The extension \'ZipArchive\' is not enabled.', 'jobportal-framework')); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		}
	...
	{
		$result      = false;
		$skip_prefix = apply_filters('jobportal_placeholder_skip_prefix', '__');
