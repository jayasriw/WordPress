 ...
 * Update theme
 */
if (!class_exists('JobPortal_Updater')) {

	class JobPortal_Updater
	{

		public static $info = array(
			'support' => 'https://uxper.ticksy.com/',
			'faqs'    => 'https://uxper.gitbook.io/jobportal-job-board-wordpress-theme/',
			'docs'    => 'https://uxper.gitbook.io/jobportal-job-board-wordpress-theme/',
			'api'     => 'https://data.uxper.co/' . JOBPORTAL_THEME_SLUG . '/update/',
			...
			'tf'      => 'https://themeforest.net/item/jobportal-city-guide-wordpress-theme/25397810',
		);

		/**
		 * Remote destination for theme updates
		 *
		 * @var string
		 */
		public $remote_destination = '';
		...
		public function __construct()
		{
			...
			add_action('wp_ajax_jobportal_patcher', array($this, 'ajax_patcher'));
		...
		public function check_for_update($transient)
		{
			...
			if ($update) {
				...
				$transient->response[JOBPORTAL_THEME_SLUG] = $response;

				// If the purchase code is valide, user can get the update package
				if (self::check_valid_update()) {
					$transient->response[JOBPORTAL_THEME_SLUG]['package'] = $update['package'];
				} else {
					unset($transient->response[JOBPORTAL_THEME_SLUG]['package']);
				}
			}
		...
		public function rename_theme_folder_after_upgrade()
		{
			...
			if (!empty($this->remote_destination) && get_theme_root() === dirname($this->remote_destination) && file_exists($this->remote_destination)) {
				rename($this->remote_destination, JOBPORTAL_THEME_DIR);
			}
		}
		...
		public static function check_theme_patcher()
		{
			...
			if (isset($patchers[JOBPORTAL_THEME_VERSION]) && (count($patchers[JOBPORTAL_THEME_VERSION]) > 0)) {
				$patchers_status = (array) get_option('uxper_patcher');
				foreach ($patchers[JOBPORTAL_THEME_VERSION] as $key => $value) {
		...
		public function ajax_patcher()
		{
			...
			@ob_flush();
			@flush();
			if (is_writable(JOBPORTAL_THEME_DIR)) {
				$package = download_url($uxper_patcher_url, 18000);
				$unzip   = unzip_file($package, JOBPORTAL_THEME_DIR);
		...
		public static function check_purchase_code($code)
		{
			...
			$userAgent = "Purchase code verification on " . JOBPORTAL_THEME_NAME;
			...
			if (!preg_match("/^([a-f0-9]{8})-(([a-f0-9]{4})-){3}([a-f0-9]{12})$/i", $code)) {
				$message = esc_html__('Invalid code', 'jobportal-framework');
			}
			...
			if (curl_errno($ch) > 0) {
				$message = esc_html__('Error connecting to API', 'jobportal-framework');
			}
			...
			if ($responseCode === 404) {
				$message = esc_html__('The purchase code was invalid', 'jobportal-framework');
			}
			...
			if ($responseCode !== 200) {
				$message = esc_html__('Failed to validate code due to an error: HTTP {$responseCode}', 'jobportal-framework');
			}
			...
			if ($body === false && json_last_error() !== JSON_ERROR_NONE) {
				$message = esc_html__('Error parsing response', 'jobportal-framework');
			}
			...
			if (self::$info['api']) {
				...
				if (is_array($updates)) {
					foreach ($updates as $ukey => $uval) {
						if (version_compare($ukey, JOBPORTAL_THEME_VERSION) == 1) {
