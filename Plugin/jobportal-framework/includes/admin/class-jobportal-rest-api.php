...
if (!class_exists('JobPortal_Rest_API')) {

	/**
	 * Class JobPortal_Rest_API
	 */
	class JobPortal_Rest_API
	{
		...
		public function register_fields_api()
		{
			...
			register_rest_route('global', 'countries', array(
				'methods'  => 'GET',
				'callback' => function () {
					return jobportal_get_countries();
				},
			...
			register_rest_field(
				...
				array(
					...
					'update_callback' => function ($value, $object) {
						if (!empty($value)) {
							$object->set_featured_media($value);
						} else {
							$object->set_featured_media(null);
						}
						return $object;
					},
					'schema' => array(
						'description' => 'Jobs featured media url',
						'type' => 'string',
			...
			foreach ($meta_box_field_keys as $meta_id => $field_value) {
				...
				register_rest_field(
					...
					array(
						...
						'update_callback' => function ($value, $object, $field_name) use ($meta_id) {
							// Update field in post meta.
							return update_post_meta($object->ID, $meta_id, $value);
						},
						'schema' => array(
							'description' => 'Jobs ' . $meta_id,
							'type' => 'string',
							'context' => array('view', 'edit')
						),
					)
			...
			$jobs_gallery = JOBPORTAL_METABOX_PREFIX . 'jobs_images';
			register_rest_field(
				'jobs',
				$jobs_gallery,
				array(
					'get_callback' => function ($object) use ($jobs_gallery) {
						$jobs_gallery = get_post_meta($object['id'], JOBPORTAL_METABOX_PREFIX . 'jobs_images', true);
					...
					'update_callback' => function ($value, $object) use ($jobs_gallery) {
						// Update field in post meta.
						return update_post_meta($object->ID, $jobs_gallery, $value);
					},
					'schema' => array(
						'description' => 'Jobs gallery images',
						'type' => 'string',
						'context' => array('view', 'edit')
					)
				)
			...
			// Add jobs rating
			$jobs_rating = JOBPORTAL_METABOX_PREFIX . 'jobs_rating';
			register_rest_field(
				'jobs',
				$jobs_rating,
				array(
					...
					'update_callback' => function ($value, $object) use ($jobs_rating) {
						// Update field in post meta.
						return update_post_meta($object->ID, $jobs_rating, $value);
					},
					'schema' => array(
						'description' => 'Jobs rating',
						'type' => 'string',
						'context' => array('view', 'edit')
					)
				)
			...
			$review_count = JOBPORTAL_METABOX_PREFIX . 'review_count';
			register_rest_field(
				'jobs',
				$review_count,
				array(
					...
					'update_callback' => function ($value, $object) use ($review_count) {
						// Update field in post meta.
						return update_post_meta($object->ID, $review_count, $value);
					},
					'schema' => array(
						'description' => 'Jobs rating',
						'type' => 'string',
			...
			register_rest_field(
				...
				array(
					...
					'update_callback' => function ($value, $object) use ($comment_rating) {
						// Update field in post meta.
						return update_comment_meta($object->ID, $comment_rating, $value);
					},
					'schema' => array(
						'description' => 'Jobs rating',
						'type' => 'string',
					...
					array(
						'get_callback' => function ($object) use ($meta_id) {
							...
							if ($meta_id == 'jobs_city_country') {
								...
								$country_name = jobportal_get_country_by_code($country);
						...
						'update_callback' => function ($value, $object) use ($meta_id) {
							// Update field in post meta.
							return update_term_meta($object->ID, $meta_id, $value);
						},
						'schema' => array(
							'description' => $meta_id,
							'type' => 'string',
			...
			register_rest_field(
				...
				array(
					...
					'update_callback' => function ($value, $object) {
						// Update field in post meta.
						return update_term_meta($object->ID, 'jobs_categories_icon_marker', $value);
					},
					'schema' => array(
						'description' => 'Icon marker url',
						'type' => 'string',
						'context' => array('view', 'edit')
					)
				)
			...
			$term_meta_user_field_keys = array(
				...
				JOBPORTAL_METABOX_PREFIX . 'author_mobile_number',
				JOBPORTAL_METABOX_PREFIX . 'my_wishlist',
				JOBPORTAL_METABOX_PREFIX . 'my_follow',
			);
				...
				register_rest_field(
					...
					array(
						'get_callback' => function ($object) use ($meta_id) {
							...
							if ($meta_id == JOBPORTAL_METABOX_PREFIX . 'my_wishlist') {
								// Get field as single value from post meta.
								return get_user_meta($object['id'], $meta_id, true);
							}

							if ($meta_id == JOBPORTAL_METABOX_PREFIX . 'my_follow') {
								// Get field as single value from post meta.
								return get_user_meta($object['id'], $meta_id, true);
							}
						...
						'update_callback' => function ($value, $object) use ($meta_id) {
							// Update field in post meta.
							return update_user_meta($object->ID, $meta_id, $value);
						},
						'schema' => array(
							'description' => $meta_id,
							'type' => 'string',
		...
		public function rest_add_to_wishlist($request = [])
		{
			$response = [
				'success' => false,
				'message' => __('You are not login', 'jobportal-framework')
			];
			$status_code = 403;
			...
			if ($user_id > 0) {
				...
				if ($added) {
					$response = [
						'success' => true,
						'message' => __('Added', 'jobportal-framework')
					];
					$status_code = 200;
				}
				if ($removed) {
					$response = [
						'success' => true,
						'message' => __('Removed', 'jobportal-framework')
					];
					$status_code = 200;
				}
			} else {
		...
		public function rest_user_login($request = [])
		{
			$response = [
				'success' => false,
				'message' => __('Login failed', 'jobportal-framework')
			];
			$status_code = 403;
			$parameters = $request->get_json_params();
			...
			if ($user instanceof WP_User) {
				...
				$response['message'] = __('Login successful', 'jobportal-framework');
				$status_code = 200;
			}
		...
		public function rest_user_register($request = [])
		{
			$response = [
				'success' => false,
				'message' => __('Register failed', 'jobportal-framework')
			];
			$status_code = 403;
			$parameters = $request->get_json_params();
			...
			if (!empty($firstname) && !empty($lastname) && !empty($email) && !empty($password)) {
				...
				if (!is_wp_error($user_id)) {
					$response['success'] = true;
					$response['message'] = __('Register successful', 'jobportal-framework');
					$status_code = 200;
				} else {
					$response['success'] = false;
					$response['message'] = __('Username/Email address is existing', 'jobportal-framework');
				}
			}
