...
if (!class_exists('JobPortal_User')) {

    class JobPortal_User
    {

        function __construct()
        {
            add_action('init', array($this, 'add_user_roles'));
            add_action('init', array($this, 'jobportal_deactive_user'));
            ...
            add_action('jobportal_delayed_avatar_sync_after_register', array($this, 'delayed_avatar_sync_after_register'), 10, 1);
            ...
            if (jobportal_get_option('enable_status_user') === '1') {
                ...
                add_action('wp_ajax_jobportal_approve_user_status', array($this, 'ajax_approve_user_status'));
            ...
            $enable_identity_verification  = jobportal_get_option('enable_identity_verification');
        ...
        add_action('show_user_profile', array($this, 'jobportal_identity_verification_meta_box'));
        add_action('edit_user_profile', array($this, 'jobportal_identity_verification_meta_box'));
        add_action('personal_options_update', array($this, 'jobportal_save_identity_verification_meta_box'));
        add_action('edit_user_profile_update', array($this, 'jobportal_save_identity_verification_meta_box'));
        }
        /**
         * Hiển thị meta box xác thực danh tính trên trang user profile
         */
        public function jobportal_identity_verification_meta_box($user)
        {
            ...
            ?>
            <h2><?php esc_html_e('Identity Verification', 'jobportal-framework'); ?></h2>
            <table class="form-table">
                <tr>
                    <th><label><?php esc_html_e('Document Type', 'jobportal-framework'); ?></label></th>
                    <td>
                        <input type="text" name="identity_document" value="<?php echo esc_attr($identity_document); ?>" />
                    </td>
                </tr>
                <tr>
                    <th><label><?php esc_html_e('Front Side Image', 'jobportal-framework'); ?></label></th>
                    <td>
                        <?php if ($before_url): ?>
                ...
                <tr>
                    <th><label><?php esc_html_e('Back Side Image', 'jobportal-framework'); ?></label></th>
                    <td>
                        <?php if ($after_url): ?>
                ...
                <tr>
                    <th><label><?php esc_html_e('Verification Status', 'jobportal-framework'); ?></label></th>
                    <td>
         ...
         */
        public function jobportal_save_identity_verification_meta_box($user_id)
        {
            if (!current_user_can('edit_users')) return;
        ...
        {
            add_role(
                'jobportal_user_candidate',
                esc_html__('Candidate', 'jobportal-framework'),
                array(
                    'read' => true,
                    'edit_posts' => false,
                    'delete_posts' => false,
            ...
            add_role(
                'jobportal_user_employer',
                esc_html__('Employer', 'jobportal-framework'),
                array(
                    'read' => true,
                    'edit_posts' => false,
                    'delete_posts' => false,
        ...
        public function handle_identity_verification()
        {
            ...
            if (isset($_REQUEST['verify']) && $_REQUEST['verify'] == 'true') {
                ...
                jobportal_send_email($user->user_email, 'mail_approve_identify_for_user', $args);
                jobportal_send_email($admin_email, 'mail_approve_identify_for_admin', $args);
                $data = array(
                    'post_type' => 'candidate',
                    'author' => $user_id,
                    'post_status' => 'publish',
                ...
                update_post_meta($candidate_id, JOBPORTAL_METABOX_PREFIX . 'candidate_featured', 1);
            }

            if (isset($_REQUEST['verify']) && $_REQUEST['verify'] == 'false') {
                update_user_meta($user_id, 'identity_verification', '');
                jobportal_send_email($user->user_email, 'mail_unverify_identify_for_user', $args);
                jobportal_send_email($admin_email, 'mail_unverify_identify_for_admin', $args);
            }
        }

        public function jobportal_deactive_user()
        {
            if (
                isset($_SERVER['REQUEST_METHOD']) && $_SERVER['REQUEST_METHOD'] === 'POST' &&
                isset($_POST['action']) && $_POST['action'] === 'jobportal_deactive_user'
            ) {
                if (!isset($_POST['_wpnonce']) || !wp_verify_nonce($_POST['_wpnonce'], 'jobportal_deactive_user_action')) {
                    wp_die(__('Security check failed.', 'jobportal-framework'));
                }
                if (!is_user_logged_in()) {
                    wp_die(__('You must be logged in to perform this action.', 'jobportal-framework'));
                }
                ...
                if (in_array('administrator', $current_user->roles)) {
                    wp_die(__('Administrators cannot be deactivated this way.', 'jobportal-framework'));
                }
        ...
        {
            return 'jobportal_user_candidate';
        }
        ...
        public function handle_role_change($user_id, $role, $old_roles)
        {
            ...
            if ($role === 'jobportal_user_candidate') {
                $existing_candidate_id = get_user_meta($user_id, 'jobportal-cpt_id', true);
                ...
                if ($existing_post && $existing_post->post_type === 'candidate' && $existing_post->post_status !== 'trash') {
                    ...
                    delete_post_meta($existing_candidate_id, JOBPORTAL_METABOX_PREFIX . 'candidate_approval_status');
                    ...
                    if ($new_profile_id > 0) {
                        update_user_meta($user_id, 'jobportal-cpt_id', $new_profile_id);
                        $this->sync_candidate_status_by_user_status($new_profile_id, $user_id);
                    }
                }
            }

            if ($role === 'jobportal_user_employer') {
                $existing_candidate_id = get_user_meta($user_id, 'jobportal-cpt_id', true);
                $existing_post = !empty($existing_candidate_id) ? get_post($existing_candidate_id) : null;
                ...
                if ($existing_post && $existing_post->post_type === 'candidate' && $existing_post->post_status !== 'trash') {
                    ...
                    update_post_meta($existing_candidate_id, JOBPORTAL_METABOX_PREFIX . 'candidate_approval_status', 'locked');
                }

                // Ensure employer has company profile
                $existing_company_id = get_user_meta($user_id, 'jobportal-company_id', true);
                if (empty($existing_company_id) || !get_post($existing_company_id)) {
                    $new_company_id = $this->create_profile_for_new_employer($user_id);
                    if ($new_company_id > 0) {
                        update_user_meta($user_id, 'jobportal-company_id', $new_company_id);
                    }
                }
            }
        }
        ...
        {
            $enable_status_user = jobportal_get_option('enable_status_user');

            if ($enable_status_user === '1') {
                $user_status = get_user_meta($user_id, JOBPORTAL_METABOX_PREFIX . 'user_status', true);
            ...
            update_post_meta($candidate_id, JOBPORTAL_METABOX_PREFIX . 'candidate_approval_status', $approval_status);
        }
        ...
        public function register_candidate_archive_cpt()
        {
            register_post_type('candidate_archive', array(
                'labels' => array(
                    'name' => __('Candidate Archives', 'jobportal-framework'),
                ),
                'public' => false,
                'show_ui' => false,
                'show_in_menu' => false,
        ...
        public function prevent_locked_candidate_publish($data, $postarr)
        {
            ...
            if ($data['post_status'] === 'publish') {
                ...
                if ($post_id > 0) {
                    $approval_status = get_post_meta($post_id, JOBPORTAL_METABOX_PREFIX . 'candidate_approval_status', true);
                    ...
                    if ($approval_status === 'locked') {
                        ...
                        add_action('admin_notices', function () {
                            ...
                            echo '<p><strong>' . esc_html__('Cannot Publish', 'jobportal-framework') . ':</strong> ';
                            echo esc_html__('This candidate profile is locked because the user role has been changed to Employer. Please change the user role back to Candidate to enable publishing.', 'jobportal-framework');
                            echo '</p>';
                            echo '</div>';
                        });
                    }
        ...
        public function enqueue_users_scripts($hook)
        {
            ...
            wp_enqueue_script('jobportal-approve-user', JOBPORTAL_PLUGIN_URL . 'assets/js/admin/approve-user.js', array('jquery'), JOBPORTAL_THEME_VERSION, true);
            wp_localize_script('jobportal-approve-user', 'jobportalApproveUser', array(
        ...
        public function enforce_candidate_status_by_author_role($data, $postarr)
        {
            if (isset($data['post_type']) && $data['post_type'] === 'candidate') {
                ...
                if ($author_id > 0) {
                    ...
                    if ($user && !in_array('jobportal_user_candidate', (array) $user->roles, true)) {
                        $data['post_status'] = 'draft';
                    }
                }
            }
        ...
        {
            $columns['identity_verification'] = esc_html__('Identity Verification', 'jobportal-framework');
            return $columns;
        }
        ...
        public function display_identity_verification_column_data($value, $column_name, $user_id)
        {
            ...
            if (!in_array('jobportal_user_candidate', $user_roles) && !in_array('jobportal_user_employer', $user_roles)) {
                return $value;
            }
            ...
            if ($identity_verification == 'verified') {
                return '<span class="label jobportal-label-green">' . esc_html__('Verified', 'jobportal-framework') . '</span><br>';
            }
            ...
            if ($identity_verification == 'pending' && !empty($verify_id_before_id) && !empty($verify_id_after_id)) {
                ...
                if ($before_url && $after_url) {
                    $html = '<span class="label jobportal-label-yellow">' . esc_html__('Pending', 'jobportal-framework') . '</span><br>';
                    $html .= '<div class="jobportal-identity-verification-images">';
            ...
            return '<span class="label jobportal-label-red">' . esc_html__('Not Verified', 'jobportal-framework') . '</span>';
        }
        ...
        {
            $columns['user_created_at'] = esc_html__('Created At', 'jobportal-framework');
            return $columns;
        }
        ...
        public function display_user_custom_column_data($value, $column_name, $user_id)
        {
            ...
            if (!$user || empty($user->user_registered)) {
                return esc_html__('N/A', 'jobportal-framework');
            }
        ...
        public function add_user_status_action_link($actions, $user_object)
        {
            ...
            $user_status = get_user_meta($user_object->ID, JOBPORTAL_METABOX_PREFIX . 'user_status', true);
            if ($user_status !== 'approve' && current_user_can('promote_user', $user_object->ID)) {
                $nonce = wp_create_nonce('jobportal_approve_user_status');
                $actions['jobportal-approve-user-status'] = '<a href="#" class="jobportal-approve-user-status" data-user-id="' . esc_attr($user_object->ID) . '" data-nonce="' . esc_attr($nonce) . '">' . esc_html__('Approve User Status', 'jobportal-framework') . '</a>';
            }
            return $actions;
        }
        ...
        {
            check_ajax_referer('jobportal_approve_user_status');
            if (!current_user_can('promote_users')) {
                wp_send_json_error(array('message' => __('Permission denied', 'jobportal-framework')));
            }
            $user_id = isset($_POST['user_id']) ? absint($_POST['user_id']) : 0;
            if (!$user_id) {
                wp_send_json_error(array('message' => __('Invalid user', 'jobportal-framework')));
            }

            update_user_meta($user_id, JOBPORTAL_METABOX_PREFIX . 'user_status', 'approve');
            ...
            if (!empty($candidate_post_id)) {
                ...
                update_post_meta($candidate_post_id, JOBPORTAL_METABOX_PREFIX . 'candidate_approval_status', 'approved');
            }
            ...
            wp_send_json_success(array('message' => __('Approved', 'jobportal-framework')));
        }
        ...
        public function add_custom_user_action_link($actions, $user_object)
        {
            ...
            if ($identity_verification == 'pending' && (in_array('jobportal_user_candidate', $user_roles) || in_array('jobportal_user_employer', $user_roles))) {
            ...
            if ($identity_verification == 'verified' && (in_array('jobportal_user_candidate', $user_roles) || in_array('jobportal_user_employer', $user_roles))) {
                $url = admin_url('users.php?s=' . $user_email . '&verify=false&id=' . $user_id . '&new_role&paged=1&action2=-1&new_role2');
        ...
        {
            $columns['custom_column'] = esc_html__('Status', 'jobportal-framework');
            return $columns;
        }
        ...
        public static function custom_display_user_column_data($value, $column_name, $user_id)
        {
            if ('custom_column' === $column_name) {
                $status = '';
                $user_status = get_the_author_meta(JOBPORTAL_METABOX_PREFIX . 'user_status', $user_id);
                if (empty($user_status) || $user_status == 'pending') {
                    $user_status = '<span class="label jobportal-label-yellow">' . esc_html__('Pending', 'jobportal-framework') . '</span>';
                } else {
                    $user_status = '<span class="label jobportal-label-blue">' . esc_html__('Approved', 'jobportal-framework') . '</span>';
                }
                return $user_status;
            }
            return $value;
        ...
        public static function jobs_single_bottombar()
        {
            if (is_singular('jobs')) {
                ...
                $jobs_id = get_the_ID(); ?>
                <div class="jobportal-apply-bottombar">
        ...
        public function create_a_profile_post_for_new_candidate($user_id)
        {
            ...
            $existing_profile_id = get_user_meta($user_id, 'jobportal-cpt_id', true);
            ...
            global $jobportal_registering_account_type;
            $is_new_registration = isset($jobportal_registering_account_type);

            // If registering as employer but has candidate profile, remove it
            if ($is_new_registration && $jobportal_registering_account_type === 'jobportal_user_employer' && $existing_post) {
                wp_delete_post($existing_profile_id, true); // Force delete
                delete_user_meta($user_id, 'jobportal-cpt_id');
            ...
            if (!empty($existing_profiles)) {
                ...
                update_user_meta($user_id, 'jobportal-cpt_id', $found_profile_id);
            ...
            if (is_admin() && !wp_doing_ajax()) {
                $is_candidate = in_array('jobportal_user_candidate', $user_roles, true);
            } else {
                // Check global variable first (set during registration)
                global $jobportal_registering_account_type;
                if (isset($jobportal_registering_account_type)) {
                    $is_candidate = ($jobportal_registering_account_type === 'jobportal_user_candidate');
                } elseif (isset($_POST['account_type'])) {
                    $is_candidate = (sanitize_text_field(wp_unslash($_POST['account_type'])) === 'jobportal_user_candidate');
                } else {
                    $is_candidate = in_array('jobportal_user_candidate', $user_roles, true);
                }
            }
        ...
        private function create_profile_for_new_candidate($user_id)
        {
            ...
            if (!empty($existing_profiles)) {
                ...
                update_user_meta($user_id, 'jobportal-cpt_id', $profile_id);
                return $profile_id;
            }
            ...
            if ($new_profile_id && !is_wp_error($new_profile_id)) {
                ...
                update_post_meta($new_profile_id, JOBPORTAL_METABOX_PREFIX . 'candidate_approval_status', $approval_status);
                ...
                wp_schedule_single_event(time() + 3, 'jobportal_delayed_avatar_sync_after_register', array($user_id));
            }
            return $new_profile_id;
        }
        ...
        private function sync_user_to_candidate_data($user_id, $candidate_id)
        {
            ...
            $enable_status_user = jobportal_get_option('enable_status_user');
            update_post_meta($candidate_id, JOBPORTAL_METABOX_PREFIX . 'candidate_user_id', $user_id);
            update_post_meta($candidate_id, JOBPORTAL_METABOX_PREFIX . 'candidate_first_name', $user_data->first_name);
            update_post_meta($candidate_id, JOBPORTAL_METABOX_PREFIX . 'candidate_last_name', $user_data->last_name);
            update_post_meta($candidate_id, JOBPORTAL_METABOX_PREFIX . 'candidate_email', $user_data->user_email);

            // Check for phone in global variable first (set during registration before user meta)
            global $jobportal_registering_phone;
            $phone = isset($jobportal_registering_phone) ? $jobportal_registering_phone : get_user_meta($user_id, JOBPORTAL_METABOX_PREFIX . 'author_mobile_number', true);

            if (!empty($phone)) {
                update_post_meta($candidate_id, JOBPORTAL_METABOX_PREFIX . 'candidate_phone', $phone);
            }
            update_post_meta($candidate_id, JOBPORTAL_METABOX_PREFIX . 'candidate_profile_strength', 10);
            update_post_meta($candidate_id, JOBPORTAL_METABOX_PREFIX . 'candidate_featured', 0);
            if ($enable_status_user === '1') {
                update_post_meta($candidate_id, JOBPORTAL_METABOX_PREFIX . 'candidate_approval_status', 'pending');
            } else {
                update_post_meta($candidate_id, JOBPORTAL_METABOX_PREFIX . 'candidate_approval_status', 'approved');
            }
        }
        ...
        private function sync_complete_avatar_data($user_id, $candidate_id)
        {
            ...
            if (!empty($existing_avatar_url)) {
                update_post_meta($candidate_id, JOBPORTAL_METABOX_PREFIX . 'candidate_avatar_image_url', $existing_avatar_url);
                if (!empty($existing_avatar_id)) {
                    update_post_meta($candidate_id, JOBPORTAL_METABOX_PREFIX . 'candidate_avatar_image_id', $existing_avatar_id);
                }
            }
        }
        ...
        {
            global $current_user;
            if (in_array("jobportal_user_employer", (array) $current_user->roles)) {
                return;
            }
            $current_page_id = get_the_ID();
            $alerts_title = esc_html__('Job Alert', 'jobportal-framework');
            $alerts_desc = esc_html__('Subscribe to receive instant alerts of new relevant jobs directly to your email inbox.', 'jobportal-framework');
            $alerts_button_title = esc_html__('Subscribe', 'jobportal-framework');
            $jobportal_job_alerts_page_id  = jobportal_get_option('jobportal_job_alerts_page_id');
            if (($current_page_id == $jobportal_job_alerts_page_id) || isset($_COOKIE["cookie_job_alerts"]) || (get_post_type() != 'jobs')) {
                return;
            }
            ?>
            <div class="alert-form">
                ...
                <div class="inner">
                    ...
                    <div class="content">
                        ...
                        <a href="<?php echo esc_url(get_page_link($jobportal_job_alerts_page_id)); ?>" class="jobportal-button"><?php echo $alerts_button_title; ?></a>
                    </div>
                </div>
            </div>
<?php
        ...
        public function create_a_profile_post_for_new_employer($user_id)
        {
            ...
            $existing_profile_id = get_user_meta($user_id, 'jobportal-company_id', true);
            $existing_post = !empty($existing_profile_id) ? get_post($existing_profile_id) : null;
            ...
            if (!empty($existing_profiles)) {
                $found_profile_id = $existing_profiles[0];
                update_user_meta($user_id, 'jobportal-company_id', $found_profile_id);
                return $found_profile_id;
            }

            // Clean up orphaned meta
            if (!empty($existing_profile_id) && !$existing_post) {
                delete_user_meta($user_id, 'jobportal-company_id');
            }
            ...
            if (is_admin() && !wp_doing_ajax()) {
                ...
                if (isset($_POST['account_type'])) {
                    $is_employer = (sanitize_text_field(wp_unslash($_POST['account_type'])) === 'jobportal_user_employer');
                } else {
                    $is_employer = in_array('jobportal_user_employer', $user_roles, true);
                }
            }
            ...
            if ($new_profile_id > 0) {
                update_user_meta($user_id, 'jobportal-company_id', $new_profile_id);
            }
        ...
        private function create_profile_for_new_employer($user_id)
        {
            ...
            if (!empty($existing_profiles)) {
                $profile_id = $existing_profiles[0];
                update_user_meta($user_id, 'jobportal-company_id', $profile_id);
                return $profile_id;
            }
        ...
        private function sync_user_to_company_data($user_id, $company_id)
        {
            ...
            update_post_meta($company_id, JOBPORTAL_METABOX_PREFIX . 'company_user_id', $user_id);
            update_post_meta($company_id, JOBPORTAL_METABOX_PREFIX . 'company_email', $user_data->user_email);
            update_post_meta($company_id, JOBPORTAL_METABOX_PREFIX . 'company_featured', 0);

            if ($enable_status_user === '1') {
                update_post_meta($company_id, JOBPORTAL_METABOX_PREFIX . 'company_approval_status', 'pending');
            } else {
                update_post_meta($company_id, JOBPORTAL_METABOX_PREFIX . 'company_approval_status', 'approved');
            }
        }
    }
    new JobPortal_User();
...
if (!function_exists('jobportal_update_user_avatar')) {
    function jobportal_update_user_avatar($user_id)
    {
        return JobPortal_User::quick_update_avatar($user_id);
    }
}
