<?php
function jobportalGetReviews($placeid)
{

	$google_review_api_key  = jobportal_get_option('google_review_api_key', 'AIzaSyBvgFc-WtkrUrAhTE_7an-OkBNlqReY_Bc');
	...
	if ($data['error_message']) {
		...
		echo '<p class="error-box">' . 'JOBPORTAL msg: Place review file exists? ' . file_exists($option['google_maps_review_cid'] . '.json') . '</p>';
	} else {
		...
		if (!empty($reviews)) {
			...
			$html .= '<h3 class="entry-title">' . esc_html__('Google Reviews', 'jobportal-framework') . '</h3>';
			$html .= '</div>';
			...
			if (isset($option['show_cname_as_headline']) and $option['show_cname_as_headline'] == true) {
				...
				$icon = JOBPORTAL_PLUGIN_URL . 'assets/images/google.png';
				...
				if (isset($option['show_stars_in_headline']) and $option['show_stars_in_headline'] == true) {
					for ($i = 1; $i <= $data['result']['rating']; ++$i) $html .= '<i class="fas fa-star check"></i>';
				...
				if ($data['result']['user_ratings_total'] == 1) {
					$html .= '<span class="count">' . $data['result']['user_ratings_total'] . esc_html__(' review', 'jobportal-framework') . '</span>';
				} else {
					$html .= '<span class="count">' . $data['result']['user_ratings_total'] . esc_html__(' reviews', 'jobportal-framework') . '</span>';
				}

				$html .= '</div>'; // google-reviews-info
				$html .= '<div class="google-reviews-add">';
				$html .= '<a href="https://search.google.com/local/writereview?placeid=' . $option['google_maps_review_cid'] . '" target="_Blank"><i class="fas fa-comment-alt-dots"></i>' . esc_html__('Add Review', 'jobportal-framework') . '</a>';
			...
			foreach ($reviews as $key => $review) {
				...
				for ($i = 1; $i <= $review['rating']; ++$i) $html .= '<i class="fas fa-star check"></i>';
