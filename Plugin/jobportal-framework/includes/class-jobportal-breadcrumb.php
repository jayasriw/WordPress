...
if (!class_exists('JobPortal_Breadcrumb')) {

    /**
     *  Class JobPortal_Breadcrumb
     */
    class JobPortal_Breadcrumb
    {
        ...
        public static function breadcrumb($args = '')
        {
            ...
            $default = array(
                'type' => 'list',
                'home_label' => esc_html__('Home', 'jobportal-framework'),
                'search_label' => esc_html__('Search Result of "%s"', 'jobportal-framework'),
                '404_label' => esc_html__('404 Not Found', 'jobportal-framework'),
                'category_label' => _x('%s', 'category label', 'jobportal-framework'),
                'tag_label' => _x('%s', 'tag label', 'jobportal-framework'),
                'taxonomy_label' => _x('%s', 'taxonomy label', 'jobportal-framework'),
                'author_label' => _x('%s', 'author label', 'jobportal-framework'),
                'attachment_label' => _x('%s', 'attachment label', 'jobportal-framework'),
                'year_label' => _x('%s', 'year label', 'jobportal-framework'),
                'month_label' => _x('%s', 'month label', 'jobportal-framework'),
                'day_label' => _x('%s', 'day label', 'jobportal-framework'),
                'post_type_label' => _x('%s', 'post type label', 'jobportal-framework'),
                'joint_string' => esc_html__(' &gt; ', 'jobportal-framework'),
                'navi_element' => '',
                'elm_class' => 'jobportal_breadcrumb',
                'elm_id' => '',
                'li_class' => '',
                'class_prefix' => '',
            ...
            $default = apply_filters('jobportal_breadcrumb_default', $default);
            ...
            if ($args['type'] == 'string') {
                ...
                $output = apply_filters('jobportal_breadcrumb_after', $output, $args);
            } else {
                ...
                $output = apply_filters('jobportal_breadcrumb_before', $output, $args);
                ...
                foreach ($breadcrumb_arr as $ancestor) {
                    ...
                    if ($cnt == count($breadcrumb_arr)) {
                        ...
                        if ( class_exists( 'WooCommerce' ) ) {
                            $output .= '"><a href="' . $ancestor['link'] . '">' . (is_wp_error($ancestor['title']) ? '' : $ancestor['title']) . '</a></li>' . "\n";
                        } else {
                            $output .= '"><a href="' . $ancestor['link'] . '">' . (is_wp_error($ancestor['title']) ? '' : apply_filters('the_title', $ancestor['title'])) . '</a></li>' . "\n";
                        }
                    }
                    $cnt++;
                }
            ...
            $output = apply_filters('jobportal_breadcrumb', $output, $args);
        ...
        public static function get_breadcrumb_array($args)
        {
            ...
            if (is_404()) {
                ...
                if ($term && !is_wp_error($term)) {
                    if (is_taxonomy_hierarchical($taxonomy) && $term->parent != 0) {
                        ...
                        foreach ($ancestors as $ancestor_id) {
                            ...
                            if ($ancestor && !is_wp_error($ancestor)) {
                                $term_link = get_term_link($ancestor, $term->slug);
                                $breadcrumb_arr[] = array(
                                    'title' => $ancestor->name,
                    ...
                    $term_link = get_term_link($term->term_id, $term->slug);
                    $breadcrumb_arr[] = array(
                ...
                if ($post_type && !is_wp_error($post_type)) {
                    ...
                    $breadcrumb_arr[] = array(
                        ...
                        'link' => is_wp_error($archive_link) ? '#' : $archive_link
                    );
                }
            }

            return apply_filters('jobportal_breadcrumb_arr', $breadcrumb_arr, $args);
        }
        ...
        public static function get_singular_breadcrumb_array($post, $args)
        {
            ...
            if ($post_type && !is_wp_error($post_type) && $post_type->has_archive) {
                $archive_link = get_post_type_archive_link($post_type->name);
                $breadcrumb_arr[] = array(
            ...
            if ($post_type && !is_wp_error($post_type) && is_post_type_hierarchical($post_type->name)) {
                ...
                if($enable_slug_categories == 1 && $post_type && !is_wp_error($post_type)){
                                ...
                                if ($terms) {
                                    ...
                                    if ($term->parent != 0) {
                                        ...
                                        foreach ($ancestors as $ancestor_id) {
                                            ...
                                            if ($ancestor && !is_wp_error($ancestor)) {
                                                $term_link = get_term_link($ancestor, $tax_slug);
                                                $breadcrumb_arr[] = array(
                                                    'title' => $ancestor->name,
                                                    'link' => is_wp_error($term_link) ? '#' : $term_link
                                                );
