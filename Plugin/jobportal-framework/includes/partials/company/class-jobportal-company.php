...
if (!class_exists('JobPortal_Company')) {
	/**
	 * Class JobPortal_Invoice
	 */
	class JobPortal_Company
	{

		/**
		 * Company breadcrumb
		 */
		public function jobportal_company_breadcrumb()
		{ ?>
...
<?php }
		...
		public function get_total_my_company()
		{
			$args = array(
				...
				'meta_query' => array(
					array(
						'key' => JOBPORTAL_METABOX_PREFIX . 'company_user_id',
		...
		public function insert_company($payment_type, $item_id, $user_id, $payment_for, $payment_method, $paid = 0, $payment_id = '', $payer_id = '')
		{
			...
			if ($payment_type != 'Package') {
				...
				if ($package_free == 1) {
					...
					$total_company = get_post_meta($item_id, JOBPORTAL_METABOX_PREFIX . 'package_price', true);
				}
			}
			$time = time();
			...
			$jobportal_meta['company_item_id'] = $item_id;
			$jobportal_meta['company_item_price'] = $total_money;
			$jobportal_meta['company_purchase_date'] = $company_date;
			$jobportal_meta['company_user_id'] = $user_id;
			$jobportal_meta['company_payment_type'] = $payment_type;
			$jobportal_meta['company_payment_method'] = $payment_method;
			...
			$rw = jobportal_get_page_by_title($posttitle, 'company');
			$company_payment_status = get_post_meta($rw->ID, JOBPORTAL_METABOX_PREFIX . 'company_payment_status', true);
			...
			if (empty($rw->ID) || ($rw->ID && $company_payment_status == '1')) {
				...
				update_post_meta($company_id, JOBPORTAL_METABOX_PREFIX . 'company_item_id', $item_id);
				update_post_meta($company_id, JOBPORTAL_METABOX_PREFIX . 'company_price', $total_money);
				update_post_meta($company_id, JOBPORTAL_METABOX_PREFIX . 'company_date', $company_date);
				update_post_meta($company_id, JOBPORTAL_METABOX_PREFIX . 'company_payment_type', $payment_type);
				update_post_meta($company_id, JOBPORTAL_METABOX_PREFIX . 'company_payment_method', $payment_method);
				update_post_meta($company_id, JOBPORTAL_METABOX_PREFIX . 'company_payment_status', $paid);

				update_post_meta($company_id, JOBPORTAL_METABOX_PREFIX . 'trans_payment_id', $payment_id);
				update_post_meta($company_id, JOBPORTAL_METABOX_PREFIX . 'trans_payer_id', $payer_id);

				update_post_meta($company_id, JOBPORTAL_METABOX_PREFIX . 'company_meta', $jobportal_meta);
				$update_post = array(
					'ID'         => $company_id,
				);
		...
		public function get_company_meta($post_id, $field = false)
		{
			...
			$meta = get_post_meta($post_id, JOBPORTAL_METABOX_PREFIX . 'company_meta', true);
		...
		public function company_print_ajax()
		{
			...
			jobportal_get_template('company/company-print.php', array('company_id' => intval($company_id), 'isRTL' => $isRTL));
			wp_die();
		}
		...
		public function submit_review_ajax()
		{
			...
			if ($company_id <= 0) {
				wp_send_json_error(['message' => __('Invalid company ID.', 'jobportal-framework')], 422);
			}
         ...
         LIMIT 1",
			...
			if (null === $my_review) {
				...
				if ($comment_approved) {
					apply_filters('jobportal_company_rating_meta', $company_id, $company_rating);
				}
				...
				jobportal_get_data_ajax_notification($company_id, 'add-review-company');
			} else {
				...
				if ($comment_approved) {
					apply_filters('jobportal_company_rating_meta', $company_id, $company_rating, false, $my_review->meta_value);
				}
		...
		{
			update_post_meta($company_id, JOBPORTAL_METABOX_PREFIX . 'company_rating', $rating_value);
		}
		...
		public function submit_reply_ajax()
		{
			...
			$user     = get_user_by('id', $user_id);
			$company_id = isset($_POST['company_id']) ? jobportal_clean(wp_unslash($_POST['company_id'])) : '';
			$comment_approved = 1;
		...
		public function company_submit_ajax()
		{
			...
			$company_form               = isset($_REQUEST['company_form']) ? jobportal_clean(wp_unslash($_REQUEST['company_form'])) : '';
			$company_action             = isset($_REQUEST['company_action']) ? jobportal_clean(wp_unslash($_REQUEST['company_action'])) : '';
			$company_id                 = isset($_REQUEST['company_id']) ? jobportal_clean(wp_unslash($_REQUEST['company_id'])) : '';
			$company_title              = isset($_REQUEST['company_title']) ? jobportal_clean(wp_unslash($_REQUEST['company_title'])) : '';
			$company_categories         = isset($_REQUEST['company_categories']) ? jobportal_clean(wp_unslash($_REQUEST['company_categories'])) : '';
			$company_new_categories     = isset($_REQUEST['company_new_categories']) ? jobportal_clean(wp_unslash($_REQUEST['company_new_categories'])) : '';
			$company_url      = isset($_REQUEST['company_url']) ? jobportal_clean(wp_unslash($_REQUEST['company_url'])) : '';
			$company_des       = isset($_REQUEST['company_des']) ? wp_kses_post(wp_unslash($_REQUEST['company_des'])) : '';
			$company_website      = isset($_REQUEST['company_website']) ? jobportal_clean(wp_unslash($_REQUEST['company_website'])) : '';
			$company_founded       = isset($_REQUEST['company_founded']) ? jobportal_clean(wp_unslash($_REQUEST['company_founded'])) : '';
			$company_phone        = isset($_REQUEST['company_phone']) ? jobportal_clean(wp_unslash($_REQUEST['company_phone'])) : '';
			$company_phone_code        = isset($_REQUEST['company_phone_code']) ? jobportal_clean(wp_unslash($_REQUEST['company_phone_code'])) : '';
			$company_email      = isset($_REQUEST['company_email']) ? jobportal_clean(wp_unslash($_REQUEST['company_email'])) : '';
			$company_size      = isset($_REQUEST['company_size']) ? jobportal_clean(wp_unslash($_REQUEST['company_size'])) : '';

			$company_twitter = isset($_REQUEST['company_twitter']) ? jobportal_clean(wp_unslash($_REQUEST['company_twitter'])) : '';
			$company_linkedin = isset($_REQUEST['company_linkedin']) ? jobportal_clean(wp_unslash($_REQUEST['company_linkedin'])) : '';
			$company_facebook = isset($_REQUEST['company_facebook']) ? jobportal_clean(wp_unslash($_REQUEST['company_facebook'])) : '';
			$company_instagram = isset($_REQUEST['company_instagram']) ? jobportal_clean(wp_unslash($_REQUEST['company_instagram'])) : '';
			$company_social_name = isset($_REQUEST['company_social_name']) ? jobportal_clean(wp_unslash($_REQUEST['company_social_name'])) : '';
			$company_social_url = isset($_REQUEST['company_social_url']) ? jobportal_clean(wp_unslash($_REQUEST['company_social_url'])) : '';

			$company_avatar_id = isset($_REQUEST['company_avatar_id']) ? jobportal_clean(wp_unslash($_REQUEST['company_avatar_id'])) : '';
			$company_avatar_url = isset($_REQUEST['company_avatar_url']) ? jobportal_clean(wp_unslash($_REQUEST['company_avatar_url'])) : '';
			$company_thumbnail_url = isset($_REQUEST['company_thumbnail_url']) ? jobportal_clean(wp_unslash($_REQUEST['company_thumbnail_url'])) : '';
			$company_thumbnail_id  = isset($_REQUEST['company_thumbnail_id']) ? jobportal_clean(wp_unslash($_REQUEST['company_thumbnail_id'])) : '';
			$jobportal_gallery_ids          = isset($_REQUEST['jobportal_gallery_ids']) ? jobportal_clean(wp_unslash($_REQUEST['jobportal_gallery_ids'])) : '';
			$company_video_url      = isset($_REQUEST['company_video_url']) ? jobportal_clean(wp_unslash($_REQUEST['company_video_url'])) : '';
			$company_map_location       = isset($_REQUEST['company_map_location']) ? jobportal_clean(wp_unslash($_REQUEST['company_map_location'])) : '';
			$company_new_location      = isset($_REQUEST['company_new_location']) ? jobportal_clean(wp_unslash($_REQUEST['company_new_location'])) : '';
			$company_map_address        = isset($_REQUEST['company_map_address']) ? jobportal_clean(wp_unslash($_REQUEST['company_map_address'])) : '';
			$company_location       = isset($_REQUEST['company_location']) ? jobportal_clean(wp_unslash($_REQUEST['company_location'])) : '';
			$company_latitude      = isset($_REQUEST['company_latitude']) ? jobportal_clean(wp_unslash($_REQUEST['company_latitude'])) : '';
			$company_longtitude       = isset($_REQUEST['company_longtitude']) ? jobportal_clean(wp_unslash($_REQUEST['company_longtitude'])) : '';

			$custom_field_company        = isset($_REQUEST['custom_field_company']) ? jobportal_clean(wp_unslash($_REQUEST['custom_field_company'])) : '';
			$submitButtonName        = isset($_REQUEST['submitButtonName']) ? jobportal_clean(wp_unslash($_REQUEST['submitButtonName'])) : '';
			...
			if ($company_action) {
				...
				$auto_publish         = jobportal_get_option('company_auto_publish', 1);
				$auto_publish_edited  = jobportal_get_option('company_auto_publish_edited', 1);
			...
			if ($company_id > 0) {
				...
				if (isset($company_website)) {
					update_post_meta($company_id, JOBPORTAL_METABOX_PREFIX . 'company_website', $company_website);
				}

				if (isset($company_founded)) {
					update_post_meta($company_id, JOBPORTAL_METABOX_PREFIX . 'company_founded', $company_founded);
				}

				if (isset($company_phone)) {
					update_post_meta($company_id, JOBPORTAL_METABOX_PREFIX . 'company_phone', $company_phone);
				}

				if (isset($company_phone_code)) {
					update_post_meta($company_id, JOBPORTAL_METABOX_PREFIX . 'company_phone_code', $company_phone_code);
				}

				if (isset($company_twitter)) {
					update_post_meta($company_id, JOBPORTAL_METABOX_PREFIX . 'company_twitter', $company_twitter);
				}

				if (isset($company_linkedin)) {
					update_post_meta($company_id, JOBPORTAL_METABOX_PREFIX . 'company_linkedin', $company_linkedin);
				}

				if (isset($company_facebook)) {
					update_post_meta($company_id, JOBPORTAL_METABOX_PREFIX . 'company_facebook', $company_facebook);
				}

				if (isset($company_instagram)) {
					update_post_meta($company_id, JOBPORTAL_METABOX_PREFIX . 'company_instagram', $company_instagram);
				}

				if (isset($company_email)) {
					update_post_meta($company_id, JOBPORTAL_METABOX_PREFIX . 'company_email', $company_email);
				}

				if (isset($company_video_url)) {
					update_post_meta($company_id, JOBPORTAL_METABOX_PREFIX . 'company_video_url', $company_video_url);
				}

				if (isset($company_map_address)) {
					update_post_meta($company_id, JOBPORTAL_METABOX_PREFIX . 'company_address', $company_map_address);
				}

				if (!empty($company_website) && !empty($company_phone) && !empty($company_location)) {
					update_post_meta($company_id, JOBPORTAL_METABOX_PREFIX . 'company_green_tick', 1);
				} else {
					update_post_meta($company_id, JOBPORTAL_METABOX_PREFIX . 'company_green_tick', 0);
				}
				...
				if (isset($company_map_location)) {
					...
					update_post_meta($company_id, JOBPORTAL_METABOX_PREFIX . 'company_location', $arr_location);
					update_post_meta($company_id, JOBPORTAL_METABOX_PREFIX . 'company_address', $company_map_address);
					update_post_meta($company_id, JOBPORTAL_METABOX_PREFIX . 'company_latitude', $company_latitude);
					update_post_meta($company_id, JOBPORTAL_METABOX_PREFIX . 'company_longtitude', $company_longtitude);
				}
				...
				if (!empty($company_social_name)) {
					...
					for ($i = 1; $i < count($company_social_name); $i++) {
						$social_data[] = array(
							JOBPORTAL_METABOX_PREFIX . 'company_social_name'   => $company_social_name[$i],
							JOBPORTAL_METABOX_PREFIX . 'company_social_url'    => $company_social_url[$i],
						);
					}
					update_post_meta($company_id, JOBPORTAL_METABOX_PREFIX . 'company_social_tabs', $social_data);
				}
				...
				if (isset($company_avatar_url) && isset($company_avatar_id)) {
					...
					update_post_meta($company_id, JOBPORTAL_METABOX_PREFIX . 'company_logo', $company_avatar);
				}
				...
				if (isset($jobportal_gallery_ids)) {
					$str_img_ids = '';
					foreach ($jobportal_gallery_ids as $company_img_id) {
						$jobportal_gallery_ids[] = intval($company_img_id);
						$str_img_ids .= '|' . intval($company_img_id);
					}
					$str_img_ids = substr($str_img_ids, 1);
					update_post_meta($company_id, JOBPORTAL_METABOX_PREFIX . 'company_images', $str_img_ids);
				}

				$get_additional = jobportal_render_custom_field('company');
