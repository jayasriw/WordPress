...
if (!class_exists('JobPortal_Payment')) {
	/**
	 * Class JobPortal_Payment
	 */
	class JobPortal_Payment
	{
		protected $jobportal_invoice;
		protected $jobportal_package;
		protected $jobportal_trans_log;
		...
		{
			$this->jobportal_package = new JobPortal_Package();
			$this->jobportal_invoice = new JobPortal_Invoice();
			$this->jobportal_trans_log = new JobPortal_Trans_Log();
	...
	public function event_woocommerce_payment_complete($order_id)
	{
			...
			foreach ($items as $item) {
				...
				if ($product_id) {
					...
					if ($order_package_id && ($order->get_status() == 'completed' || $order->get_status() == 'processing')) {
						update_post_meta($order_package_id, JOBPORTAL_METABOX_PREFIX . 'invoice_payment_status', '1');
						update_post_meta($order_package_id, JOBPORTAL_METABOX_PREFIX . 'invoice_wc_order_id', $order->get_order_number());
						$user_id = $order->get_user_id();
						// Get package ID from invoice meta (stored as invoice_item_id)
						$package_id = get_post_meta($order_package_id, JOBPORTAL_METABOX_PREFIX . 'invoice_item_id', true);
						if (!$package_id) {
							$package_id = get_post_meta($order_package_id, JOBPORTAL_METABOX_PREFIX . 'package_id', true);
						}

						// Assign package to user when payment is successful
						if ($package_id && $user_id) {
							$invoice_pdf_id = get_post_meta($order_package_id, JOBPORTAL_METABOX_PREFIX . 'invoice_pdf_id', true);
							$this->jobportal_package->activate_package_with_cleanup($user_id, $package_id, $invoice_pdf_id);
						}

						do_action('jobportal_woocommerce_payment_success', $user_id, $package_id, $order_package_id);
					} else {
						// Update order status
						update_post_meta($order_package_id, JOBPORTAL_METABOX_PREFIX . 'invoice_payment_status', '0');
					}
				}
			}
		}
		...
		{
			require_once(JOBPORTAL_PLUGIN_DIR . 'includes/partials/payment/stripe-php/init.php');
			$stripe_secret_key = jobportal_get_option('stripe_secret_key');
			$stripe_publishable_key = jobportal_get_option('stripe_publishable_key');
			...
			$payment_completed_link = jobportal_get_permalink('payment_completed');
			...
			wp_localize_script('stripe-checkout', 'jobportal_stripe_vars', array(
				'jobportal_stripe_per_package' => array(
					...
					'params' => array(
						...
						'description' => wp_kses_post(sprintf(__('%s Package Payment', 'jobportal-framework'), $package_name))
					)
				)
			));
?>
			<form class="jobportal-stripe-form" action="<?php echo esc_url($stripe_processor_link) ?>" method="post" id="jobportal_stripe_per_package">
		...
		{
			$client_id = jobportal_get_option('paypal_client_id');
			$secret_key = jobportal_get_option('paypal_client_secret_key');
		...
		{
			check_ajax_referer('jobportal_payment_ajax_nonce', 'jobportal_security_payment');
			global $current_user;
			wp_get_current_user();
			$user_id = $current_user->ID;
			...
			$invoice_id = $this->jobportal_invoice->insert_invoice('Package', $package_id, $user_id, 0, 'Paypal', 0, '', '', true);

			$currency = jobportal_get_option('currency_type_default', 'USD');
			$payment_description = $package_name . ' ' . esc_html__('Membership payment on ', 'jobportal-framework') . $blogInfo;
			$is_paypal_live = jobportal_get_option('paypal_api');
			...
			$dash_profile_link = jobportal_get_permalink('employer_dashboard');
			...
			$payment['transactions'][0]['item_list']['items'][] = array(
				'quantity' => '1',
				'name' => esc_html__('Payment Package', 'jobportal-framework'),
				'price' => $package_price,
				'currency' => $currency,
				'sku' => $package_name . ' ' . esc_html__('Payment Package', 'jobportal-framework'),
			);
			...
			update_user_meta($user_id, JOBPORTAL_METABOX_PREFIX . 'paypal_transfer', $output);
		...
		public function stripe_create_invoice_per_package_ajax()
		{
			...
			if (!is_user_logged_in()) {
				wp_send_json_error(array('message' => __('No Login', 'jobportal-framework')));
			}

			$package_id = isset($_POST['package_id']) ? absint($_POST['package_id']) : 0;
			if (!$package_id) {
				wp_send_json_error(array('message' => __('Missing package', 'jobportal-framework')));
			}

			$payment_method = 'Stripe';
			$invoice_id = $this->jobportal_invoice->insert_invoice('Package', $package_id, $user_id, 0, $payment_method, 0, '', '', true);

			wp_send_json_success(array('invoice_id' => $invoice_id));
		}

		public function jobportal_razor_package_addons($package_id)
		{
			$payment_completed_link = jobportal_get_permalink('payment_completed');
		?>
...
<?php
		...
		public function jobportal_razor_payment_verify()
		{
			$payment_completed_link = jobportal_get_permalink('payment_completed');
		...
		public function jobportal_razor_package_create_order()
		{
			require_once(JOBPORTAL_PLUGIN_DIR . 'includes/partials/payment/razorpay-php/Razorpay.php');
			...
			$key_id_razor  = jobportal_get_option('razor_key_id');
			$key_secret    = jobportal_get_option('razor_key_secret');
			$currency_code = jobportal_get_option('currency_type_default', 'USD');
			...
			$invoice_id = $this->jobportal_invoice->insert_invoice('Package', $package_id, $user_id, 0, 'Razor', 0, '', '', true);
		...
		public function razor_payment_completed()
		{
			...
			if (!empty($attributes)) {
				...
				if ($success === true) {
					...
					if ($invoice_id) {
						update_post_meta($invoice_id, JOBPORTAL_METABOX_PREFIX . 'invoice_payment_status', 1);
						update_post_meta($invoice_id, JOBPORTAL_METABOX_PREFIX . 'trans_payment_id', $_REQUEST['razorpay_order_id']);
						update_post_meta($invoice_id, JOBPORTAL_METABOX_PREFIX . 'trans_payer_id', $user_id);
					} else {
						$invoice_id = $this->jobportal_invoice->insert_invoice('Package', $package_id, $user_id, 0, $payment_method, 1, $_REQUEST['razorpay_order_id'], $user_id, true);
					}
					$invoice_pdf_id = get_post_meta($invoice_id, JOBPORTAL_METABOX_PREFIX . 'invoice_pdf_id', true);
					$this->jobportal_package->activate_package_with_cleanup($user_id, $package_id, $invoice_pdf_id);
					...
					do_action('jobportal_razor_payment_success', $user_id, $package_id, $payment_id);
				} else {
		...
		public function wire_transfer_per_package_ajax()
		{
			...
			$coupon_amount = $_POST['coupon_amount'] ? intval($_POST['coupon_amount']) : 0;
			$package_id = intval($package_id);
			$total_price = get_post_meta($package_id, JOBPORTAL_METABOX_PREFIX . 'package_price', true);
        ...
        * Send email
      * */
			jobportal_send_email($user_email, 'mail_new_wire_transfer', $args);
			jobportal_send_email($admin_email, 'admin_mail_new_wire_transfer', $args);
			do_action('jobportal_wire_transfer_payment_success', $user_id, $package_id, $invoice_id);
			$payment_completed_link = jobportal_get_permalink('payment_completed');
		...
		public function free_package_ajax()
		{
			...
			// Insert invoice
			$invoice_id = $this->jobportal_invoice->insert_invoice('Package', $package_id, $user_id, 0, $payment_method, 1);
			$invoice_pdf_id = get_post_meta($invoice_id, JOBPORTAL_METABOX_PREFIX . 'invoice_pdf_id', true);
			$this->jobportal_package->activate_package_with_cleanup($user_id, $package_id, $invoice_pdf_id);
			update_user_meta($user_id, JOBPORTAL_METABOX_PREFIX . 'free_package', 'yes');
			update_user_meta($user_id, 'used_free_package', 'yes');

			do_action('jobportal_free_package_success', $user_id, $package_id, $invoice_id);
			...
			try {
				...
				if ($payment_Status == "succeeded") {
					if ($paid_submission_type == 'per_package') {
						...
						if ($invoice_id) {
							update_post_meta($invoice_id, JOBPORTAL_METABOX_PREFIX . 'invoice_payment_status', 1);
							update_post_meta($invoice_id, JOBPORTAL_METABOX_PREFIX . 'trans_payment_id', $paymentId);
							update_post_meta($invoice_id, JOBPORTAL_METABOX_PREFIX . 'trans_payer_id', $payerId);
						} else {
							$invoice_id = $this->jobportal_invoice->insert_invoice('Package', $package_id, $user_id, 0, $payment_method, 1, $paymentId, $payerId, true);
						}
						$invoice_pdf_id = get_post_meta($invoice_id, JOBPORTAL_METABOX_PREFIX . 'invoice_pdf_id', true);
						$this->jobportal_package->activate_package_with_cleanup($user_id, $package_id, $invoice_pdf_id);
						...
						do_action('jobportal_stripe_payment_success', $user_id, $package_id, $user_id, $paymentId, $payerId);
					}
				} else {
					$message = esc_html__('Transaction failed', 'jobportal-framework');
					if ($paid_submission_type == 'per_listing') {
						//Payment Stripe listing
						$jobs_id = absint(wp_unslash($_POST['jobs_id']));

						if ($payment_for == 3) {
							$this->jobportal_trans_log->insert_trans_log('Upgrade_To_Featured', $jobs_id, $user_id, 3, $payment_method, 0, $paymentId, $payerId, 0, $message);
						} else {
							if ($payment_for == 2) {
								$this->jobportal_trans_log->insert_trans_log('Listing_With_Featured', $jobs_id, $user_id, 2, $payment_method, 0, $paymentId, $payerId, 0, $message);
							} else {
								$this->jobportal_trans_log->insert_trans_log('Listing', $jobs_id, $user_id, 1, $payment_method, 0, $paymentId, $payerId, 0, $message);
							}
						}
					} else if ($paid_submission_type == 'per_package') {
						//Payment Stripe package
						$package_id = absint(wp_unslash($_POST['package_id']));
						$this->jobportal_trans_log->insert_trans_log('Package', $package_id, $user_id, 0, $payment_method, 0, $paymentId, $payerId, 0, $message);
					}
		...
		public function paypal_payment_completed()
		{
			...
			try {
				if (isset($_GET['token']) && isset($_GET['PayerID'])) {
					...
					$transfered_data = get_user_meta($user_id, JOBPORTAL_METABOX_PREFIX . 'paypal_transfer', true);
					...
					delete_user_meta($user_id, JOBPORTAL_METABOX_PREFIX . 'paypal_transfer');
					if ($json_response['state'] == 'approved') {
						if ($paid_submission_type == 'per_package') {
							...
							do_action('jobportal_paypal_payment_success', $user_id, $package_id, $paymentId, $payerId);
						}
					} else {
						$message = esc_html__('Transaction failed', 'jobportal-framework');
						if ($paid_submission_type == 'per_listing') {
