...
if (!class_exists('JobPortal_candidate_package')) {
    /**
     * Class JobPortal_candidate_package
     */
    class JobPortal_candidate_package
    {
        /**
        ...
        public function insert_user_candidate_package($user_id, $candidate_package_id)
        {
            ...
            $candidate_package_number_service = get_post_meta($candidate_package_id, JOBPORTAL_METABOX_PREFIX . 'candidate_package_number_service', true);
            $candidate_package_number_service_featured = get_post_meta($candidate_package_id, JOBPORTAL_METABOX_PREFIX . 'candidate_package_number_service_featured', true);
            ...
            // Reset service quota
            update_user_meta($user_id, JOBPORTAL_METABOX_PREFIX . 'candidate_package_number_service', $candidate_package_number_service);
            ...
            if ($enable_package_service_featured_unlimited == 1) {
                ...
                update_user_meta($user_id, JOBPORTAL_METABOX_PREFIX . 'candidate_package_number_service_featured', 999999999999999999);
            } else {
                ...
                update_user_meta($user_id, JOBPORTAL_METABOX_PREFIX . 'candidate_package_number_service_featured', $remaining_featured);
            }
	...
	foreach ($field_package as $field) {
		...
		$field_number = get_post_meta($candidate_package_id, JOBPORTAL_METABOX_PREFIX . 'candidate_package_number_' . $field, true);
		...
		if (intval($show_field) == 1) {
			...
			update_user_meta($user_id, JOBPORTAL_METABOX_PREFIX . 'candidate_package_number_' . $field, $field_number);
		}
	}

            do_action('jobportal_ajax_field_package_candidate', $user_id, $candidate_package_id);

            $time = time();
            $date = date('Y-m-d H:i:s', $time);
            update_user_meta($user_id, JOBPORTAL_METABOX_PREFIX . 'candidate_package_activate_date', $date);
            update_user_meta($user_id, JOBPORTAL_METABOX_PREFIX . 'candidate_package_id', $candidate_package_id);
            $candidate_package_key = uniqid();
            update_user_meta($user_id, JOBPORTAL_METABOX_PREFIX . 'candidate_package_key', $candidate_package_key);
        }
        ...
        public function get_expired_time($candidate_package_id,$candidate_package_user_id)
        {
            ...
            $candidate_package_activate_date = strtotime(get_user_meta($candidate_package_user_id, JOBPORTAL_METABOX_PREFIX . 'candidate_package_activate_date', true));
            $seconds = 0;
        ...
        private function count_active_featured_services($user_id)
        {
            $args = array(
                ...
                'meta_query' => array(
                    array(
                        'key' => JOBPORTAL_METABOX_PREFIX . 'service_featured',
                        'value' => '1',
                        'compare' => '='
                    )
                )
        ...
        public function get_active_package_info($user_id)
        {
            ...
            $activate_date = get_user_meta($user_id, JOBPORTAL_METABOX_PREFIX . 'candidate_package_activate_date', true);

            // Check if service feature is enabled
            $enable_post_type_service = jobportal_get_option('enable_post_type_service');
            ...
            if ($enable_post_type_service === '1') {
                $remaining_quota['service'] = intval(get_user_meta($user_id, JOBPORTAL_METABOX_PREFIX . 'candidate_package_number_service', true));
                $remaining_quota['service_featured'] = intval(get_user_meta($user_id, JOBPORTAL_METABOX_PREFIX . 'candidate_package_number_service_featured', true));
            }

            $remaining_quota['jobs_apply'] = intval(get_user_meta($user_id, JOBPORTAL_METABOX_PREFIX . 'candidate_package_number_jobs_apply', true));
            $remaining_quota['jobs_wishlist'] = intval(get_user_meta($user_id, JOBPORTAL_METABOX_PREFIX . 'candidate_package_number_jobs_wishlist', true));
            $remaining_quota['company_follow'] = intval(get_user_meta($user_id, JOBPORTAL_METABOX_PREFIX . 'candidate_package_number_company_follow', true));
        ...
        public function get_package_quotas($package_id)
        {
            ...
            // Service quotas
            $quotas['service'] = intval(get_post_meta($package_id, JOBPORTAL_METABOX_PREFIX . 'candidate_package_number_service', true));
            $quotas['service_unlimited'] = get_post_meta($package_id, JOBPORTAL_METABOX_PREFIX . 'enable_package_service_unlimited', true) == 1;

            $quotas['service_featured'] = intval(get_post_meta($package_id, JOBPORTAL_METABOX_PREFIX . 'candidate_package_number_service_featured', true));
            $quotas['service_featured_unlimited'] = get_post_meta($package_id, JOBPORTAL_METABOX_PREFIX . 'enable_package_service_featured_unlimited', true) == 1;

            // Field quotas
            $fields = array('jobs_apply', 'jobs_wishlist', 'company_follow', 'send_message', 'review_and_commnent');
            foreach ($fields as $field) {
                $quotas[$field] = intval(get_post_meta($package_id, JOBPORTAL_METABOX_PREFIX . 'candidate_package_number_' . $field, true));
                $quotas[$field . '_unlimited'] = get_post_meta($package_id, JOBPORTAL_METABOX_PREFIX . 'enable_package_' . $field . '_unlimited', true) == 1;
                $quotas[$field . '_enabled'] = get_post_meta($package_id, JOBPORTAL_METABOX_PREFIX . 'show_package_' . $field, true) == 1;
            }
        ...
        public function get_current_resource_counts($user_id)
        {
            ...
            $featured_count = $wpdb->get_var($wpdb->prepare(
                ...
                AND pm.meta_value = '1'",
                $user_id,
                JOBPORTAL_METABOX_PREFIX . 'service_featured'
            ));

            // Get wishlist and follow counts
            $wishlist = get_user_meta($user_id, JOBPORTAL_METABOX_PREFIX . 'my_wishlist', true);
            $follows = get_user_meta($user_id, JOBPORTAL_METABOX_PREFIX . 'my_follow', true);
        ...
        public function auto_remove_wishlist_items($user_id, $count)
        {
            ...
            update_user_meta($user_id, JOBPORTAL_METABOX_PREFIX . 'my_wishlist', $wishlist);
        ...
        public function auto_unfollow_companies($user_id, $count)
        {
            ...
            update_user_meta($user_id, JOBPORTAL_METABOX_PREFIX . 'my_follow', $follows);
        ...
        {
            // Verify nonce
            if (!isset($_POST['nonce']) || !wp_verify_nonce($_POST['nonce'], 'jobportal_package_warning_nonce')) {
                wp_send_json_error(array('message' => esc_html__('Invalid security token', 'jobportal-framework')));
                return;
            }

            // Check user logged in
            if (!is_user_logged_in()) {
                wp_send_json_error(array('message' => esc_html__('Please login first', 'jobportal-framework')));
                return;
            }
            ...
            if (!$new_package_id) {
                wp_send_json_error(array('message' => esc_html__('Invalid package ID', 'jobportal-framework')));
                return;
            }
