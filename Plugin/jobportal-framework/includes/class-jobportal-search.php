...
if (!class_exists('JobPortal_Search')) {
    class JobPortal_Search
    {
        ...
        public function __construct()
        {
            ...
            add_action('wp_ajax_jobportal_create_fulltext_index', array($this, 'ajax_create_fulltext_index'));
            add_action('wp_ajax_jobportal_dismiss_fulltext_notice', array($this, 'ajax_dismiss_fulltext_notice'));

            // Enqueue admin script for ajax actions
            add_action('admin_enqueue_scripts', array($this, 'enqueue_admin_scripts'));

            // Hook into archive template filters for initial page load optimization
            add_filter('jobportal/archive-jobs/layout-default/query/args', array(__CLASS__, 'optimize_args'), 10, 1);
            add_filter('jobportal/archive-service/layout-default/query/args', array(__CLASS__, 'optimize_args'), 10, 1);
            add_filter('jobportal/archive-candidate/layout-default/query/args', array(__CLASS__, 'optimize_args'), 10, 1);
            add_filter('jobportal/archive-company/layout-default/query/args', array(__CLASS__, 'optimize_args'), 10, 1);
        }

        public function enqueue_admin_scripts($hook)
        {
            if (strpos($hook, 'jobportal_page_jobportal-setup') === false && strpos($hook, 'theme-options') === false) {
                // return; // Or check specific pages
            }
            wp_enqueue_script('jobportal-admin-search', JOBPORTAL_PLUGIN_URL . 'assets/js/admin/jobportal-search.js', array('jquery'), JOBPORTAL_THEME_VERSION, true);
            wp_localize_script('jobportal-admin-search', 'jobportal_search_vars', array(
                'ajax_url' => admin_url('admin-ajax.php'),
                'nonce_create_index' => wp_create_nonce('jobportal_create_fulltext_index'),
                'nonce_dismiss_notice' => wp_create_nonce('jobportal_dismiss_fulltext_notice'),
            ));
        }
        ...
        {
            if (defined('JOBPORTAL_ENABLE_FULLTEXT_SEARCH') && JOBPORTAL_ENABLE_FULLTEXT_SEARCH) {
                return true;
            }
            $option = jobportal_get_option('enable_fulltext_search', '0');
            return $option === '1';
        }
        ...
        public static function optimize_args($args)
        {
            ...
            switch ($search_plugin) {
                ...
                //     break;
                default:
                    $args['jobportal_optimize_search'] = true;
                    break;
            }
        ...
        public function posts_search_filter($search, $wp_query)
        {
            ...
            if (!$is_supported && empty($wp_query->get('jobportal_optimize_search'))) {
                return $search;
            }
        ...
        public function filter_search_post_types($query)
        {
            ...
            if ($query->get('jobportal_bypass_search_filter')) {
                return;
            }
        ...
        public function fulltext_index_notice()
        {
            ...
            $dismissed = get_option('jobportal_dismiss_fulltext_notice');
            if ($dismissed) {
                return;
            }

            $class = 'notice notice-warning is-dismissible jobportal-fulltext-notice';
            $message = esc_html__('JobPortal Theme: You have enabled FULLTEXT search but the database index is missing. Please create the index to improve search performance.', 'jobportal-framework');
            $btn_text = esc_html__('Create Index Now', 'jobportal-framework');

            printf(
                '<div class="%1$s"><p>%2$s</p><p><button id="jobportal-create-fulltext-index" class="button button-primary">%3$s</button> <span class="spinner" style="float:none"></span></p></div>',
                esc_attr($class),
                esc_html($message),
                esc_html($btn_text)
            );
        ...
        {
            check_ajax_referer('jobportal_create_fulltext_index', 'security');
            ...
            global $wpdb;
            $table = $wpdb->posts;
            $index_name = 'jobportal_fulltext_idx';

            // Check if index exists again to be safe
            if (self::fulltext_index_exists()) {
                wp_send_json_success(array('message' => esc_html__('Index already exists.', 'jobportal-framework')));
            }
            ...
            if ($wpdb->query($sql) === false) {
                wp_send_json_error(array('message' => esc_html__('Failed to create index: ', 'jobportal-framework') . $wpdb->last_error));
            }

            self::$fulltext_index_exists = true;

            wp_send_json_success(array('message' => esc_html__('Index created successfully!', 'jobportal-framework')));
        }
        ...
        {
            check_ajax_referer('jobportal_dismiss_fulltext_notice', 'security');
            update_option('jobportal_dismiss_fulltext_notice', 1);
