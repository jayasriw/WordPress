...
class JobPortal_Quick_URL_Fix
{
    ...
    public static function maybe_show_fix_notice()
    {
        ...
        if (self::check_sites_2_urls()) {
            add_action('admin_notices', function() {
                ?>
                <div class="notice notice-warning is-dismissible" id="jobportal-fix-urls-notice">
                    <p>
                        <strong>⚠️ Phát hiện URLs không đúng:</strong>
                        Có media URLs chứa <code>/sites/2/</code> cần được sửa.
                        <a href="<?php echo admin_url('tools.php?page=jobportal-elementor-url-replace'); ?>" class="button button-primary">🔧 Sửa ngay</a>
                        <button type="button" class="button" onclick="jobportalQuickFixUrls()">⚡ Quick Fix</button>
                    </p>
                </div>

                <script>
                function jobportalQuickFixUrls() {
                    if (!confirm('Chạy Quick Fix để loại bỏ /sites/2/ URLs?\n\nLưu ý: Nên backup database trước!')) {
                        return;
                    }

                    const notice = document.getElementById('jobportal-fix-urls-notice');
                    notice.innerHTML = '<p>⏳ Đang chạy Quick Fix...</p>';

                    fetch(ajaxurl, {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
                        body: 'action=jobportal_quick_fix_urls&_wpnonce=<?php echo wp_create_nonce('jobportal_quick_fix'); ?>'
                    })
    ...
    {
        if (!check_ajax_referer('jobportal_quick_fix', '_wpnonce', false)) {
    ...
    {
        // Add AJAX handler
        add_action('wp_ajax_jobportal_quick_fix_urls', [__CLASS__, 'ajax_quick_fix']);
    ...
    public static function auto_fix_after_import($selected_import)
    {
        try {
            ...
            $timestamp = wp_next_scheduled('jobportal_delayed_url_fix');
            if ($timestamp) {
                wp_unschedule_event($timestamp, 'jobportal_delayed_url_fix');
            }

            // Schedule new delayed event
            wp_schedule_single_event(time() + 30, 'jobportal_delayed_url_fix');

            // Register the hook
            if (!has_action('jobportal_delayed_url_fix', [__CLASS__, 'run_delayed_url_fix'])) {
                add_action('jobportal_delayed_url_fix', [__CLASS__, 'run_delayed_url_fix']);
            }
