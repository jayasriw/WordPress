...
(function ($) {
	...
	JOBS = {
		init: function () {
			...
			this.toggle_review();
			this.apply_popup();
			this.jobportal_alert_message();
			this.jobportal_crop_image();
		...
		toggle_overview: function () {
			...
			var $wrapper = $(".jobportal-description-details");
			var $height_des = $(".jobportal-description").height();
			$wrapper.addClass("on");
			$show.on('click', function (e) {
				e.preventDefault();
				$wrapper.removeClass("on");
		...
		toggle_review: function () {
			...
			var $wrapper = $(".jobportal-review-details");
			var $height_des = $(".jobportal-review").height();
			$wrapper.addClass("on");
			$show.on('click', function (e) {
				e.preventDefault();
				$wrapper.removeClass("on");
		...
		apply_popup: function () {
			...
			var $bottom_bar = $('.jobportal-apply-bottombar');
			...
			$form_popup.each(function () {
				...
				var $btn_popup = $('.jobportal-button-apply.' + ($(this).attr('id')));
		...
		jobportal_alert_message: function () {
			$("body").on("click", ".btn-add-to-message", function (e) {
				e.preventDefault();
				var $text = $(this).data("text");
				var $html = '<div class="jobportal_alert_message fadeInRight">' + $text + "</div>";
				$("body").find(".jobportal_alert_message").remove();
				$("body").append($html).fadeIn(500);
				setTimeout(function () {
					$("body").find(".jobportal_alert_message").removeClass("fadeInRight").addClass("fadeOutRight show");
				}, 2000);
			});
		},

		jobportal_crop_image: function () {
			var $crop_image = $('.jobportal_crop_image img'),
		...
		handling_closing_date: function () {
			...
			function getExpiryText(daysDiff) {
				const hasI18n = typeof jobportal_jobs_vars !== 'undefined';

				if (daysDiff <= 0) {
					return hasI18n ? jobportal_jobs_vars.expires_past_date : 'Expires in past date';
				} else if (daysDiff === 1) {
					return hasI18n ? jobportal_jobs_vars.expires_one_day : 'Expires in 1 day';
				} else {
					return hasI18n
						? jobportal_jobs_vars.expires_many_days.replace('%d', daysDiff)
