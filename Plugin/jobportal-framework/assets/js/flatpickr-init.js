...
(function($) {
	...
	const Utils = {
		...
		getLocaleConfig: function() {
			try {
				...
				// Priority 2: jobportal_flatpickr_vars
				if (typeof jobportal_flatpickr_vars !== "undefined") {
					return {
						locale: jobportal_flatpickr_vars.locale || DEFAULTS.LOCALE,
						wpLocale: jobportal_flatpickr_vars.wp_locale,
						dateFormat: jobportal_flatpickr_vars.date_format || DEFAULTS.DATE_FORMAT,
						source: "flatpickr_vars"
					};
				}

				// Priority 3: jobportal_template_vars
				if (typeof jobportal_template_vars !== "undefined") {
					return {
						locale: jobportal_template_vars.flatpickr_locale || DEFAULTS.LOCALE,
						wpLocale: jobportal_template_vars.wp_locale,
						dateFormat: jobportal_template_vars.date_format || DEFAULTS.DATE_FORMAT,
				...
				console.error("JobPortalFlatpickr: Error reading locale config", e);
				return {
	...
	const LocaleManager = {
		...
		getFlatpickrLocale: function(localeCode, wpLocale) {
			try {
				...
				console.warn("JobPortalFlatpickr: Error loading locale", localeCode, e);
				return null;
			}
		},
	...
	/**
	 * Main JobPortalFlatpickr object
	 */
	window.JobPortalFlatpickr = {
		initialized: false,
		...
		init: function(options) {
			// Validation
			if (typeof flatpickr === "undefined") {
				console.warn("JobPortalFlatpickr: flatpickr library not loaded");
				return;
			}

			if (typeof $ === "undefined") {
				console.warn("JobPortalFlatpickr: jQuery not loaded");
				return;
			}
			...
			try {
				...
				console.error("JobPortalFlatpickr: Error in init", e);
			} finally {
		...
		createOnChangeHandler: function() {
			return function(selectedDates, dateStr, instance) {
				try {
					...
					console.warn("JobPortalFlatpickr: Error in onChange", e);
				}
			};
		},
		...
		createOnReadyHandler: function() {
			return function(selectedDates, dateStr, instance) {
				try {
					...
					console.warn("JobPortalFlatpickr: Error in onReady", e);
				}
			};
		},
			...
			try {
				...
				$elements.each((index, element) => {
					try {
						...
						console.warn("JobPortalFlatpickr: Error initializing element", element, e);
					}
				});
			} catch (e) {
				console.error("JobPortalFlatpickr: Error in initializeElements", e);
			}
		},
	...
	const DOMObserver = {
		...
		init: function() {
			if (typeof MutationObserver === "undefined") {
				console.warn("JobPortalFlatpickr: MutationObserver not supported");
				return;
			}

			// Create debounced init function
			this.debouncedInit = Utils.debounce(() => {
				if (typeof window.JobPortalFlatpickr !== "undefined") {
					window.JobPortalFlatpickr.autoInit();
				}
