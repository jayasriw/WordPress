(function ($) {
	...
	var ajax_url = jobportal_admin_vars.ajax_url;

	if( $(window).width() < 767 ){
		$("#toplevel_page_jobportal_jobs > a, #toplevel_page_jobportal_employer > a, #toplevel_page_jobportal_candidate > a, #toplevel_page_jobportal_extensions > a, #toplevel_page_jobportal_builder > a").on("click", function (event) {
	...
	$("body").on("click", ".btn-close,.bg-overlay", function () {
		$(this).parents(".jobportal-popup").removeClass("active");
	...
	$(".claim-wrap").on("click", "input.button", function (event) {
		...
		$.ajax({
			...
			dataType: "json",
			data: {
				action: "jobportal_action_claim_listing",
				status: status,
				jobs_id: jobs_id,
			},
			beforeSend: function () {
				$(".claim-wrap").find(".jobportal-loading-effect").fadeIn();
			},
			success: function (data) {
				$(".claim-wrap").find(".jobportal-loading-effect").fadeOut();
	...
	// Handle AJAX actions for jobs (Approve, Expire, Hide, Show)
	$("body").on("click", ".jobportal-ajax-action", function (event) {
		event.preventDefault();
		var $link = $(this);
		...
		// Disable link during processing
		$link.css("pointer-events", "none").text("Processing...");
		...
		$.ajax({
			...
			dataType: "json",
			data: {
				action: "jobportal_ajax_" + action + "_job",
				post_id: postId,
				nonce: nonce,
			},
			success: function (response) {
				if (response.success) {
					...
					// Reload the page to update the row actions based on new status
					location.reload();
				} else {
					// Show error message
					var errorMsg = typeof response.data !== "undefined" && typeof response.data.message !== "undefined"
	...
	$(".jobportal-countries-settings .remove-all").on("click", function (event) {
		event.preventDefault();
		$(this)
			.parents(".jobportal-countries-settings")
	...
	function hide_field() {
		$("#jobportal-applicants_jobs_id").hide();
		$("#jobportal-applicants_date").hide();
		$("#jobportal-jobs_salary_convert_min").hide();
		$("#jobportal-jobs_salary_convert_max").hide();
		$("#jobportal-jobs_price_convert_min").hide();
		$("#jobportal-jobs_price_convert_max").hide();
	}
	...
	function ajaxCall(data) {
		$.ajax({
			...
			url: ajax_url,
			data: {
				action: "jobportal_import_demo_data",
			},
			beforeSend: function () {
	...
	function plugin_action() {
		$(".jobportal-plugin-action").on("click", function (e) {
			e.preventDefault();

			var $el = $(e.currentTarget),
				$pluginsTable = $(".jobportal-box--plugins table"),
				$pluginRow = $el.closest(".jobportal-plugin--required"),
			...
			$.ajax({
				...
				if (response.success) {
					if ("deactivate-plugin" === pluginAction) {
						$pluginRow
							.removeClass("jobportal-plugin--activated")
							.addClass("jobportal-plugin--deactivated");
						$el
              .text("Activate")
              .attr("data-plugin-action", "activate-plugin")
              .attr("data-nonce", response.data.nonce)
              ...
              .addClass("plugin-activate");
					...
					if ("activate-plugin" === pluginAction) {
						$pluginRow
							.removeClass("jobportal-plugin--deactivated")
							.addClass("jobportal-plugin--activated");
						$el
              .text("Deactivate")
              ...
              .addClass("plugin-deactivate");
					...
					var requiredPluginCount = $pluginsTable.find(
							".jobportal-plugin--required.jobportal-plugin--deactivated"
						).length,
						$pluginCount = $(".jobportal-box--plugins .jobportal-box__footer span");
	...
	function additionalFieldProcess() {
        ...
        //candidate
        $wrapCandidateField.on("glf_add_clone_field", function (event) {
            var $target = $(event.target);
            additionalFieldEventCandidate($target);
            ...
            $target
                .find(".glf-field-panel-content,.glf-clone-field-panel-inner")
                .find(
                    "#custom_field_candidate_label input,#custom_field_candidate_id input,#custom_field_candidate_select_choices textarea"
                )
                .val("");

            $target
                .find(".glf-field-panel-content,.glf-clone-field-panel-inner")
                .find("#custom_field_candidate_field_type select")
                .val("text");
        });
	...
	function additionalFieldReadOnly() {
		$(".glf-field-panel-content,.glf-clone-field-panel-inner").each(
			function () {
                ...
                var $label_candidate = $(this).find("#custom_field_candidate_label input"),
                    $id_candidate = $label_candidate
                        .closest(".glf-field-panel-content,.glf-clone-field-panel-inner")
                        .find("#custom_field_candidate_id input");
                if ($id_candidate.val() !== "") {
                    $id_candidate.attr("readonly", "readonly");
                }
    ...
    function additionalFieldEventCandidate($wrapCandidate) {
        $wrapCandidate
            .find(".glf-field-panel-content,.glf-clone-field-panel-inner")
            .find("#custom_field_candidate_label input")
            .on("change", function () {
                var $label = $(this),
                    $id = $label
                        .closest(".glf-field-panel-content,.glf-clone-field-panel-inner")
	...
	function additionalFeaturesProcess() {
		var $wrap = $("#jobportal_additional_features");
		...
		$wrap.find("button").on("click", function () {
			...
			var html =
				...
				"    </td>\n" +
				'    <td class="title">\n' +
				'        <input type="text" name="jobportal_additional_feature_title[' +
				count +
				']" value="">\n' +
				"    </td>\n" +
				'    <td class="value">\n' +
				'        <input type="text" name="jobportal_additional_feature_value[' +
				count +
				']" value="">\n' +
				"    </td>\n" +
	...
	function salary_convert() {
		var $salary_rate = $('select[name="jobportal-jobs_salary_rate"]');
		var $salary_minimum = $('input[name="jobportal-jobs_salary_minimum"]');
		var $salary_maximum = $('input[name="jobportal-jobs_salary_maximum"]');
		var $maximum_price = $('input[name="jobportal-jobs_maximum_price"]');
		var $minimum_price = $('input[name="jobportal-jobs_minimum_price"]');
		var $salary_show = $('select[name="jobportal-jobs_salary_show"]');
		...
		var $salary_convert_min = $('input[name="jobportal-jobs_salary_convert_min"]');
		var $salary_convert_max = $('input[name="jobportal-jobs_salary_convert_max"]');
		var $price_convert_min = $('input[name="jobportal-jobs_price_convert_min"]');
		var $price_convert_max = $('input[name="jobportal-jobs_price_convert_max"]');
		var $convert_rate = "";
		...
		$salary_show
			.on('change', function () {
				...
				if (salary_show == "range") {
					...
					$price_convert_min.val("");
					$price_convert_max.val("");
					$("#jobportal-jobs_salary_convert_min").show();
					$("#jobportal-jobs_salary_convert_max").show();
					$("#jobportal-jobs_price_convert_min").hide();
					$("#jobportal-jobs_price_convert_max").hide();
				}
				if (salary_show == "maximum_amount") {
					...
					$("#jobportal-jobs_salary_convert_max").hide();
					$("#jobportal-jobs_price_convert_min").hide();
					$("#jobportal-jobs_price_convert_max").show();
				}
				if (salary_show == "starting_amount") {
					...
					$("#jobportal-jobs_salary_convert_max").hide();
					$("#jobportal-jobs_price_convert_max").hide();
					$("#jobportal-jobs_price_convert_min").show();
				}
