...
(function ($) {
    ...
    FOLLOW_CANDIDATE = {
        init: function () {
            var follow_candidate_save = jobportal_template_vars.follow_save,
                follow_candidate_saved = jobportal_template_vars.follow_saved,
                package_expires = jobportal_template_vars.package_expires,
                ajax_url = jobportal_template_vars.ajax_url;

            $("body").on("click", ".jobportal-add-to-follow-candidate", function (e) {
                e.preventDefault();
                if (!$(this).hasClass("on-handle")) {
                    ...
                    $.ajax({
                        ...
                        dataType: "json",
                        data: {
                            action: "jobportal_add_to_follow_candidate",
                            candidate_id: candidate_id,
                            nonce: jobportal_ajax_nonce.follow_candidate,
                        },
                        beforeSend: function () {
                        ...
                        success: function (data) {
                            if (data.added) {
                                ...
                                $this.parents(".jobportal-candidate-item").removeClass("removed-follow_candidate");
                                $this.html('<span class="icon-plus"><i class="far fa-check"></i></span>' + follow_candidate_saved);
                            } else {
                                $this.removeClass("added").addClass("removed");
                                $this.parents(".jobportal-candidate-item").addClass("removed-follow_candidate");
