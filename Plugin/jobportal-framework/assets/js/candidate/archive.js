(function ($) {
  ...
  var ajax_url = jobportal_template_vars.ajax_url,
    ...
    item_amount = jobportal_candidate_archive_vars.item_amount,
    not_candidate = jobportal_candidate_archive_vars.not_candidate;
  ...
  var jobportal_hover_map_effects = function () {
    if (map_effects !== "" && has_map) {
      $(".map-event .area-candidates .jobportal-candidates-item").each(function () {
  ...
  CANDIDATE.elements = {
    init: function () {
      ...
      $(".jobportal-menu-filter").on("input", "input.input-control", function () {
    ...
    candidate_layout: function () {
      $(".candidate-layout a").on("click", function (event) {
        ...
        $(".area-candidates .jobportal-candidates-item").each(function () {
    ...
    display_clear: function () {
      ...
      var has_active_filters = (
        ...
        ($("select.jobportal-select-country").val() && $("select.jobportal-select-country").val() !== "")
      );
      ...
      if ($("select.jobportal-select-country").val() && $("select.jobportal-select-country").val() !== "") {
        $("select.jobportal-select-country").closest(".entry-filter").addClass("open");
      }
      if (has_active_filters) {
        $(".jobportal-nav-filter").addClass("active");
        archive_candidate.find(".jobportal-clear-filter").show();
      } else {
        $(".jobportal-nav-filter").removeClass("active");
        archive_candidate.find(".jobportal-clear-filter").hide();
      }
    },
    ...
    filter_clear_top: function () {
      $(".jobportal-clear-top-filter").on("click", function () {
        ...
        $(".form-candidate-top-filter .jobportal-select2").val("");
        $(".form-candidate-top-filter .jobportal-select2").select2("destroy");
        $(".form-candidate-top-filter .jobportal-select2").each(function () {
    ...
    ajax_load: function (ajax_call, pagination) {
        ...
        var jobportal_add_markers = function (props, map) {
          $.each(props, function (i, prop) {
            ...
            google.maps.event.addListener(marker, "click", function () {
              ...
              var elem = $(marker.url);
              $(".area-candidate .jobportal-candidate-item").removeClass(
                "highlight"
              );
              if (
                elem.length > 0 &&
      ...
      var type_pagination = $(".jobportal-pagination").attr("data-type");
      $(".area-candidates .jobportal-candidates-item").addClass("skeleton-loading");

      $.ajax({
        dataType: "json",
        url: ajax_url,
        data: {
          action: "jobportal_candidate_archive_ajax",
          paged: paged,
          title: title,
        ...
        success: function (data) {
          ...
          $(".area-candidates .jobportal-candidates-item").removeClass(
            "skeleton-loading"
          );
            ...
            if (ajax_call == true) {
              if (
                ...
                $(".area-candidates").html(
                  '<div class="jobportal-ajax-result">' + not_candidate + "</div>"
                );
                $(".archive-layout .result-count").html(data.count_post);
                $(".jobportal-pagination .pagination").html("");
              } else {
          ...
          if (data.tax_with_count) {
            ...
            if (data.tax_with_count != "not_found") {
              $.each(data.tax_with_count, function (index, value) {
                $(
                  '.jobportal-menu-filter li input[value="' +
                  index +
                  '"] + label span.count'
                ).text("(" + value + ")");
              });
          ...
          if (has_map) {
            loadGoogleMapsAPI().then(function () {
              if (mapType == "google_map") {
                ...
                markers = [];
                jobportal_add_markers(data.candidate, jobportal_map);
                ...
                jobportal_osm_add_markers(data.candidate, maps);
              } else {
                jobportal_mapbox_add_markers(data.candidate, map);
              }
            });
          }
          // Scroll to first item after pagination
          if (pagination === "pagination") {
            CANDIDATE.elements.scroll_to(".area-candidates .jobportal-candidates-item:first");
          }
        },
      });
    },
