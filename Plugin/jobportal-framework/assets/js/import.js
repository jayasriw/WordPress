...
var civi = (($) => {
	return {
		...
		updateTheme() {
			$(".jobportal-update-btn").on("click", (e) => {
				$(e.currentTarget)
		...
		refreshTransients() {
			$(".jobportal-box--update__refresh").on("click", (e) => {
				e.preventDefault();

				var $el = $($(e.currentTarget));
				$el.find("i, .svg-inline--fa").addClass("jobportal-spin");

				$.ajax({
					type: "POST",
					url: jobportal_import_vars.ajax_url,
					data: {
		...
		processPluginActions() {
			$(".jobportal-plugin-link").on("click", (e) => {
				e.preventDefault();
						...
						if (response.success) {
							if ("deactivate-plugin" === pluginAction) {
								...
								$el
									...
									.removeClass("jobportal-plugin-link--deactivate")
									.addClass("jobportal-plugin-link--activate");
							...
							if ("activate-plugin" === pluginAction) {
								...
								$el
									...
									.attr("data-nonce", response.data)
									.removeClass("jobportal-plugin-link--activate")
									.addClass("jobportal-plugin-link--deactivate");
		...
		refreshPlace: function ($this) {
			$(".jobportal-import-refresh__button").on("click", (e) => {
				e.preventDefault();
				...
				$.ajax({
					...
					success: function (response) {
						...
						$("#jobportal-import-demo-popup").html(response.data);

						$.magnificPopup.open({
							items: {
								src: "#jobportal-import-demo-popup",
								type: "inline",
							},
							modal: true,
							removalDelay: 300,
		...
		goToChangelog() {
			$("#go-to-changelog").on("click", (e) => {
				...
				$("html, body").animate({
					scrollTop: $(".jobportal-box--changelog").offset().top,
				});
			});
		},

		applyPatch() {
			$(".jobportal-apply-patch").on("click", (e) => {
				e.preventDefault();

				var $el = $(e.currentTarget),
					$error = $(".jobportal-error-text"),
					ajaxData = {
						action: "apply_patch",
				...
				$(".jobportal-apply-patch").attr("disabled", true);
				$el.html(
		...
		fetchDemoSteps() {
			$(".jobportal-import-demo__button").on("click", (e) => {
				e.preventDefault();
		...
		selectDemoSteps() {
			$(document).on("click", ".jobportal-demo-steps__svg", (e) => {
				$(e.currentTarget).prev('input[type="checkbox"]').trigger("click");
			});

			$(document).on("change", "#jobportal-all-demo-steps", (e) => {
				var $checkbox = $(e.currentTarget);

				if ($checkbox.is(":checked")) {
					$(".jobportal-demo-steps__checkbox").not($checkbox).attr("checked", true);
				} else {
					$(".jobportal-demo-steps__checkbox").not($checkbox).attr("checked", false);
				}
			});

			$(document).on("change", ".jobportal-demo-steps__checkbox", (e) => {
				var $checkbox = $(e.currentTarget),
					$checkAll = $("#jobportal-all-demo-steps"),
					uncheckCount = 0;

				if ($checkbox.is(":checked")) {
					$(".jobportal-demo-steps__checkbox")
						.not($checkAll)
						.each((idx, chkbox) => {
		...
		closeImportPopup() {
			$(document).on("click", ".jobportal-popup__close-button", (e) => {
				e.preventDefault();
				$.magnificPopup.close();
			});
		},
		importDemo() {
			$(document).on("submit", "#demo-steps-form", (e) => {
				...
				var $form = $(e.currentTarget),
					$popup = $("#jobportal-import-demo-popup"),
					$error = $form.find(".jobportal-error-text"),
					formData = $form.serialize();

				$popup.addClass("jobportal-loading");
				$error.hide();
				...
				$.ajax({
					...
					.done((response) => {
						...
						$popup.removeClass("jobportal-loading");
					})
					.fail((jqXHR, textStatus) => {
						console.error(`${jqXHR.responseText}: ${jqXHR.status}`);
						console.error(`${textStatus}`);
		...
		downloadMediaPackage() {
			...
			$("#download-media-package-form").on("submit", (e) => {
				...
				var $form = $(e.currentTarget),
					$error = $form.find(".jobportal-error-text"),
					$note = $form.find(".jobportal-popup__note"),
					$closeButton = $form.find(".jobportal-popup__close-button"),
					$progressBar = $form.find(".jobportal-progress-bar"),
					formData = $form.serialize();
							...
							var downloadPromise = new Promise((resolve, reject) => {
								...
								xhr.onprogress = (e) => {
									if (0 < e.total) {
										...
										$progressBar
											.find(".jobportal-progress-bar__inner")
											.css("width", `${percent}%`);
										$progressBar
											.find(".jobportal-progress-bar__text")
		...
		copyImages() {
			$("#copy-images-form").on("submit", (e) => {
				...
				var $popup = $("#jobportal-import-demo-popup"),
					$form = $(e.currentTarget),
					$error = $form.find(".jobportal-error-text"),
					$note = $form.find(".jobportal-popup__note"),
					$closeButton = $form.find(".jobportal-popup__close-button"),
					$title = $form.find(".jobportal-popup__title"),
					formData = $form.serialize();
				...
				this.playLottie(
					$title[0],
					jobportal_import_vars.animation_url + "file-copying.json",
					true
				);
						...
						if (response.success) {
							if (response.data) {
								...
								if ($("#import-success").length) {
									this.playLottie(
										$("#import-success .jobportal-popup__subtitle")[0],
										jobportal_import_vars.animation_url + "star-success.json",
										true
									);
								}
							}
		...
		importData() {
			var $firstStep = $(
					"#import-content-wrapper .jobportal-import-content__item:first-child"
				),
				$title = $("#import-content-wrapper .jobportal-popup__title"),
				data = {
			...
			this.playLottie(
				$title[0],
				jobportal_import_vars.animation_url + "import-content.json",
				true
			);
		...
		runImportContentAjax(data) {
			var $wrapper = $("#import-content-wrapper"),
				$popup = $("#jobportal-import-demo-popup"),
				$error = $wrapper.find(".jobportal-error-text"),
				$note = $wrapper.find(".jobportal-popup__note"),
				$closeButton = $wrapper.find(".jobportal-popup__close-button"),
				$title = $wrapper.find(".jobportal-popup__title");
			...
			$.ajax({
				...
				.done((response) => {
					if (
						...
						$("#import-content-wrapper .jobportal-import-content__item:last-child")
							.find("i, .svg-inline--fa")
							.removeClass("la-circle-notch la-spin")
							.addClass("la-check");
						setTimeout(() => {
						...
						$title.find("svg").remove();
						$(".jobportal-import-content-list").hide();
						$note.css({
							opacity: "0",
							visibility: "hidden",
						});
...
jQuery(document).ready(() => civi.init());
