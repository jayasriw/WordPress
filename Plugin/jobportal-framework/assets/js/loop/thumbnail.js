(function ($) {
	"use strict";
	var ajax_url = jobportal_thumbnail_vars.ajax_url,
		thumbnail_title = jobportal_thumbnail_vars.thumbnail_title,
		thumbnail_type = jobportal_thumbnail_vars.thumbnail_type,
		thumbnail_file_size = jobportal_thumbnail_vars.thumbnail_file_size,
		thumbnail_text = jobportal_thumbnail_vars.thumbnail_text,
		thumbnail_url = jobportal_thumbnail_vars.thumbnail_url,
		thumbnail_upload_nonce = jobportal_thumbnail_vars.thumbnail_upload_nonce;

	jQuery(document).ready(function () {
		var jobportal_thumbnail = function () {
			var uploader_thumbnail = new plupload.Uploader({
				browse_button: "jobportal_select_thumbnail",
				file_data_name: "jobportal_thumbnail_upload_file",
				drop_element: "jobportal_thumbnail_view",
				container: "jobportal_thumbnail_container",
				url: thumbnail_url,
				filters: {
					mime_types: [
						{
			...
			uploader_thumbnail.bind("UploadProgress", function (up, file) {
				document.getElementById("jobportal_select_thumbnail").innerHTML =
			...
			uploader_thumbnail.bind("Error", function (up, err) {
				document.getElementById("jobportal_thumbnail_errors").innerHTML +=
					"Error #" + err.code + ": " + err.message + "<br/>";
			});

			var $image_id = $("#jobportal_thumbnail_view").data("image-id");
			var $image_url = $("#jobportal_thumbnail_view").data("image-url");
			if ($image_id && $image_url) {
				var $html =
					...
					"</div>" +
					"</figure>";
				$("#jobportal_thumbnail_view").html($html);
				$("#jobportal_add_thumbnail").hide();
			}
			uploader_thumbnail.bind(
				"FileUploaded",
				function (up, file, ajax_response) {
					document.getElementById("jobportal_drop_thumbnail").style.display = "none";
					...
					if (response.success) {
						...
						$("#jobportal_add_thumbnail .la-upload").hide();
		...
		var jobportal_thumbnai_delete = function ($type) {
			$("body").on("click", ".icon-thumbnail-delete", function (e) {
				...
				var $this = $(this),
					icon_delete = $this,
					thumbnail = $this
						.closest("#jobportal_thumbnail_view")
						.find(".media-thumb-wrap"),
					attachment_id = $this.data("attachment-id"),
					$drop = $("#jobportal_drop_thumbnail");
				...
				$.ajax({
					...
					success: function (response) {
						if (response.success) {
							...
							$("#jobportal_add_thumbnail").show();
						}
						icon_delete.html('<i class="fal fa-spinner fa-spin large"></i>');
						$drop.css("display", "block");
						$("#jobportal_add_thumbnail .la-upload").show();
						$("#jobportal_select_thumbnail").html(thumbnail_text);
