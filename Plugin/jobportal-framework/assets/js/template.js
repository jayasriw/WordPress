  ...
  GLF.element = {
    ...
    package_coupon: function () {
      $(".package-coupon-form").on("submit", function (e) {
        ...
        var jobportal_coupon = $(this).find('input[name="jobportal_coupon"]');
        if (coupon_code.val() != "" && coupon_code.val() == jobportal_coupon.val()) {
        ...
        $.ajax({
          ...
          success: function (data) {
            ...
            if (data.success) {
              ...
              $(".jobportal-total-price")
                .find(".old-price")
                .html(data.package_price_format);
              $(".jobportal-total-price")
                .find(".price")
                .html(data.package_price_with_coupon);
              $(".jobportal-stripe-form")
                .find('input[name="payment_money"]')
                .val(data.stripe_price);
              // Update the amount in jobportal_stripe_vars
              if (typeof jobportal_stripe_vars !== "undefined") {
                if (
                  typeof jobportal_stripe_vars.jobportal_stripe_per_package !==
                  "undefined"
                ) {
                  jobportal_stripe_vars.jobportal_stripe_per_package.params.amount =
                    data.stripe_price;
                }
                if (
                  typeof jobportal_stripe_vars.jobportal_stripe_candidate_per_package !==
                  "undefined"
                ) {
                  jobportal_stripe_vars.jobportal_stripe_candidate_per_package.params.amount =
                    data.stripe_price;
                }
              }
            } else {
              coupon_code.val("");
              coupon_amount.val("");
              $(".jobportal-total-price").find(".old-price").html("");
              $(".jobportal-total-price")
                .find(".price")
                .html(data.package_price_format);
              $(".jobportal-stripe-form")
    ...
    payment_method: function () {
      $(".jobportal-payment-method-wrap .radio").on("click", function () {
        $(".jobportal-payment-method-wrap .radio").removeClass("active");
    ...
    select2: function () {
      ...
      $('.jobportal-ajax-select2').select2({
      ...
      if ($(".elementor-editor-active").length) {
        elementorFrontend.hooks.addAction(
          ...
          function ($scope) {
            $scope.find(".jobportal-select2").select2();
          }
        );
      }
      ...
      $(".jobportal-select2.prefix-code").on("select2:select", function () {
        var group = $(this).closest(".tel-group");
        var rendered = $(this).find("option:selected").val();
      ...
      // Autocomplete for jobportal-ajax-ui
      $('.jobportal-ajax-ui').autocomplete({
        minLength: 0,
        delay: 500,
      ...
      $('.jobportal-ajax-ui').on("focus", function () {
        if ($(this).val().length >= 0) {
          $(this).autocomplete("search", $(this).val());
        }
      });

      // Select2 for jobportal-ajax-select2
      $('.jobportal-ajax-select2').each(function() {
      ...
      $('.jobportal-clear-top-filter').on('click', function() {
        ...
        form.find('.jobportal-select2, .jobportal-ajax-select2').val('').trigger('change');
      });
    },
    ...
    filter_toggle: function () {
      ...
      $(".archive-filter").on(
        "click",
        ".btn-close,.bg-overlay,.show-result .jobportal-button",
        function (e) {
          e.preventDefault();
    ...
    slick_carousel: function () {
      ...
      $(".jobportal-slick-carousel").each(function () {
        var slider = $(this);
        var defaults = {
          slidesToShow: 1,
          slidesToScroll: 1,
