(function ($) {
	...
	$(document).ready(function () {
		var ajax_url = jobportal_meetings_vars.ajax_url;
		var $form = $("#jobportal-form-reschedule-meeting");
		var meetings_dashboard = $(".meetings-dashboard");
		var not_meetings = jobportal_meetings_vars.not_meetings;
		...
		var form_popup = "#jobportal-form-reschedule-meeting";
		...
		$("body").on("click", "#btn-meetings-reschedule", function (event) {
			...
			$.ajax({
				...
				dataType: "json",
				data: {
					action: "jobportal_meetings_reschedule_ajax",
					date: date,
		...
		$("body").on("click", "#btn-saved-meetings", function (event) {
			event.preventDefault();
			var form_settings = $("#jobportal-form-setting-meetings"),
			...
			$.ajax({
				...
				url: ajax_url,
				data: {
					action: "jobportal_meetings_settings",
					link: link,
					password: password,
				},
				beforeSend: function () {
		...
		$("body").on(
			"click",
			"#tab-upcoming .jobportal-pagination a.page-numbers",
			function (e) {
				e.preventDefault();
				tabs_upcoming
		...
		function ajax_load_upcoming(item_id = "", action_click = "") {
			...
			$.ajax({
				...
				url: ajax_url,
				data: {
					action: "jobportal_meetings_upcoming_dashboard",
		...
		$("body").on(
			"click",
			"#tab-completed .jobportal-pagination a.page-numbers",
			function (e) {
				e.preventDefault();
				tabs_completed
					.find(".jobportal-pagination li .page-numbers")
		...
		function ajax_load_completed(item_id = "", action_click = "") {
			...
			$.ajax({
				...
				url: ajax_url,
				data: {
					action: "jobportal_meetings_completed_dashboard",
					item_amount: item_amount,
					paged: paged,
					item_id: item_id,
					action_click: action_click,
		...
		//Tabs
		meetings_dashboard.find(".tab-list li").on('click', function () {
			var item_id = $(this).attr("meeting-id");
		...
		$("body").on(
			"click",
			".meetings-dashboard-candidate .jobportal-pagination a.page-numbers",
			function (e) {
				e.preventDefault();
				meetings_candidate
					.find(".jobportal-pagination li .page-numbers")
		...
		function ajax_load_candidate(item_id = "") {
			...
			$.ajax({
				...
				url: ajax_url,
				data: {
					action: "jobportal_meetings_candidate_dashboard",
					item_amount: item_amount,
					paged: paged,
					item_id: item_id,
				},
