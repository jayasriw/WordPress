(function ($) {
	...
	var my_wishlist = $(".jobportal-my-wishlist");

	var ajax_url = jobportal_my_wishlist_vars.ajax_url,
		not_jobs = jobportal_my_wishlist_vars.not_jobs;

	$(document).ready(function () {
		my_wishlist.find("select-pagination").on('change', function () {
			var number = "";
		...
		my_wishlist.find("select.search-control").on("change", function () {
			my_wishlist.find(".jobportal-pagination").find('input[name="paged"]').val(1);
			ajax_load_wishlist();
		});
		...
		my_wishlist.find("input.jobs-search-control").on('keyup',
			delay(function () {
				my_wishlist.find(".jobportal-pagination").find('input[name="paged"]').val(1);
				ajax_load_wishlist();
			}, 1000)
		...
		$("body").on(
			"click",
			".jobportal-my-wishlist .jobs-control .btn-delete",
			function (e) {
				e.preventDefault();
		...
		$("body").on(
			"click",
			".jobportal-my-wishlist .jobportal-pagination a.page-numbers",
			function (e) {
				e.preventDefault();
				my_wishlist
				...
				var current_page = 1;
				if (
					my_wishlist.find(".jobportal-pagination").find('input[name="paged"]').val()
				) {
		...
		function ajax_load_wishlist(item_id = "", action_click = "") {
			...
			paged = my_wishlist.find('.jobportal-pagination input[name="paged"]').val();

			$.ajax({
				dataType: "json",
				url: ajax_url,
				data: {
					action: "jobportal_filter_my_wishlist",
				...
				beforeSend: function () {
					my_wishlist.find(".jobportal-loading-effect").addClass("loading").fadeIn();
