(function ($) {
    ...
    var invite_candidate = $(".jobportal-invite-candidate");

    var ajax_url = jobportal_invite_candidate_vars.ajax_url,
        not_candidate = jobportal_invite_candidate_vars.not_candidate;

    $(document).ready(function () {
        invite_candidate.find("select-pagination").on('change', function () {
            var number = "";
        ...
        invite_candidate.find("select.search-control").on("change", function () {
            invite_candidate.find(".jobportal-pagination").find('input[name="paged"]').val(1);
            ajax_load_invite();
        });
        ...
        invite_candidate.find("input.search-control").on('keyup',
            delay(function () {
                invite_candidate.find(".jobportal-pagination").find('input[name="paged"]').val(1);
                ajax_load_invite();
            }, 1000)
        ...
        $("body").on("click",".jobportal-invite-candidate .action-setting .btn-delete",
            function (e) {
                e.preventDefault();
        ...
        $("body").on(
            "click",
            ".jobportal-invite-candidate .jobportal-pagination a.page-numbers",
            function (e) {
                e.preventDefault();
                invite_candidate
                ...
                var current_page = 1;
                if (
                    invite_candidate.find(".jobportal-pagination").find('input[name="paged"]').val()
                ) {
        ...
        function ajax_load_invite(item_id = "", action_click = "") {
            ...
            var candidate_search = invite_candidate.find('input[name="candidate_search"]').val(),
                ...
                candidate_sort_by = invite_candidate
                    ...
                    .val();
            paged = invite_candidate.find('.jobportal-pagination input[name="paged"]').val();

            $.ajax({
                dataType: "json",
                url: ajax_url,
                data: {
                    action: "jobportal_filter_invite_candidate",
                ...
                beforeSend: function () {
                    invite_candidate.find(".jobportal-loading-effect").addClass("loading").fadeIn();
