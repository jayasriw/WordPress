(function ($) {
  ...
  var ajax_url = jobportal_jobs_dashboard_vars.ajax_url,
    not_jobs = jobportal_jobs_dashboard_vars.not_jobs;
  ...
  $(document).ready(function () {
    ...
    jobs_dashboard.find("input.jobs-search-control").on('keyup',
      delay(function () {
        $(".jobportal-pagination").find('input[name="paged"]').val(1);
        ajax_load();
      }, 1000)
    ...
    function ajax_load(item_id = "", action_click = "") {
      ...
      $.ajax({
        ...
        data: {
          action: "jobportal_filter_jobs_dashboard",
        ...
        success: function (data) {
          if (data.success === true) {
            ...
            if (paged > 1) {
              paged = paged - 1;
              jobs_dashboard.find(".jobportal-pagination").find('input[name="paged"]').val(paged);
              ajax_load();
              return;
            }
            jobs_dashboard
