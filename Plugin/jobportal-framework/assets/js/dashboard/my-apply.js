(function ($) {
  ...
  var my_apply = $(".jobportal-my-apply");

  var ajax_url = jobportal_my_apply_vars.ajax_url,
    not_jobs = jobportal_my_apply_vars.not_jobs;

  $(document).ready(function () {
    my_apply.find("select-pagination").on('change', function () {
      var number = "";
    ...
    my_apply.find("select.search-control").on("change", function () {
      my_apply.find(".jobportal-pagination").find('input[name="paged"]').val(1);
      ajax_load_apply();
    });
    ...
    my_apply.find("input.search-control").on('keyup',
      delay(function () {
        my_apply.find(".jobportal-pagination").find('input[name="paged"]').val(1);
        ajax_load_apply();
      }, 1000)
    ...
    $("body").on(
      "click",
      ".jobportal-my-apply .jobs-control .btn-delete",
      function (e) {
        e.preventDefault();
    ...
    $("body").on(
      "click",
      ".jobportal-my-apply .jobportal-pagination a.page-numbers",
      function (e) {
        e.preventDefault();
        my_apply
        ...
        var current_page = 1;
        if (
          my_apply.find(".jobportal-pagination").find('input[name="paged"]').val()
        ) {
    ...
    function ajax_load_apply(item_id = "", action_click = "") {
      ...
      var jobs_search = my_apply.find('input[name="jobs_search"]').val(),
        ...
        paged = my_apply.find('.jobportal-pagination input[name="paged"]').val();

      $.ajax({
        dataType: "json",
        url: ajax_url,
        data: {
          action: "jobportal_filter_my_apply",
          ...
          security: theme_vars.filter_apply_nonce,
        },
        beforeSend: function () {
          my_apply.find(".jobportal-loading-effect").addClass("loading").fadeIn();
