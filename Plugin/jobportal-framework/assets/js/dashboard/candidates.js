(function ($) {
	...
	var ajax_url = jobportal_candidates_dashboard_vars.ajax_url,
		not_candidates = jobportal_candidates_dashboard_vars.not_candidates;
	...
	$(document).ready(function () {
		...
		candidates_dashboard.find("input.search-control").on('keyup',
			delay(function () {
				$(".jobportal-pagination").find('input[name="paged"]').val(1);
				ajax_load();
			}, 1000)
		...
		$("body").on(
			"click",
			".candidates-dashboard .jobportal-pagination a.page-numbers",
			function (e) {
				e.preventDefault();
		...
		function ajax_load(
			...
			$.ajax({
				...
				data: {
					action: "jobportal_filter_candidates_dashboard",
				...
				success: function (data) {
					if (data.success === true) {
						...
						if (paged > 1) {
							paged = paged - 1;
							candidates_dashboard.find(".jobportal-pagination").find('input[name="paged"]').val(paged);
							ajax_load();
							return;
						}
						candidates_dashboard
