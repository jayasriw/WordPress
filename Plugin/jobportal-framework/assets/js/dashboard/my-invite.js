(function ($) {
    ...
    var my_invite = $(".jobportal-my-invite");

    var ajax_url = jobportal_my_invite_vars.ajax_url,
        not_jobs = jobportal_my_invite_vars.not_jobs;

    $(document).ready(function () {
        my_invite.find("select-pagination").on('change', function () {
            var number = "";
        ...
        my_invite.find("select.search-control").on("change", function () {
            my_invite.find(".jobportal-pagination").find('input[name="paged"]').val(1);
            ajax_load_invite();
        });
        ...
        my_invite.find("input.jobs-search-control").on('keyup',
            delay(function () {
                my_invite.find(".jobportal-pagination").find('input[name="paged"]').val(1);
                ajax_load_invite();
            }, 1000)
        ...
        $("body").on(
            "click",
            ".jobportal-my-invite .jobs-control .btn-delete",
            function (e) {
                e.preventDefault();
        ...
        $("body").on(
            "click",
            ".jobportal-my-invite .jobportal-pagination a.page-numbers",
            function (e) {
                e.preventDefault();
                my_invite
                ...
                var current_page = 1;
                if (
                    my_invite.find(".jobportal-pagination").find('input[name="paged"]').val()
                ) {
        ...
        function ajax_load_invite(item_id = "", action_click = "") {
            ...
            paged = my_invite.find('.jobportal-pagination input[name="paged"]').val();

            $.ajax({
                dataType: "json",
                url: ajax_url,
                data: {
                    action: "jobportal_filter_my_invite",
                ...
                beforeSend: function () {
                    my_invite.find(".jobportal-loading-effect").addClass("loading").fadeIn();
