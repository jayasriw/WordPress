(function ($) {
    ...
    var follow_candidate = $(".jobportal-follow-candidate");

    var ajax_url = jobportal_follow_candidate_vars.ajax_url,
        not_candidate = jobportal_follow_candidate_vars.not_candidate;

    $(document).ready(function () {
        follow_candidate.find("select-pagination").on('change', function () {
            var number = "";
        ...
        follow_candidate.find("select.search-control").on("change", function () {
            follow_candidate.find(".jobportal-pagination").find('input[name="paged"]').val(1);
            ajax_load_follow();
        });
        ...
        follow_candidate.find("input.search-control").on('keyup',
            delay(function () {
                follow_candidate.find(".jobportal-pagination").find('input[name="paged"]').val(1);
                ajax_load_follow();
            }, 1000)
        ...
        $("body").on("click",".jobportal-follow-candidate .action-setting .btn-delete",
            function (e) {
                e.preventDefault();
        ...
        $("body").on(
            "click",
            ".jobportal-follow-candidate .jobportal-pagination a.page-numbers",
            function (e) {
                e.preventDefault();
                follow_candidate
                ...
                var current_page = 1;
                if (
                    follow_candidate.find(".jobportal-pagination").find('input[name="paged"]').val()
                ) {
        ...
        function ajax_load_follow(item_id = "", action_click = "") {
            ...
            var candidate_search = follow_candidate.find('input[name="candidate_search"]').val(),
                ...
                candidate_sort_by = follow_candidate
                    ...
                    .val();
            paged = follow_candidate.find('.jobportal-pagination input[name="paged"]').val();

            $.ajax({
                dataType: "json",
                url: ajax_url,
                data: {
                    action: "jobportal_filter_follow_candidate",
                ...
                beforeSend: function () {
                    follow_candidate.find(".jobportal-loading-effect").addClass("loading").fadeIn();
