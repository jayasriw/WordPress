(function ($) {
	...
	var ajax_url = jobportal_applicants_dashboard_vars.ajax_url,
		not_applicants = jobportal_applicants_dashboard_vars.not_applicants,
		ajax_nonce = jobportal_applicants_dashboard_vars.ajax_nonce;
	...
	$(document).ready(function () {
        ...
        function read_mess_ajax_load(item_id = "") {
            $.ajax({
                ...
                url: ajax_url,
                data: {
                    action: "jobportal_read_mess_ajax_load",
                    item_id: item_id,
                    security: ajax_nonce,
                },
                beforeSend: function () {
        ...
        function realy_mess_ajax_load(item_id = "",title = "",content = "",jobs_id = "") {
            ...
            $.ajax({
                ...
                url: ajax_url,
                data: {
                    action: "jobportal_realy_mess_ajax_load",
                    item_id: item_id,
                    title: title,
                    content: content,
                    jobs_id: jobs_id,
		...
		applicants_dashboard.find("input.search-control").on('keyup',
			delay(function () {
				$(".jobportal-pagination").find('input[name="paged"]').val(1);
				ajax_load();
			}, 1000)
		...
		function ajax_load(item_id = "", action_click = "") {
			...
			$.ajax({
				...
				data: {
					action: "jobportal_filter_applicants_dashboard",
				...
				success: function (data) {
					if (data.success === true) {
						...
						if (paged > 1) {
							paged = paged - 1;
							applicants_dashboard.find(".jobportal-pagination").find('input[name="paged"]').val(paged);
							ajax_load();
							return;
						}
						applicants_dashboard
