(function ($) {
    ...
    var service_dashboard = $(".jobportal-service-order");

    var ajax_url = jobportal_service_order_vars.ajax_url,
        form_view_reason = $("#form-service-view-reason"),
        not_service = jobportal_service_order_vars.not_service;
    ...
    $(document).ready(function () {
        ...
        function ajax_load(item_id = "", action_click = "") {
            var paged = 1;
            var height = service_dashboard.find("#candidate-service-order").height();
            ...
            $.ajax({
                ...
                url: ajax_url,
                data: {
                    action: "jobportal_candidate_order_service",
                ...
                beforeSend: function () {
                    service_dashboard
                        ...
                        .addClass("loading")
                        .fadeIn();
                    service_dashboard.find("#candidate-service-order").height(height);
                },
                success: function (data) {
                    if (data.success === true) {
                        ...
                        service_dashboard.find("#candidate-service-order tbody").fadeOut("fast", function () {
                            service_dashboard.find("#candidate-service-order tbody").html(data.service_html);
                            service_dashboard.find("#candidate-service-order tbody").fadeIn(300);
                        });
                        service_dashboard.find("#candidate-service-order").css("height", "auto");
                    } else {
                        service_dashboard
                            .find("#candidate-service-order tbody")
